        ...
        if (column.type === 'json') {
            ...
            transferList.forEach((transferItem) => {
                if ([null, undefined, ''].includes(transferItem[column.propertyName])) {
                    transferItem[column.propertyName] = null
                } else if (![DATA_TYPES.OBJECT.VAL, DATA_TYPES.ARRAY.VAL].includes(getTypeByValue(transferItem[column.propertyName]))) {
                    throw new Error(`【${column.propertyName}】字段是【json】类型，但是数据【${transferItem[column.propertyName]}】不符合json格式`)
                }
            })
        }
    })
