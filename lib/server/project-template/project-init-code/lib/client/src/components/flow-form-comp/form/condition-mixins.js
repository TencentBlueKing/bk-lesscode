...
export default {
    methods: {
        ...
        parseFieldConditions () {
            ...
            for (let i = 0; i < len; i++) {
                // 必填
                if (this.fields[i].mandatory_conditions && this.isObjectHaveAttr(this.fields[i].mandatory_conditions)) {
                ...
                // 只读
                if (this.fields[i].read_only_conditions && this.isObjectHaveAttr(this.fields[i].read_only_conditions)) {
                ...
                // 显隐
                if (!this.show_type && this.fields[i].show_conditions && this.isObjectHaveAttr(this.fields[i].show_conditions)) {
                    this.isFullfillCondition(this.fields[i].show_conditions)
                        ? this.$set(this.fields[i], 'isHide', true)
                        : this.$set(this.fields[i], 'isHide', false)
                }
            }
        },
        // 是否满足条件
        isFullfillCondition (condition) {
            ...
            if (condition.type === 'and' || condition.type === 'and') {
                // 这里需要对符号判断
