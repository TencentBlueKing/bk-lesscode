...
export default {
    methods: {
        ...
        parseFieldConditions () {
            ...
            for (let i = 0; i < len; i++) {
                const isNumber = this.fields[i].type === 'INT'
                // 必填
                if (this.isObjectHaveAttr(this.fields[i].mandatory_conditions)) {
                    this.isFullfillCondition(this.fields[i].mandatory_conditions, isNumber)
                ...
                if (this.isObjectHaveAttr(this.fields[i].read_only_conditions)) {
                    this.isFullfillCondition(this.fields[i].read_only_conditions, isNumber)
                ...
                if (this.isObjectHaveAttr(this.fields[i].show_conditions)) {
                    // 1 为展示，0为隐藏
                    this.isFullfillCondition(this.fields[i].show_conditions, isNumber)
        ...
        // 是否满足条件
        isFullfillCondition (condition, isNumber) {
            ...
            if (condition.connector === 'and' || condition.type === 'and') {
                ...
                return condition.expressions?.every((item) => {
                    ...
                    console.log(item.value, this.localValue[item.key])
                    return this[func](
                        isNumber ? Number(item.value) : item.value,
                        isNumber ? Number(this.value[item.key]) : this.value[item.key]
                    )
                })
            }
            //  or 的条件处理
            return condition.expressions?.some((item) => {
                ...
                return this[func](
                    ...
                    isNumber ? this.value[item.key] : Number(this.value[item.key])
                )
            })
        },
        // 流程详情中判断显隐
