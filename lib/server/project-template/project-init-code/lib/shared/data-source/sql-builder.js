 ...
 * @returns sql
 */
export const generateSqlByCondition = (condition = {}) => {
...
const genFieldIdNameMap = (tableList) => {
    ...
    tableList?.forEach((table) => {
        JSON.parse(table.columns).forEach((column) => {
            fieldIdNameMap[`${table.tableName}_${column.columnId}`] = column.name
        })
...
const getConnectionSql = (item, index, key, fieldIdNameMap) => {
    let sql = `${item.tableName}.${fieldIdNameMap[`${item.tableName}_${item.fieldId}`]} ${getConnectionExpression(item)}`
    if (index > 0) {
        sql = `  ${item.type} ${sql}`
    } else {
        sql = `${key} ${sql}`
...
const generateTableSql = (table, fieldIdNameMap) => {
    ...
    fieldList.forEach((field, index) => {
        if (field.fieldId) {
            let name = `${field.tableName}.${fieldIdNameMap[`${field.tableName}_${field.fieldId}`]}`
            // 去重
            if (field.distinct) {
                name = `DISTINCT ${name}`
            }
    ...
    table.forEach((table, index) => {
        if (!isEmpty(table.tableName) && !isEmpty(fieldList[0]?.fieldId)) {
            if (index <= 0) {
                ...
                sql += `\r\n${table.joinType} ${table.tableName}`
                table.on.forEach(({ left, right, type }, index) => {
                    if (
                        ...
                        let connectSql = `${left.tableName}.${fieldIdNameMap[`${left.tableName}_${left.fieldId}`]} = ${right.tableName}.${fieldIdNameMap[`${right.tableName}_${right.fieldId}`]}`
                        if (index > 0) {
                            connectSql = `\r\n    ${type} ${connectSql}`
                        } else {
                            connectSql = `\r\n  ON ${connectSql}`
        ...
        groupBy
            ...
            .forEach((field, index) => {
                if (!isEmpty(field.tableName) && !isEmpty(field.fieldId)) {
                    ...
                    sql += ` ${field.tableName}.${fieldIdNameMap[`${field.tableName}_${field.fieldId}`]}`
                }
            })
        groupBy
            ?.having
...
const generateOrderBySql = (orderBy, fieldIdNameMap) => {
    ...
    orderBy.forEach((item, index) => {
        if (!isEmpty(item.tableName) && !isEmpty(item.fieldId)) {
            ...
            sql += ` ${item.tableName}.${fieldIdNameMap[`${item.tableName}_${item.fieldId}`]} ${item.type}`
        }
    })
    return sql
}
