<template>
  <div class="table">
    <bk-table
      ...
      <bk-table-column
        v-for="col in field.choice"
        :label="col.name"
        :key="col.key + col.name + col.display"
        :render-header="(h, data) => renderTableHeader(h, data,col)"
      >
    ...
    tableData() {
      ...
      if (this.needPagination) {
        return this.val.slice(
          (this.pagingConfig.current - 1) * this.pagingConfig.limit,
