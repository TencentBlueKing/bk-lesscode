...
export function evalFun (formulaStr) {
    ...
    if (!isSafety) {
        ...
        try {
            ...
            const err = { msg: window.i18n.t('请设置正确的计算公式') }
            throw err
        }
    } else {
        const err = { msg: window.i18n.t('无法确定表达式的安全性') }
        throw err
    }
    return res
}
