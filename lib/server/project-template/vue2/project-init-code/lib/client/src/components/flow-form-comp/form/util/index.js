...
export function computDateDiff(computConfigInfo) {
  const { startDate } = computConfigInfo.dateTime;
  const { endDate } = computConfigInfo.dateTime;
  ...
  if (startDateValue && endDateValue && (dayjs(endDateValue).diff(dayjs(startDateValue)) > 0)) {
    ...
    value = `${days}天`;
    const { accuracyResult } = computConfigInfo.dateTime;
...
function setDateAccuracy(startDateValue, endDateValue, computConfigInfo) {
  ...
  if (checkAccuracy(startDateValue, endDateValue) && computConfigInfo.dateTime.accuracyResult !== 'day') {
    const { defaultTime } = computConfigInfo.dateTime;
    if (defaultTime) {
...
export function evalFun(formulaStr) {
  ...
  let res = '';
  const isSafety = [...formulaStr].some(char => !safetyChars.includes(char));
  if (!isSafety) {
    // 表示不存在危险字符
    try {
...
export const computeFormulas = {
  ...
  median(fields) {
    const leng = fields.length;
    let res = 0;
    const fieldsNumber = fields.map(item => Number(item.value));
    fieldsNumber.sort((a, b) => a - b);
    if (leng % 2 === 0) {
      const index = leng / 2;
  ...
  maxVlaue(fields) {
    const fieldsNumber = fields.map(item => item.value);
