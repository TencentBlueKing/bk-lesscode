...
export const getNodeComponentConfig = (node) => {
    ...
    if (pluginName === 'pause_node') {
        ...
        const apiURL = `${process.env.BK_API_URL_TMPL.replace('{api_name}', apiName)}/${process.env.BKPAAS_ENVIRONMENT}/dataManage`
        componentConfig.data = {
                ...
                value: [
                    {
                        ...
                        value: JSON.stringify({
                            app_code: token.bk_app_code,
                            app_secret: token.bk_app_secret
                        })
                    }
                ]
            },
            bk_http_request_body: { hook: false, need_render: true, value: JSON.stringify({ ...node.config, creator: '${_system.operator}' }) },
