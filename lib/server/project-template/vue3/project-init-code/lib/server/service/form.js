...
// 通过条件过滤数据
export const filterTableDataWithConditions = async (conditions, tableName, group, fields) => {
...
export const grantApiGWPermissionForApps = async () => {
    ...
    try {
        ...
        // 应用为自己授权，apigateway自己调用自己也需要授权
        await grantPermissions(
            apiName,
            {
                target_app_code: token.bk_app_code,
                grant_dimension: 'api'
            },
            token,
            'prod'
        )
        console.log('为应用自己授权完成')
