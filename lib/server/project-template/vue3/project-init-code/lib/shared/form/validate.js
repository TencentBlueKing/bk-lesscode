  ...
  switch (regexKey) {
    case 'AFTER_DATE': // 系统日期之后
      return dayjs(value).format('YYYY-MM-DD')
        .valueOf() > dayjs().format('YYYY-MM-DD')
        .valueOf();
    case 'BEFORE_DATE':
      return dayjs(value).format('YYYY-MM-DD')
        .valueOf() < dayjs().format('YYYY-MM-DD')
    ...
    case 'AFTER_TIME': // 系统时间之后
      return dayjs(value).format('YYYY-MM-DD HH:mm:ss')
        .valueOf() > dayjs().format('YYYY-MM-DD HH:mm:ss')
        .valueOf();
    case 'BEFORE_TIME':
      return dayjs(value).format('YYYY-MM-DD HH:mm:ss')
        .valueOf() < dayjs().format('YYYY-MM-DD HH:mm:ss')
...
function setSerialFielVlaue(serialNumber, serialRules, fieldData, data) {
  fieldData.value = serialRules.map((item) => {
    if (item.type === 'formField') {
      const formField = data.find(formValue => formValue.key === item.configValue) || {};
      if (formField.choice?.length) {
        const values = formField.value.split(',');
        return formField.choice.filter(choiceItem => values.includes(choiceItem.key)).map(choiceItem => choiceItem.name)
          .join(',');
      }
