...
export default {
  ...
  methods: {
    ...
    async exportData(btn) {
      try {
        ...
        const { list } = res.data;
        const header = this.tableFields.map(field => field.name || '');
        const body = [];
        list.forEach((item) => {
          const row = [];
  ...
  render() {
    ...
    const renderButtons = () => this.buttons.slice(0, 4).map(btn => h(
    ...
    const renderDropDown = () => h(
      ...
      [
        ...
        h(
          ...
          self.buttons.slice(4).map(btn => h(
            'li',
            {
              class: 'button-item',
              onClick: () => {
