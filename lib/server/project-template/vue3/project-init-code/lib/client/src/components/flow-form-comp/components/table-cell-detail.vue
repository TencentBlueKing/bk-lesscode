<template>
  <bk-sideslider
    ...
    <template #content>
      <div class="custom-detail-form" style="min-height: 400px;" v-bkloading="{ isLoading: loading }">
          ...
          <bk-form-item v-for="field in fields" :label="`${field.name}:`" :key="field.key">
            ...
            <span v-else-if="field.type === 'IMAGE'">
              <image-file
                ...
                v-if="value[field.key] && value[field.key].length !== 0"
              ></image-file>
              <span v-else>--</span>
            </span>
            <span v-else-if="field.type === 'TABLE'">
              <bk-table :data="value[field.key]" v-if="value[field.key] && value[field.key].length !== 0">
                <bk-table-column v-for="col in field.choice" :label="col.name" :key="col.key">
...
export default {
  ...
  methods: {
    ...
    async setApiData(field) {
      try {
        const { id, api_info: apiInfo, api_instance_id, kv_relation } = field;
        const params = {
          id,
          api_instance_id,
          kv_relation,
          api_info: {
            api_instance_info: apiInfo,
            remote_api_info: apiInfo.remote_api_info,
          },
        };
