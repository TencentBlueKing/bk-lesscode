...
export default {
  ...
  methods: {
    ...
    getValAssociatedFields(key) {
      return this.fields.filter((field) => {
        if (field.meta.default_val_config) {
          const { type, rules } = field.meta.default_val_config;
          return rules.some(group => group.relations.find((item) => {
            if (type === 'currentTable') {
    ...
    getRateSatisfyIntervalRule(group, relation) {
      ...
      const isContain = group.intervals.some(item => (item.min <= relationValue) && (relationValue < item.max));
      return isContain;
    },
    // 获取关联他表字段的筛选数据
