 ...
 * 提供代码格式化或者代码检查服务
 */
import eslintConfig from '../../system-conf/eslint-config'
import {
    ESLint
} from 'eslint'
import {
...
export const checkEslint = async (code, globals = {}) => {
    ...
    if (errorArr.length) {
        ...
        const data = results.reduce((acc, cur) => {
            acc.push(...cur.messages)
            return acc
        }, [])
        throw new global.BusinessError(
            `<pre style="margin:0">${global.i18n.t('ESLint检查不通过，可点击')} <i class="bk-drag-icon bk-drag-fix"></i> ${global.i18n.t('进行自动修复')}：\n${message}</pre>`,
            499,
            200,
            (new Error()).stack,
            data
...
export const fixEslint = async (code, globals = {}) => {
    ...
    if (errorArr.length) {
        const message = escapeHtmlStringList(errorArr.map(x => x.message)).join('')
        throw new global.BusinessError(`<pre style="margin:0">${global.i18n.t('自动修复ESLint失败，请手动修复下面的问题后重试')}：\n${message}</pre>`, 499)
    }
