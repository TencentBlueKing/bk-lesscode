...
const endWithComment = (code = '') => {
    const singleLineComment = /\/\/[^\r\n]*$/
    const multiLineComment = /\/\*[\s\S]*?\*\/$/
    return singleLineComment.test(code.trim()) || multiLineComment.test(code.trim())
}
...
export const code = (bkToken, prompt, promptSuffix, lang) => {
    ...
    if (isOnlyComment(prompt)) {
        scene = 'annotation2code'
    }

    if (!promptSuffix.trim() && endWithComment(prompt) && !isOnlyComment(prompt)) {
