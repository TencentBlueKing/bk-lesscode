...
const operateLogConf = require('../../system-conf/operate-log')
...
class OperationLogger {
    ...
    async log (data = {}, error = {}) {
        ...
        const operateDesc = data.operateStatus ? `${operateCodeText}${global.i18n.t('成功')}` : `${this.getErrorMessage(error)}`
        const logData = {
            projectId,
            operateObj,
            operateCode,
    ...
    getLogConf (request) {
        ...
        if (this.options.apiKey) {
            return locations[this.options.apiKey]
        }
