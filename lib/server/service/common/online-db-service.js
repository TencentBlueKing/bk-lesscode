...
import { escapeTableName } from '../../../shared/util'
/**
 * 操作线上环境数据库服务
 */
export default class OnlineService {
    ...
    async describeTables (tableNames, database) {
        ...
        const escapedTableNames = tableNames.map(escapeTableName)
        const columnSql = `
            SELECT * FROM INFORMATION_SCHEMA.TABLES t
            LEFT JOIN INFORMATION_SCHEMA.COLUMNS c
            ON t.TABLE_NAME = c.TABLE_NAME
            WHERE t.TABLE_NAME IN (${placeholders}) AND t.TABLE_SCHEMA = ? AND c.TABLE_SCHEMA = ?
            ORDER BY t.CREATE_TIME DESC, c.ORDINAL_POSITION ASC;
        `
        const indexSql = `
            SELECT * FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_NAME IN (${placeholders});
        `
        const columnsList = await this.dbEngine.execSql(columnSql, [escapedTableNames, database, database])
        const indexList = await this.dbEngine.execSql(indexSql, [escapedTableNames])
    ...
    async getTableData (tableName, page, pageSize, filterKey, filterValue, sortKey, sortValue) {
        const where = filterKey && filterValue ? 'WHERE ?? LIKE ?' : ''
        const finalSortKey = sortKey || 'id'
        const finalSortValue = (sortValue || 'DESC').toUpperCase()
        const orderBy = `ORDER BY ?? ${finalSortValue}`

        if (!['ASC', 'DESC'].includes(finalSortValue)) {
            throw new Error('Invalid sort direction')
        }

        const sql = `
            SELECT SQL_CALC_FOUND_ROWS * FROM \`${escapeTableName(tableName)}\` ${where} ${orderBy} LIMIT ?, ?;
            SELECT FOUND_ROWS();
        `
        const params = [
            // 第一条 SQL 的参数
            [
                ...(filterKey && filterValue ? [
                    filterKey, // WHERE ?? 的参数
                    `%${filterValue}%` // LIKE ? 的参数
                ] : []),
                finalSortKey, // ORDER BY ?? 的参数
                (page - 1) * pageSize, // LIMIT 的第一个参数
                +pageSize // LIMIT 的第二个参数
            ],
            // 第二条 SQL 不需要参数
            []
        ]
        const [list, foundRows = []] = await this.dbEngine.execMultSql(sql, params)
    ...
    async getTableAllData (tableName, filterKey, filterValue, sortKey, sortValue) {
        ...
        const sql = `
            SELECT SQL_CALC_FOUND_ROWS * FROM \`${escapeTableName(tableName)}\` ${where} ${orderBy};
            SELECT FOUND_ROWS();
        `
