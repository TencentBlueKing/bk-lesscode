...
export function getDataService (name = 'default', customEntityMap) {
    ...
    return {
        ...
        async update (tableFileName, data) {
            ...
            try {
                const { affected = 0 } = await repository.update({ id: data.id }, transformData(editData, repository.metadata.columns))
                return affected
            } catch (error) {
                throw new Error(error.sqlMessage || error)
            }
        ...
        async bulkUpdate (tableFileName, dataList) {
            ...
            try {
                ...
                const updateResult = await Promise.all(transformList.map((data) => repository.update({ id: data.id }, data)))
...
export const TABLE_FILE_NAME = {
    ...
    PROJECT_BKFLOW_SPACE: 'project-bkflow-space',
    ROLE_PERM: 'role-perm',
    ROLE: 'role',
    ROUTE: 'route',
    TOKEN: 'token',
