...
export function getDataService (name = 'default', customEntityMap) {
    ...
    return {
        ...
        async delete (tableFileName, id) {
            if ([undefined, null, ''].includes(id)) throw new Error(global.i18n.t('删除 {{n}} 表数据的时候，需要传入 id 参数', { n: tableFileName }))
            const repository = getRepositoryByName(tableFileName)
            try {
        ...
        async bulkDelete (tableFileName, ids) {
            if ([undefined, null].includes(ids)) throw new Error(global.i18n.t('批量删除 {{n}} 表数据的时候，需要传入 ids 参数', { n: tableFileName }))
            const repository = getRepositoryByName(tableFileName)
            try {
