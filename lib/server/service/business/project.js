...
export const getExportProjectData = async (projectId, versionId = null) => {
    ...
    const [
        ...
        // LCDataService.leftJoinQuery(
        //     TABLE_FILE_NAME.LAYOUT_INST,
        //     [{ tableName: 'layout', on: 'layout.id = layout_inst.routeId' }],
        //     { express: 'layout_inst.projectId = :projectId AND layout_inst.versionId = :versionId', data: { projectId, versionId: null } }
        // ),
        LCDataService.get({
    ...
    if (projectPage.length) {
        ...
        // pageComp
        const { list: pageCompData } = await LCDataService.get({
            tableFileName: TABLE_FILE_NAME.PAGE_COMP,
            query: {
                pageId: pageIds
            }
        })
        pageComp = pageCompData || []
    }
...
export const importProject = async (projectData, userProjectRoleData, importData, bkTicket) => {
    ...
    try {
        ...
        return Promise.reject(new Error('导入应用失败'))
    }
}
