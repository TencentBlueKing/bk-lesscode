...
export const queryApiDoc = async (appName, bkToken) => {
    const apiHost = httpConf.apiGateWayUrlTmpl?.replace('{api_name}', 'bkaidev')
    const stageName = apiHost + httpConf.stageName === 'prod' ? '/prod' : '/stage'
    const url = `${apiHost}${stageName}/worker_service/preview/${appName}/api_doc/schema/`
