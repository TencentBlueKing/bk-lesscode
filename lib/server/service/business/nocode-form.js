...
import { whereVersion } from '../../model/common'
...
export const createNocodeForm = async (formData = {}) => {
    ...
    content.map(item => {
        if (keyWords.includes(item.key)) {
            throw new Error(`${item.key}为数据表的内置字段，唯一标识不能命名为${item.key}`)
        }
    })
    let resData = {}
    ...
    await LCDataService.transaction(async (transactionalEntityManager) => {
        ...
        if (hasTable && hasTable.id) {
            ...
            dbRes = await createNCTable({ tableName, projectId: formData.projectId, columns: dbJson })
        }
...
export const getFormList = async (projectId, versionId) => {
    return getRepository(Form).createQueryBuilder('form')
        .leftJoinAndSelect(DataTable, 'dataTable', 'form.dataSourceId = dataTable.id')
        .select(['form.*', 'dataTable.tableName as tableName'])
        .where('form.projectId = :projectId', { projectId })
        .andWhere(whereVersion(versionId, 'form'))
        .andWhere('form.deleteFlag = 0')
        .getRawMany()
}

// 获取项目下对应版本的form列表
const getProjectFormList = async (projectId, versionId) => getRepository(Form).find({
...
export const versionTask = async (ctx, next) => {
    ...
    const formList = await getProjectFormList(projectId, versionId)
