...
export const createNocodeForm = async (formData = {}) => {
    ...
    await LCDataService.transaction(async (transactionalEntityManager) => {
        ...
        if (dbRes && dbRes.id) {
            Object.assign(formItem, { content: JSON.stringify(formItem.content), tableName, versionId: formData.versionId || null })
            // 新增form表记录
...
export const updateNocodeForm = async (formData = {}) => {
    ...
    if (table?.id) {
        ...
        throw new Error(`找不到tableName为${tableName}的数据表`)
    }
}
