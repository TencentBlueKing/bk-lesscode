        ...
        request.post({ url, formData }, function (error, response, body) {
            if (!error && response.statusCode === 200) {
                ...
                if (response?.statusCode === 403) {
                    reject(new Error(global.i18n.t('您没有执行该操作的权限，请联系应用【code为{{0}}】开发负责人{{1}}在蓝鲸开发者中心添加开发者权限', [appInfo.appCode, appInfo.createUser])))
                } else {
    ...
    return new Promise((resolve, reject) => {
        request.post({ url, json: true, body: data }, async function (error, response, body) {
            if (typeof body === 'object' && body.module) {
                ...
                if (response?.statusCode === 403) {
                    reject(new Error(global.i18n.t('您没有执行该操作的权限，请联系应用【code为{{0}}】开发负责人{{1}}在蓝鲸开发者中心添加开发者权限', [appData.appCode, appData.createUser])))
                } else {
