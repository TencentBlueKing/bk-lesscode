...
export const deploy = async (releaseForm, appInfo, bkTicket) => {
    ...
    if (!['prod', 'stag'].includes(releaseForm.env)) {
        return false
    }
    
    if (!CODE_REGEX.test(appInfo.appCode) || !CODE_REGEX.test(appInfo.moduleCode)) {
        return false
    }
...
export const createModule = async (appData, bkTicket) => {
    if (!CODE_REGEX.test(appData.appCode)) return false
    if (!v3Config.APP_ID || !v3Config.APP_SECRET) return false
