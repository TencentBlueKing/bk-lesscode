...
export const getReleaseInfo = async (projectId, appCode, moduleCode, env, bkTicket, bindInfo) => {
    const url = `${v3Config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/envs/${env}/released_state/?bk_app_code=${v3Config.APP_ID}&bk_app_secret=${encodeURI(v3Config.APP_SECRET)}&bk_token=${bkTicket}`
...
export const getModuleInfo = async (appCode, moduleCode, bkTicket) => {
    const url = `${v3Config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/?bk_app_code=${v3Config.APP_ID}&bk_app_secret=${encodeURI(v3Config.APP_SECRET)}&bk_token=${bkTicket}`
    return new Promise((resolve, reject) => {
        request.get(url, async function (error, response, body) {
            try {
                let res = body
...
export const createV3App = async (appData, bkTicket) => {
    ...
    const data = {
        region: 'default', // 内部版为 ieod，私有化版本为 default
    ...
    return new Promise((resolve, reject) => {
        request.post({ url, json: true, body: data }, async function (error, response, body) {
            ...
            if (typeof body === 'object' && body.application) {
                ...
                reject(new Error(body.detail || error || '同步创建蓝鲸开发者应用失败'))
            }
        })
    })
}
