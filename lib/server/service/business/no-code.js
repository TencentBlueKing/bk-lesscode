...
export const createNCTable = async (dataTable) => {
    ...
    dataTable.columns.forEach((column) => {
        ...
        if (NocodeBaseColumn.find(baseColumn => baseColumn.name === column.name)) {
...
export const updateNCTable = async (dataTable) => {
    return LCDataService.transaction(async (transactionalEntityManager) => {
        ...
        dataTable.columns.forEach((column) => {
            ...
            if (NocodeBaseColumn.find(baseColumn => baseColumn.name === column.name && baseColumn.columnId !== column.columnId)) {
                throw new Error(`字段名【${column.name}】为系统内置字段，请修改后再试`)
            }
        })
        // 获取原始 table
        ...
        console.log(updateTable.columns, '正在更新')
        ...
        dataTable.columns.forEach((column) => {
            ...
            console.log(getColumnString(column), hasSameColumn, '正在对比', column.name)
            if (!hasSameColumn) {
                updateTable.columns.push(column)
            }
        })
        console.log(updateTable.columns, '比对完成')
        // 构造数据入库 sql
        const originTable = {
            ...originData,
            columns: JSON.parse(originData.columns)
