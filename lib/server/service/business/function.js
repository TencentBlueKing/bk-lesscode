        ...
        const checkParam = (param) => {
            if (param?.required) {
                if (param.valueType === API_PARAM_VALUE_TYPES.VARIABLE && isEmpty(param.code)) {
                    throw new global.BusinessError(`函数参数【${param.name}】必填，但是没有选择变量，请修改后再试`, 400, 400)
                }
                if (
                    ...
                    throw new global.BusinessError(`函数参数【${param.name}】必填，但是没有填值，请修改后再试`, 400, 400)
                }
            }
        }
            ...
            const handleParamVariable = (param) => {
                if (API_PARAM_VALUE_TYPES.VARIABLE === param?.valueType) {
                    if (param.code) {
                        ...
                        throw new global.BusinessError(`函数参数【${param.name}】类型是变量，但是未选择变量，请修改后再试`)
                    }
                }
            }
