...
import { getPreviewDbConfig } from '../service/preview-db-service'
...
export const createNCTable = async (dataTable) => {
    ...
    dataTable.columns.forEach((column) => {
        if (!column.name) {
            throw new Error('字段名为空，请修改后再试')
        }
        if (BASE_COLUMNS.find(baseColumn => baseColumn.name === column.name)) {
            throw new Error(`字段名【${column.name}】为系统内置字段，请修改后再试`)
        }
    })
    // 入库
...
export const updateNCTable = async (dataTable) => {
    return LCDataService.transaction(async (transactionalEntityManager) => {
        ...
        dataTable.columns.forEach((column) => {
            ...
            if (BASE_COLUMNS.find(baseColumn => baseColumn.name === column.name && baseColumn.columnId !== column.columnId)) {
                throw new Error(`字段名【${column.name}】为系统内置字段，请修改后再试`)
            }
        })
        // 获取原始 table
...
export const getFlowList = async ({ page, pageSize, query, projectId, versionId }) => {
    const [{ list, count }, pageList] = await Promise.all([
        ...
        PageModel.getProjectPages(projectId, versionId)
    ])
    const pageIdName = {}
    pageList.forEach(page => {
        pageIdName[page.id] = page.pageName
    })
    const finalList = list.map(item => {
        ...
        if (item.managePageIds) {
            ...
            pageIds.forEach(id => {
                pageIdName[id] && (managePageNames += pageIdName[id] + ',')
            })
            managePageNames && managePageNames.trimEnd(',')
        }
        return {
            ...item,
            pageName: pageIdName[item.pageId] || '--',
