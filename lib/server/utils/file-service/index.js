import { sanitizeUploadPath } from '../../../shared/security/file-protection'
...
const checkBkRepoSetting = (throwErr = true) => {
    if (!config.BKREPO_USERNAME || !config.BKREPO_PROJECT || !config.BKREPO_ENDPOINT_URL) {
        if (throwErr) {
            throw new Error(global.i18n.t('bkRepo: 请先搭建bkRepo服务并确认配置信息已设置正确'))
        } else {
            return false
        }
    } else {
...
const downloadFile = (targetFile, dest) => {
    ...
    // 安全过滤 targetFile 路径
    targetFile = sanitizeUploadPath(targetFile)
    if (!targetFile) {
        return Promise.reject(new Error(global.i18n.t('文件路径无效')))
    }
    ...
    return new Promise((resolve, reject) => {
        request.get(options, function (err, res, body) {
            if (err || res.statusCode !== 200) {
                ...
                reject(new Error(`${global.i18n.t('文件下载失败')}: ${err || res.statusCode}`))
            } else {
                if (dest) {
...
const getSignUrl = async (fullPathSet = [], expireSeconds = 0) => {
    ...
    return new Promise((resolve, reject) => {
        request.post({ url, body: data, json: true, headers }, function (err, res, body) {
            if (err || res.statusCode !== 200) {
                reject(new Error(`${global.i18n.t('获取下载链接失败')}: ${err || res.statusCode}`))
            } else if (typeof body === 'object' && body.code !== 0) {
                reject(new Error(`${global.i18n.t('获取下载链接失败')}:${body.message}`))
            } else {
                if (body.data[0]?.url) {
...
const uploadImage = async (filePath, uploadKey, settings = {}) => {
    ...
    if (settings.fileStream) {
        fileStream = settings.fileStream
    } else {
...
const uploadRepoFile = async (uploadPath, fileStream, settings = {}) => {
    ...
    const options = {
        url,
        method: 'PUT',
        headers: Object.assign({}, headers, { 'X-BKREPO-OVERWRITE': false }),
        body: fileStream
    }
    return new Promise((resolve, reject) => {
        request(options, function (err, res, body) {
            ...
            if (err || res.statusCode !== 200) {
                reject(new Error(`${global.i18n.t('上传至repo失败')}: ${err || res.statusCode}`))
            } else if (typeof body === 'object' && body.code > 0) {
                reject(new Error(`${global.i18n.t('上传至repo失败')}： ${body.message}`))
            } else if (body.code === 0) {
                resolve(settings.fullUrl ? url : uploadPath)
            } else {
                reject(new Error(global.i18n.t('上传至repo失败')))
            }
        })
    })
}
