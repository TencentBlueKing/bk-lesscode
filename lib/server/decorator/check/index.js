...
import { LCDataService } from '../../service/common/data-service'
...
export const DoubleCheck = ({
    ...
    checkList.forEach(({ key, getValue }) => {
        if (typeof key !== 'string' || typeof getValue !== 'function') {
            throw Error(global.i18n.t('使用了 DoubleCheck 装饰器，但是参数类型不正确，请修改后再试'))
        }
    })
        ...
        descriptor.value = async (ctx) => {
            try {
                ...
                checkList.forEach(({ key, getValue }) => {
                    ...
                    if (doubleData) {
                        throw new global.BusinessError(global.i18n.t('表【{{0}}】的【{{1}}】字段已存在值为【{{2}}】的数据，请修改后再试', [tableFileName, key, doubleData[key]]), 400, 400)
                    }
                })
