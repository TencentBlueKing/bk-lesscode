 ...
 * 输出 json
 */
export const OutputJson = (standardize = true) => {
    return (target, propertyKey, descriptor) => {
        ...
        descriptor.value = async (ctx) => {
            try {
                ...
                ctx.body = standardize ? JSON.stringify({ code: 0, message: 'success', data }) : JSON.stringify(data)
            } catch (error) {
...
export const OutputStream = () => {
    return (target, propertyKey, descriptor) => {
        ...
        descriptor.value = async (ctx) => {
            try {
                const a = await originValue.apply(this, [ctx])
                ctx.body = a
            } catch (error) {
                outputError(error, ctx)
            }
    ...
    return (target, propertyKey, descriptor) => {
        ...
        descriptor.value = async (ctx) => {
            try {
                ...
                // controller 需要返回stream
                ctx.body = await originValue.apply(this, [ctx])
                ctx.status = 200
            } catch (error) {
                outputError(error, ctx)
            }
