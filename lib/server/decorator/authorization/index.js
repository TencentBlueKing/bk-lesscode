 ...
 */
export const ProjectAuthorization = ({ getId = ctx => ctx.request.query.projectId, action = IAM_ACTION.develop_app[0] }) => {
    return (target, propertyKey, descriptor) => {
        // const originValue = descriptor.value
        // descriptor.value = async (ctx) => {
        //     try {
        ...
        descriptor.value = async (ctx) => {
            try {
                ...
                if (!project) {
                    throw new global.BusinessError(`应用[ID:${projectId}]不存在`, 404)
                }

                // 不是项目的创建者，那么需要去权限中心查询是否具有 develop_app 权限
                if (!project.isCreator) {
                    ctx.request.body.action = action
                    ctx.request.body.resourceId = String(projectId)
                    const iamRes = await check(ctx)
                    // 权限中心操作权限未申请或未审批通过
                    if (!iamRes.pass) {
                        throw new global.BusinessError(`您没有应用[ID:${projectId}]的权限，请去权限中心申请权限`, 403, 200, (new Error()).stack, iamRes)
                    }
                }
