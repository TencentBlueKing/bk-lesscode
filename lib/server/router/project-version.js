...
router.use(['/create', '/update', '/list'], async (ctx, next) => {
    ...
    // 不是项目的创建者，那么需要去权限中心查询是否具有 deploy_app 权限
    if (!project.isCreator) {
        ctx.request.body.action = IAM_ACTION.deploy_app[0]
        ctx.request.body.resourceId = String(id)
        const iamRes = await check(ctx)
        // 权限中心 deploy_app 操作权限未申请或未审批通过
        if (!iamRes.pass) {
            ctx.send({
                code: 403,
