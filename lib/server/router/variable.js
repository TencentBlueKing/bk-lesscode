...
// 只校验projectId
router.use(['/getAllVariable', '/addVariable', '/uploadImage', '/updatePageBuildInVariable',  '/getFunctionVariable'], async (ctx, next) => {
    await handleProjectPerm(ctx, next)
})

// 校验projectId跟资源id
router.use(['/editVariable', '/deleteVariable'], async (ctx, next) => {
