...
// 校验projectId跟资源id
router.use(['/update', '/off', '/online', '/scope', '/delete', '/page-useing-version'], async (ctx, next) => {
...
// 需要同时校验组件id跟categoryId
router.use(['/updateData'], async (ctx, next) => {
    const { id, categoryId } = ctx.request.body
    
    let tableName = 'COMP'
    let resourceId = id
    let projectId = await findRealProjectId(ctx, { tableName, resourceId })
    if (!projectId) {
        ctx.send({
            code: 500,
            message: global.i18n.t('projectId非法'),
        ...
        await handleProjectPerm(ctx, () => {}, projectId)
    }

    tableName = 'COMP_CATEGORY'
    resourceId = categoryId
    projectId = await findRealProjectId(ctx, { tableName, resourceId })
    if (!projectId) {
        ctx.send({
            code: 500,
            message: global.i18n.t('projectId非法'),
