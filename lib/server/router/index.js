...
const renderParams = { STATIC_URL: '', BK_API_URL_TMPL: httpConf.apiGateWayUrlTmpl }

// 需要权限中心鉴权的 path 前缀
const needIamAuthPrefix = [
    '/api/project',
    '/api/page',
    '/api/projectVersion'
]
...
pageRouter.get('*', async (ctx, next) => {
    ...
    let isNeedAuthPath = false
    for (let i = 0; i < needIamAuthPrefix.length; i++) {
        if (path.startsWith(needIamAuthPrefix[i])) {
            isNeedAuthPath = true
            console.error(ctx.path)
            console.error(ctx.url)
            console.error(ctx.originalUrl)
            console.error()
            break
        }
    }
    console.warn('isNeedAuthPathisNeedAuthPathisNeedAuthPath', isNeedAuthPath)
