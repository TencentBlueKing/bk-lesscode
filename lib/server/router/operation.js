 ...
 */
import { checkNoResourcePerm } from '../service/common/project-auth'
...
// 校验是否有view_operation_data权限
router.use('*', async (ctx, next) => {
    const res = await checkNoResourcePerm(ctx, IAM_ACTION.view_operation_data[0])
    if (res.code > 0) {
        ctx.send(res)
        return
    }
