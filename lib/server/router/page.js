...
router.use(['/getList'], async (ctx, next) => {
    ...
    console.error('projectproject', project)
    if (!project) {
        ctx.send({
            code: 404,
            message: '项目不存在',
    ...
    if (!project.isCreator) {
        ...
        console.error(iamRes)
        // 已在权限中心申请了 develop_app 操作权限
        if (!iamRes.pass) {
            // ctx.throw(403, '项目不存在或者没有权限', { user: { name: 'foo' } })
            ctx.send({
                code: 403,
            ...
            return
            // throw new global.BusinessError('项目不存在或者没有权限', 400003, 403, (new Error()).stack, iamRes)
        }
        await next()
    }
...
router.use(['/update', '/copy', '/delete', '/detail', 'getDeletePageCodes'], async (ctx, next) => {
    ...
    console.error('ssssssssssssssssssssssssssssssssssssssssssssssssssssss')
