...
router.use(['/releaseProject', '/offlineProject', '/updateReleaseVersion', '/getProjectVersionOptionList', '/getSucVersionList', '/detailInfo', '/detailFromV3', '/getList'], async (ctx, next) => {
    let id = ['POST', 'PUT', 'DELETE'].includes(ctx.request.method)
        ? (ctx.request.body.id || ctx.request.body.projectId)
        : (ctx.request.query.id || ctx.request.query.projectId)
    if (ctx.request.url === '/api/release/releaseProject') {
        id = ctx.request.body?.releaseForm?.projectId
    }
