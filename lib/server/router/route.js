...
// 只校验projectId
const checkList = ['/query/:projectId', '/project/:projectId/page', '/project/:id/tree']
router.use(checkList, async (ctx, next) => {
    await handleProjectPerm(ctx, next)
})

router.use(['/project/:id'], async (ctx, next) => {
    if (ctx.request.method === 'POST') {
        const tableName = 'LAYOUT_INST'
        const resourceKey = 'id'
        const resourceId = ctx.request.body.pageRoute?.layoutId
...
router.use(['/bind-page-route'], async (ctx, next) => {
    const { pageId, routeId } = ctx.request.body
    if (!pageId && !routeId) {
        await handleProjectPerm(ctx, next)
    }
    ...
    if (routeId) {
        const tableName = 'PAGE_ROUTE'
        const resourceKey = 'routeId'
        const resourceId = routeId
        const projectId = await findRealProjectId(ctx, { tableName, resourceId, resourceKey })
        if (!projectId) {
            ctx.send({
                code: 500,
