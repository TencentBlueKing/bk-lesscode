...
const openApiList = {
    decodeUserFromJWT: ['/api/data/getApiData'],
    getUserFromHeaders: ['/api/data-source/user/tableName']
}
    ...
    const authMiddleware = async function (ctx, next) {
        try {
            ...
            if (authWhiteList.some(authWhitePath => ctx.originalUrl.includes(authWhitePath))) {
                ...
                let username
                if (openApiList.decodeUserFromJWT.some(openApiPath => ctx.originalUrl.includes(openApiPath))) {
                    // apigateway 使用jwt鉴权
                ...
                } else if (openApiList.getUserFromHeaders.some(openApiPath => ctx.originalUrl.includes(openApiPath))) {
                    console.log('-------------->ctx.header', ctx.header)
                    username = ctx.header['X-CREATOR']
                }
                if (username) {
