...
export default class DataSourceController {
    ...
    @OutputJson()
    @ProjectAuthorization({ getId: ctx => ctx.request.headers['x-project-id'] })
    @Post('/user/queryByJson')
    async queryByJson (
        @HeaderParams({ name: 'x-project-id', require: true }) projectId,
        @BodyParams() condition
    ) {
        const sql = await getSqlByCondition(projectId, condition)
        // 只能是查询语句
        onlySelectSqlCheck(sql)
    ...
    @OutputJson()
    @ProjectAuthorization({ getId: ctx => ctx.request.headers['x-project-id'] })
    @Post('/user/queryByJson/time')
    async queryByJsonWithTime (
        @HeaderParams({ name: 'x-project-id', require: true }) projectId,
        @BodyParams() condition
    ) {
        const sql = await getSqlByCondition(projectId, condition)
        ...
        const data = await dbEngine.execMultSql(sql)
