            ...
            await getConnection().transaction(async transactionalEntityManager => {
                ...
                if (varIds.length) {
                    await Promise.all(varIds.map(async valId => {
                        const val = !isNew ? await getRepository(Variable).findOne(valId) : result.valList.find(val => val.id === valId)
                ...
                if (funcIds.length) {
                    await Promise.all(funcIds.map(async funcId => {
                        const func = !isNew ? await getRepository(Func).findOne(funcId) : result.funcList.find(func => func.id === funcId)
