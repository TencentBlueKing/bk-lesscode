...
export const detail = async (ctx) => {
    try {
        ...
        if (template.isOffcial === 0) {
            const projectId = template.belongProjectId
            const res = await checkProjectPerm(ctx, projectId)
            if (res.code > 0) {
                ctx.send(res)
                return
            }
...
export const apply = async (ctx) => {
    try {
        const { id, fromProjectId, templateInfo, varList = [], funcList = [], framework } = ctx.request.body
        ...
        await Promise.all(templateInfo.map(async fromTemplate => {
            ...
            let varIds, funcIds, defaultFuncGroupId, result
            const newContent = JSON.parse(content || {})
            const isNew = newContent.vars || newContent.functions
            if (isNew) {
                result = await filterFuncAndVars({ projectId: belongProjectId, versionId, fromProjectId: 0, valList: varList, funcList })
                varIds = result.valList.map(val => val.id)
                funcIds = result.funcList.map(func => func.id)
                defaultFuncGroupId = result.defaultFuncGroupId
            } else {
                result = await getRealVarAndFunc({ projectId: belongProjectId, fromProjectId, versionId, valList: varList, funcList })
                varIds = result.varIds
                funcIds = result.funcIds
                defaultFuncGroupId = result.defaultFuncGroupId
            }
            await getConnection().transaction(async transactionalEntityManager => {
                ...
                if (varIds.length) {
                    await Promise.all(varIds.map(async valId => {
                        const val = !isNew ? await getRepository(Variable).findOne({ where: { id: valId } }) : result.valList.find(val => val.id === valId)
                        ...
                        const newVal = Object.assign(other, {
                            ...
                            pageCode: '',
                            effectiveRange: 0,
                            ...(isNew ? { defaultValue: JSON.stringify(other.defaultValue) } : {})
                        })
                ...
                if (funcIds.length) {
                    await Promise.all(funcIds.map(async funcId => {
                        const func = !isNew ? await getRepository(Func).findOne({ where: { id: funcId } }) : result.funcList.find(func => func.id === funcId)
                        ...
                        const newFunc = Object.assign(other, {
                            ...
                            projectId: belongProjectId,
                            ...(isNew ? { remoteParams: (other.remoteParams || []).join(', '),
