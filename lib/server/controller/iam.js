...
const iamController = {
    ...
    async iamAppPermRegistry (ctx, iamAppPerm) {
        ...
        //     -H 'X-Bkapi-Authorization: {"bk_app_code": "hiei-test", "bk_app_secret": "TMhghDazn9xmCVj4aJJB8ftlzx89W9ZmZ1KD5NG15tU7ZjpJ8K"}' \
        //     'http://bk-iam.apigw.o.oa.com/stage/api/v1/model/systems/hiei-test/configs/system_managers' \
        //     -d '["tianyizhu", "hailinxiao"]'
        ...
        iamAppPermActionList.forEach(action => {
            ...
            if (action.actionId === 'access_page') {
        ...
        try {
            ...
            if (method === 'fetch_instance_info') {
                ...
                if (method === 'list_instance') {
                    ...
                    console.error('pageList', pageList)
                } else if (method === 'search_instance') {
                    // 列表搜索
                    const filter = body.filter || {}
                    const query = filter.keyword || ''
