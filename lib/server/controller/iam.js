...
const iamController = {
    ...
    async myProject (ctx) {
        try {
            ...
            logger.info('2222')
            logger.info(`authedIdList: ${JSON.stringify(authedIdList)}`)

            const where = { id: In(authedIdList) }
            const authedList = await projectModel.findProjects({ where })
            logger.info('33333')
            logger.info(`authedList: ${JSON.stringify(authedList)}`)

            const myCreateAndAuthedAppList = projectList.concat(authedList || [])
            logger.info(`myCreateAndAuthedAppList: ${JSON.stringify(myCreateAndAuthedAppList)}`)
