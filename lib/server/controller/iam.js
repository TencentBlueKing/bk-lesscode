...
const iamController = {
    ...
    async updateIamAppPermAction (ctx) {
        try {
            ...
            const { affected } = await iamAppPermModel.updateIamAppPermAction(projectId, fields)
            ctx.send({
                code: 0,
                message: 'OK',
                data: affected
    ...
    async fetchResourceAppPerm (ctx) {
        ...
        console.log('ctx.request.body', body)
        console.log('pageData', pageData)
        console.error('ctx.request.query', ctx.request.query)
        logger.info(`body: ${JSON.stringify(body)}`)
        try {
            let res = {}
            const method = body.method
            ...
            if (method === 'fetch_instance_info') {
                ...
                if (method === 'list_instance') {
                    ...
                    const buildinAction = iamAppPermActions.find(item => item.actionId === IAM_APP_PERM_BUILDIN_ACTION) || {}
                    const validPageList = pageList.filter(item => buildinAction.actionRelatedResourceId.indexOf(String(item.id)) > -1)
                    res = {
