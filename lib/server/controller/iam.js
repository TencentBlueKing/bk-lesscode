...
const iamController = {
    ...
    async iamAppPermRegistry (ctx, iamAppPerm) {
        ...
        if (systemRes.code !== 0) {
            ctx.throw({
                message: `注册系统失败：${systemRes.code}: ${systemRes.message}`
            })
        }
        ...
        if (systemManagerRes.code !== 0) {
            ctx.throw({
                message: `设置系统管理员失败：${systemManagerRes.code}: ${systemManagerRes.message}`
            })
        }
        ...
        if (resourceTypeRes.code !== 0) {
            ctx.throw({
                message: `注册资源类型失败：${resourceTypeRes.code}: ${resourceTypeRes.message}`
            })
        }
        ...
        if (instanceSelectionRes.code !== 0) {
            ctx.throw({
                message: `注册实例视图失败：${instanceSelectionRes.code}: ${instanceSelectionRes.message}`
            })
        }
        ...
        if (actionRes.code !== 0) {
            ctx.throw({
                message: `注册操作失败：${actionRes.code}: ${actionRes.message}`
            })
        }
    ...
    async iamAppPermUpdate (ctx, iamAppPerm) {
        ...
        if (failedList.length) {
            ctx.throw({
                message: `修改操作失败：: ${JSON.stringify(failedList)}`
            })
        }
    ...
    async myProject (ctx) {
        try {
            ...
            logger.info(`projectIdList: ${JSON.stringify(projectIdList)}`)
            logger.info('111111')
            logger.info(`authedRet: ${JSON.stringify(authedRet)}`)
            ...
            myCreateAndAuthedAppList.forEach(item => {
                ...
                item.canDevelop = (authedRet['develop_app'] === true) || (authedRet['develop_app'].indexOf(String(item.id)) > -1)
                item.canDeploy = (authedRet['deploy_app'] === true) || (authedRet['deploy_app'].indexOf(String(item.id)) > -1)
                item.canManagePerms = (authedRet['manage_perms_in_app'] === true) || (authedRet['manage_perms_in_app'].indexOf(String(item.id)) > -1)
                item.canManage = (authedRet['manage_app'] === true) || (authedRet['manage_app'].indexOf(String(item.id)) > -1)
            })
    ...
    async fetchResourceAppPerm (ctx) {
        ...
        if (projectId === '' || projectId === undefined || projectId === null) {
            ctx.throw('权限中心回调错误: projectId 不存在')
        }
