...
const iamController = {
    ...
    async myProject (ctx) {
        try {
            ...
            const query = {
                condition: [],
                params: {},
                select: ['project.id', 'project.projectCode', 'project.projectName', 'project.isEnableDataSource', 'project.createUser']
            }
            ...
            const authedAppRet = await iamController.fetchAllAppHasActionsPerm(ctx)

            const developAppIdList = authedAppRet[IAM_ACTION.develop_app[0]] || []
            const deployAppIdList = authedAppRet[IAM_ACTION.deploy_app[0]] || []
            const managePermsInAppIdList = authedAppRet[IAM_ACTION.manage_perms_in_app[0]] || []
            const manageAppIdList = authedAppRet[IAM_ACTION.manage_app[0]] || []

            const authedAppIdList = developAppIdList.concat(deployAppIdList).concat(managePermsInAppIdList).concat(manageAppIdList)
    ...
    /**
     * 权限中心鉴权 for server 里调用，仅用于有资源依赖的操作
     */
    async checkInServer (ctx, resourceId, actions = [
        ...
        try {
            ...
            console.error('notPassedActions', notPassedActions)
