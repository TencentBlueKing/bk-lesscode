...
const iamController = {
    ...
    /**
     * 权限中心鉴权，判断当前用户是否具有资源无关操作的权限
     */
    // async checkNoResources (ctx, action, notNeedApplyUrl = false) {
    //     const userInfo = ctx.session.userInfo
    //     try {
    //         const params = {
    //             system: IAM_APP_ID,
    ...
    //         console.error(11, authedRet)
    //         const iamRet = await iamController.checkNoResources(ctx, 'manage_platform', true)
    //         // 平台管理权限不直接引导去申请权限，由平台管理员在用户组主动添加人员名单或用户自己去权限中心自定义申请
    //         // if (iamRet.pass) {
    //         //     ctx.send({ code: 0, message: '', data: iamRet })
    //         // } else {
    //         //     ctx.send({ code: 403, message: '没有权限，请去权限中心申请权限', data: iamRet })
    //         // }
    //         ctx.send({ code: 0, message: '没有权限，请去权限中心申请权限', data: iamRet })
    //     } catch (e) {
    //         ctx.throw(e)
    //     }
    // },
