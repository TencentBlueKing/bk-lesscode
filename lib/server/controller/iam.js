...
const iamController = {
    ...
    async myProject (ctx) {
        try {
            ...
            const authedAppListRes = await iamController.fetchAllAppHasActionPerm(ctx, IAM_ACTION.develop_app[0])
            const authedAppIdList = authedAppListRes.data.value || []
    ...
    async check (ctx) {
        ...
        if (action === '' || action === null || action === undefined) {
            ctx.throw({
                message: '需要 action 参数'
            })
        }
        if (!IAM_ACTION[action]) {
            ctx.throw({
                message: 'action 参数错误，该操作不存在'
            })
        }
        const userInfo = ctx.session.userInfo
        try {
     ...
     */
    async fetchAllAppHasActionPerm (ctx, action) {
        const userInfo = ctx.session.userInfo
        try {
            const data = {
                system: IAM_APP_ID,
            ...
            const res = await http.post(`${IAM_HOST}/api/v1/policy/query`, data, {
                headers: {
                    'X-Bkapi-Authorization': JSON.stringify({
                        bk_app_code: IAM_APP_ID,
                        bk_app_secret: IAM_APP_SECRET
