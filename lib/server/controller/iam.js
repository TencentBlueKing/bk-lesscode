...
import { IAM_ACTION, IAM_RESOURCE_TYPE_ID, IAM_APP_PERM_BUILDIN_ACTION, FORBIDDEN_KEYS } from '../../shared/constant.js'
...
const iamController = {
    ...
    async fetchIamAppPerm (ctx) {
        const proejectId = ctx.request.headers['x-project-id']
        try {
    ...
    async fetchAuthSubject (ctx) {
        ...
        try {
            ...
            ;(res.data.results || []).forEach((item, index) => {
                ...
                if (FORBIDDEN_KEYS.includes(key) || typeof key !== 'string') {
                    ctx.throw({
                        message: `非法actionId${key}`
                    })
                }
                ret[key] = {
