...
import { getApiData } from './mock-data'
import { execApiGateWay, getUserFromApiGW } from '@bkui/apigateway-nodejs-sdk'
...
export default class NoCodeController {
    @OutputJson()
    @All('/(workflow|state|field|service|transition|role_type|v2/itsm)/*')
    async proxyApi (
    ...
    async executeApi (
        ...
        try {
            ...
            const username = getUserFromApiGW(ctx.header['x-bkapi-jwt'])
            const axiosParam = []
