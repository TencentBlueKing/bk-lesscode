...
export default class NoCodeController {
    ...
    async executeApi (
        ...
        try {
            ...
            console.log('----------------> bodyData', url, method, apiData, creatorUsername, projectId)
            const basePath = Reflect.getMetadata('basePath', dataSourceController)
            const controllerRoutes = Reflect.getMetadata('routes', dataSourceController)
            console.log('-----------------> controllerRoutes', controllerRoutes)
            ...
            if (tableNameParam && tableOperateHandler) {
                ...
                dataSourceController.prototype[tableOperateHandler](ctx)
            }
        } catch (error) {
            throw new global.BusinessError(error.message || error, -1, 500, error.stack)
        }
