...
export default class NoCodeController {
    ...
    async proxyApi (
        ...
        @BodyParams() body,
        @QueryParams() query,
        @CookieParams({ name: global.authName }) bkToken
    ) {
        ...
        const { result, data, message } = await execApiGateWay({
            ...
            authorization: {
                ...authorization,
                [global.authName]: bkToken
            },
            data: METHODS_WITHOUT_DATA.includes(methodLowerCase) ? { ...query, project_key: 'lesscode' } : { ...body, project_key: 'lesscode' },
