...
const VueCode = {
    ...
    async getPageCode (ctx) {
        ...
        try {
            ...
            let modelFuncList = []
            let newfuncGroups = funcGroups
            modelFuncList = targetData.reduce((pre, cur) => {
                if (cur?.functions?.length) {
                    pre = pre.concat(cur.functions)
                }
                return pre
            }, [])
            if (modelFuncList.length) {
                newfuncGroups = modelFuncList.reduce((pre, cur) => {
                    let existGroup = pre.find((item) => item.id === cur.funcGroupId)
                    if (!existGroup) {
                        existGroup = pre[0]
                    }
                    !existGroup.children.find((item) => item.funcCode === cur.funcCode) && existGroup.children.push({ ...cur, funcGroupId: existGroup.id })
                    return pre
                }, funcGroups)
            }
            ...
            if (!allVariableList) {
                ...
                modelVarList = targetData.reduce((pre, cur) => {
                    if (cur?.vars?.length) {
                        pre = pre.concat(cur.vars.map((item) => ({
                            ...item,
                            effectiveRange: 0
                        })))
                    }
                ...
                if (modelVarList.length) {
                    allVariableList = modelVarList.reduce((pre, cur) => {
                        const existVar = pre.find((item) => item.variableCode === cur.variableCode)
                        if (!existVar) {
                            pre.push(cur)
                        }
                        return pre
