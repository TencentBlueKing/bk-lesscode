...
export default class FunctionController {
    ...
    async generateToken (
        ...
        @CookieParams({ name: 'bk_token' }) bkTikcet
    ) {
        // 获取token, 目前测试环境用的都是 sundy820 获取token
        const bindAppCode = process.env.V3_ENV === 'prod' ? appCode : 'sundy820'
        const url = `${v3Config.URL_PREFIX}/bkapps/applications/${bindAppCode}/oauth/token/lesscode/?bk_app_code=${v3Config.APP_ID}&bk_app_secret=${encodeURI(v3Config.APP_SECRET)}&bk_token=${bkTikcet}`
        const reData = await http.get(url, {
            headers: {
                'X-USER-BK-TOKEN': bkTikcet
            }
        })
