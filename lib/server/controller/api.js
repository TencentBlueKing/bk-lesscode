...
export default class ApiManageController {
    ...
    // 执行 api
    @OutputJson()
    @Post('/execute/project/:projectId/code/:code')
    async executeApi (
        @PathParams({ name: 'code', require: true }) code,
        ...
        if (!apiData) {
            throw global.BusinessError(`API [CODE: ${code}]不存在，请修改后再试`, 404, 404)
        }
        const httpMethod = apiData.method
        const httpParams = [apiData.url]
        if (METHODS_WITHOUT_DATA.includes(httpMethod)) {
            httpParams.push({ params: httpData })
        } else {
            httpParams.push(httpData)
        }
