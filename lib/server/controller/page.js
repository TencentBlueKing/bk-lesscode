...
export const getPageList = async (ctx) => {
    try {
        ...
        const { versionId, lite } = ctx.query
        const res = await PageModel.getProjectPages(projectId, versionId)

        let list = res
        if (lite) {
            list = list.map(({ id, pageCode, pageName, pageType, framework }) => ({ id, pageCode, pageName, pageType, framework }))
        }
...
export const createDefaultPage = async (projectId) => {
    try {
        const project = await LCDataService.findOne(TABLE_FILE_NAME.PROJECT, { id: projectId })
...
export const createPage = async (ctx) => {
    try {
        const projectId = ctx.request.headers['x-project-id']
        const { versionId, pageData, layout } = ctx.request.body
        const projectPageData = {
            projectId,
            versionId
        }
...
export const updatePage = async (ctx) => {
    try {
        const projectId = ctx.request.headers['x-project-id']
        const { pageId, pageData, customCompData, versionId, pageCode, functionData, templateData, usedVariableMap, pageComponentPermActionMap = {} } = ctx.request.body
        pageData.id = pageId
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            // 布局模板
            if (templateData) {
                const { layoutId } = await getRepository(PageRoute).findOne({ where: { pageId } })
...
export const copyPage = async (ctx) => {
    try {
        const projectId = ctx.request.headers['x-project-id']
        const { pageId, versionId, pageData } = ctx.request.body
        pageData.id = pageId
        const projectPageData = {
            projectId,
            versionId
        ...
        const prePage = await getRepository(Page).findOne({ where: { id: postId } })
...
export const getDeletePageCodes = async (ctx) => {
    try {
        const projectId = ctx.request.headers['x-project-id']
        const { versionId } = ctx.request.query
...
export const checkName = async (ctx) => {
    const { pageName, pageCode, projectId, versionId, from, blurCheck = false } = ctx.request.body
    try {
        let existPageCode
...
async function queryPageDeatail (pageId) {
    ...
    const detail = await getRepository(Page).findOne({ where: queryParams }) || {}
...
// 导入json文件并返回内容
// 校验文件魔数（检查文件头是否为合法JSON）
const validateJsonFileMagic = (filePath) => {
    const buffer = Buffer.alloc(1024) // 读取前 1KB
    ...
    if (!content.startsWith('{') && !content.startsWith('[')) {
        throw new Error(global.i18n.t('文件格式不正确，必须是JSON格式'))
    }
}
...
export const importJson = async (ctx) => {
    ...
    try {
        ...
        if (!uploadFile || !uploadFile.path) {
            throw new Error(global.i18n.t('未上传文件'))
        }
        
        tempFilePath = uploadFile.path
        
        // 2. 文件大小限制 (15MB)
        const MAX_FILE_SIZE = 15 * 1024 * 1024
        const stats = fs.statSync(uploadFile.path)
        if (stats.size > MAX_FILE_SIZE) {
            throw new Error(global.i18n.t('文件大小超过限制(15MB)'))
        }
        
        // 3. 文件类型校验 (MIME + 扩展名)
        const allowedMimes = ['application/json', 'text/plain', 'text/json']
        const fileName = uploadFile.name || uploadFile.filename || ''
        const ext = path.extname(fileName).toLowerCase()
        
        if (ext !== '.json') {
            throw new Error(global.i18n.t('仅支持 .json 文件'))
        }
        
        if (uploadFile.type && !allowedMimes.includes(uploadFile.type)) {
            throw new Error(global.i18n.t('文件MIME类型不正确，仅支持JSON格式'))
        }
        
        // 4. 魔数校验（文件头检测）
        validateJsonFileMagic(uploadFile.path)
        ...
        try {
            ...
            throw new Error(global.i18n.t('JSON 格式不正确：{{n}}', { n: e.message }))
        }
        ...
        if (tempFilePath) {
            try {
                ...
                logger.error(`Failed to clean temp file: ${tempFilePath}`, e)
            }
        }
    }
}
...
const getPageLockStatus = async (ctx, pageData) => {
    ...
    const pageResp = await getRepository(Page).findOne({ where: { id: pageId } }) || {}
...
const updagePageActiveInfo = async (pageId, activeUser) => {
    ...
    const currentPage = await repository.findOne({ where: { id: pageId } }) || {}
    ...
    await repository.update({ id: currentPage.id }, updateFields)
    return Object.assign(currentPage, updateFields)
}
...
export const releasePage = async ctx => {
    try {
        ...
        if (activeUser === currentPage.activeUser) {
            await repository.update({ id: currentPage.id }, {
                activeUser: null,
                updateTime: currentPage.updateTime,
                updateUser: currentPage.updateUser
            })
...
export const getPreviewImg = async (ctx) => {
    const { pageId } = ctx.request.query
    let { previewImg } = await PageModel.findPagePreviewImg(pageId) || {}
