...
// import { createBkFlowSpace } from './flow'
...
const Realease = {
    ...
    async releaseProject (ctx) {
        ...
        try {
            ...
            // @todo 待bkflow支持传非当前应用app_code创建空间 20241023
            // 为部署应用创建bkflow空间
            ...
            // await createBkFlowSpace({projectId: releaseForm.projectId, name: bkflowSpaceName, appCode: releaseForm.appCode })
            ...
            await execApiGateWay({
                ...
                data: {
                    ...
                    permissions: ['create_space', 'create_task_without_template', 'get_task_detail', 'get_task_states', 'get_tasks_states', 'operate_task']
                },
    ...
    async getAppDetail (ctx) {
        try {
            const { appCode } = ctx.request.query
            const url = `${config.URL_PREFIX}/system/uni_applications/query/by_id/?id=${appCode}&bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}`
            const res = await ctx.http.get(url)
