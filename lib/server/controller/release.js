...
import { getFlowList } from '../service/business/no-code'
...
const Realease = {
    ...
    async releaseProject (ctx) {
        ...
        try {
            // 部署itsm流程
            const { list: flowList } = await getFlowList({ projectId: releaseForm.projectId })
            await Promise.all(flowList.map(flow => {
