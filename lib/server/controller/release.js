...
const Realease = {
    ...
    async applicationList (ctx) {
        try {
            ...
            const bkTikcet = ctx.cookies.get('bk_token')
            const url = `${config.URL_PREFIX}/bkapps/applications/lists/minimal?include_inactive=true&bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}&bk_token=${bkTikcet}`
    ...
    async moduleList (ctx) {
        try {
            ...
            const bkTikcet = ctx.cookies.get('bk_token')
            const url = `${config.URL_PREFIX}/bkapps/applications/${appCode}/modules/?source_origin=2&bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}&bk_token=${bkTikcet}`
            const res = await ctx.http.get(url)
    ...
    async detailInfo (ctx) {
        try {
            ...
            const prodInfo = await getReleaseInfo(projectId, project.appCode, project.moduleCode, 'prod', ctx.cookies.get('bk_token'), bindInfo)
            const stagInfo = await getReleaseInfo(projectId, project.appCode, project.moduleCode, 'stag', ctx.cookies.get('bk_token'), bindInfo)
    ...
    async releaseProject (ctx) {
        ...
        try {
            if (isNewPackage) {
                ...
                // 同步包到v3
                const syncRes = await uploadToV3(releaseForm, appInfo, codeUrl, ctx.cookies.get('bk_token'))
            ...
            // 部署包
            const deployRes = await deploy(releaseForm, appInfo, ctx.cookies.get('bk_token'))
    ...
    async getRunningLog (ctx) {
        try {
            ...
            const url = `${config.URL_PREFIX}/streams/${deployId}/history_events?bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}&bk_token=${ctx.cookies.get('bk_token')}`
            const res = await ctx.http.get(url) || []
    ...
    async detailFromV3 (ctx) {
        try {
            ...
            const project = await ProjectModel.findProjectDetail({ id: projectId }) || {}
            const url = `${config.URL_PREFIX}/bkapps/applications/${project.appCode}/modules/${project.moduleCode}/deployments/${deployId}/result/?bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}&bk_token=${ctx.cookies.get('bk_token')}`
            const res = await ctx.http.get(url)
    ...
    async offlineProject (ctx) {
        try {
            ...
            const postData = {
                bk_app_code: config.APP_ID,
                bk_app_secret: config.APP_SECRET,
                bk_token: ctx.cookies.get('bk_token')
            }
    ...
    async checkOfflineResult (ctx) {
        try {
            ...
            const url = `${config.URL_PREFIX}/bkapps/applications/${project.appCode}/modules/${project.moduleCode}/offlines/${offlineId}/result/?bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}&bk_token=${ctx.cookies.get('bk_token')}`
            const res = await ctx.http.get(url)
    ...
    async getDeployPhasesData (ctx) {
        try {
            ...
            let url = `${config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/envs/${env}/deploy_phases/?bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}&bk_token=${ctx.cookies.get('bk_token')}`
            if (deployId) {
                url = `${config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/envs/${env}/deploy_phases/${deployId}?bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}&bk_token=${ctx.cookies.get('bk_token')}`
            }
            const res = await ctx.http.get(url) || []
            ctx.send({
                code: 0,
    ...
    async getLastVersionProcesses (ctx) {
        try {
            ...
            const url = `${config.URL_PREFIX}/bkapps/applications/${appCode}/modules/${moduleCode}/envs/${env}/processes/list/?bk_app_code=${config.APP_ID}&bk_app_secret=${encodeURI(config.APP_SECRET)}&bk_token=${ctx.cookies.get('bk_token')}`
            const res = await ctx.http.get(url) || []
            ctx.send({
                code: 0,
                message: 'success',
