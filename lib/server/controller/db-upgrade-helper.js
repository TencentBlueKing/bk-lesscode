...
export const updateSlot = async (ctx) => {
    try {
            ...
            switch (name) {
                ...
                case 'card':
                    if (typeof value === 'string') {
                        res = { default: { name: 'html', type: 'html', displayName: global.i18n.t('卡片内容区'), val: value } }
                    } else {
        ...
        const errorMessage = errorPageIds.length ? global.i18n.t('出现错误的pageId：{{n}}', { n: errorPageIds.join(', ') }) : ''
        ctx.send({
            code: 0,
            message: `slot ${global.i18n.t('数据更新完成')}. ${errorMessage}`
        })
    } catch (error) {
...
export const updateDialogDev = async (ctx) => {
    try {
        ...
        ctx.send({
            code: 0,
            message: global.i18n.t('dialog(dev t环境专用)数据数据更新成功')
        })
    } catch (error) {
        ctx.send({
            code: -1,
...
export const updateDialogDataProd = async (ctx) => {
    try {
                                                ...
                                                'renderDirectives': [
                                                    {
                                                        ...
                                                        'prop': 'slots',
                                                        'val': '',
                                                        'defaultVal': global.i18n.t('文本配置不能为空')
                                                    }
                                                ],
        ...
        ctx.send({
            code: 0,
            message: global.i18n.t('dialog存量数据更新成功')
        })
    } catch (error) {
        ctx.send({
            code: -1,
...
export const updateVariableManage = async (ctx) => {
    try {
        ...
        ctx.send({
            code: 0,
            message: global.i18n.t('指令管理更新成功')
        })
    } catch (error) {
...
export const updateDirectives = async (ctx) => {
    try {
        ...
        ctx.send({
            code: 0,
            message: global.i18n.t('新版指令更新完成')
        })
    } catch (error) {
        ctx.send({
            code: -1,
...
export const updatePageLifeCycle = async (ctx) => {
    try {
        ...
        ctx.send({
            code: 0,
            message: global.i18n.t('页面lifeCycle更新完成')
        })
    } catch (error) {
        ctx.send({
            code: -1,
...
export const updatePagecode = async (ctx) => {
    if (Date.now() > new Date('2020-10-03 22:00:00').getTime()) {
        ctx.send(global.i18n.t('不在有效运行时间内，已退出'))
        return
    }
    ...
    try {
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            if (!pages || !pages.length) {
                ctx.send(global.i18n.t('暂无需要升级的数据，已退出'))
                return
            }
            ...
            ctx.send({
                code: 0,
                message: global.i18n.t('数据升级完成，data为部分结果'),
                data: {
...
export const setDefaultCompCategory = async (ctx) => {
    ...
    try {
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            ctx.send({
                code: 0,
                message: global.i18n.t('数据升级完成，data 为插入的新数据'),
                data: {
...
export const setProjectAndPageLayout = async (ctx) => {
    ...
    try {
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            ctx.send({
                code: 0,
                message: global.i18n.t('数据升级完成，data为部分结果_0'),
                data: {
...
export const updateProjectLayoutRoutePath = async (ctx) => {
    ...
    try {
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            ctx.send({
                code: 0,
                message: global.i18n.t('数据升级完成，data为部分结果_1'),
                data: {
                    page: layoutInstResult.slice(0, 5)
                }
            })
...
export const updateDeledPageRel = async (ctx) => {
    ...
    try {
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            ctx.send({
                code: 0,
                message: global.i18n.t('数据升级完成，data为影响数据'),
                data: {
...
export const delPageRouteDirtyRow = async (ctx) => {
    ...
    try {
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            ctx.send({
                code: 0,
                message: global.i18n.t('数据升级完成，data为影响数据_0'),
                data: {
    ...
    try {
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            if (!emptyCodeLayoutList || !emptyCodeLayoutList.length) {
                ctx.send(global.i18n.t('暂无需要升级的数据，已退出_0'))
                return
            }
            ...
            ctx.send({
                code: 0,
                message: global.i18n.t('数据升级完成，data为部分结果_2'),
                data: {
                    page: layoutInstResult.slice(0, 5)
                }
            })
