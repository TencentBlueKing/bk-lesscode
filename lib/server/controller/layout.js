...
const Layout = {
    ...
    async create (ctx) {
        try {
            const projectId = ctx.request.headers['x-project-id']
            const { versionId = null, layoutData } = ctx.request.body
    ...
    async setDefault (ctx) {
        const projectId = ctx.request.headers['x-project-id']
        const { id: layoutId, versionId, layoutType } = ctx.request.body
        try {
            const layoutsInstRepo = getRepository(LayoutInst)
            const targetTypeInsts = await layoutsInstRepo.find({ layoutType, versionId, projectId: parseInt(projectId) })
            targetTypeInsts.forEach(inst => {
                if (inst.id === parseInt(layoutId)) {
                    inst.isDefault = true
                    return
                }
                inst.isDefault = false
            })
            await getRepository(LayoutInst).save(targetTypeInsts)

            ctx.send({
                code: 0,
                message: 'success',
                data: parseInt(layoutId)
            })
        } catch (err) {
            ctx.throwError({
                message: err.message
    ...
    async setRoutePath (ctx) {
        const projectId = ctx.request.headers['x-project-id']
        const { id, versionId, routePath, layoutType = 'PC' } = ctx.request.body
        try {
            const existRoutePath = await getRepository(LayoutInst).findOne({ where: { projectId, versionId, routePath } })
            if (existRoutePath) {
                throw Error(global.i18n.t('该模板路由已存在'))
            }
            if (layoutType !== 'MOBILE' && routePath.startsWith('/mobile')) {
                ctx.throwError({
                    message: global.i18n.t('web端路由不能以/mobile开头')
                })
            }
    ...
    async checkName (ctx) {
        ...
        try {
            if (showName) {
                const existShowName = await getRepository(LayoutInst).findOne({ where: { projectId, versionId, showName, deleteFlag: 0 } })
                if (existShowName) {
                    ctx.throwError({
                        message: global.i18n.t('该模板名称已存在')
                    })
                }
            }

            if (layoutCode) {
                const existLayoutCode = await getRepository(LayoutInst).findOne({ where: { projectId, versionId, layoutCode, deleteFlag: 0 } })
                if (existLayoutCode) {
                    ctx.throwError({
                        message: global.i18n.t('该模板ID已存在')
                    })
                }
            }

            if (routePath) {
                const existRoutePath = await getRepository(LayoutInst).findOne({ where: { projectId, versionId, routePath, deleteFlag: 0 } })
                if (existRoutePath) {
                    ctx.throwError({
                        message: global.i18n.t('该模板路由已存在_0')
                    })
                }
                if (layoutType !== 'MOBILE' && routePath.startsWith('/mobile')) {
                    ctx.throwError({
                        message: global.i18n.t('web端路由不能以/mobile开头_0')
                    })
                }
            }
