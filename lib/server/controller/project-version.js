...
module.exports = {
    async create (ctx) {
        ...
        const { version, versionLog } = ctx.request.body
        try {
            const has = await projectVersionService.has(projectId, version)
            if (has) {
                ctx.throw(400, global.i18n.t('应用版本已存在'), { code: CODE.BIZ.PROJECT_VERSION_EXISTED })
            }
    ...
    async update (ctx) {
        const projectId = ctx.request.headers['x-project-id']
        const { id: versionId, ...editData } = ctx.request.body
        try {
