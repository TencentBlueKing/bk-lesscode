...
module.exports = {
    ...
    async queryProjectPageRoute (ctx) {
        try {
            const projectId = ctx.request.header['x-project-id']
    ...
    async savePageRoute (ctx) {
        try {
            const projectId = ctx.request.header['x-project-id']
            const { versionId, pageRoute, platform } = ctx.request.body

            pageRoute.path = formatRoutePath(pageRoute.path)

            if (platform !== 'MOBILE' && pageRoute.layoutPath === '/' && pageRoute.path === 'mobile') {
                throw Error(global.i18n.t('web端路由不能以/mobile开头'))
            }
    ...
    async getProjectRouteTree (ctx) {
        try {
            ...
            routeList.forEach(route => {
                ...
                const { layoutId, layoutPath, layoutType, type, showName } = route
    ...
    async createProjectRoute (ctx) {
        try {
            const projectId = ctx.request.header['x-project-id']
            const { pageRoute, versionId, platform } = ctx.request.body
    ...
    async updatePageRoute (ctx) {
        try {
            const projectId = ctx.request.header['x-project-id']
            const { versionId, pageId, pageRoute } = ctx.request.body

            await getConnection().transaction(async transactionalEntityManager => {
                const curPageRouteData = await getRepository(PageRoute).findOne({ where: { pageId } })
                if (pageRoute.layoutId) {
                    const fullPath = `${pageRoute.layoutPath}/${pageRoute.path}`
                    if (await hasRoute({ path: fullPath }, projectId, versionId)) {
                        throw Error(global.i18n.t('该布局模板下存在相同的路由'))
                    }
                ...
                if (pageRoute.routeId) {
                    const newPageRouteData = await getRepository(PageRoute).findOne({ where: { routeId: pageRoute.routeId } })
                    await transactionalEntityManager.save(PageRoute, {
    ...
    async bindPageRoute (ctx) {
        try {
            const projectId = ctx.request.header['x-project-id']
            const { routeId, pageId, redirect, remove, createRoot, layoutId, layoutPath, versionId } = ctx.request.body

            let finalRouteId = routeId

            // 创建布局下的根路由，用于绑定布局根路由同时根路由不存在的情况
            if (createRoot) {
                const path = ''
                ...
                const routeData = { path, layoutId: layoutId, versionId }
                // 创建路由及其关联记录
                const result = await routeModel.createProjectRoute(projectId, { routeData })
                finalRouteId = result.id
            }

            const pageRouteRepo = getRepository(PageRoute)
            const pageRouteRow = await pageRouteRepo.findOne({ where: { routeId: finalRouteId } })
    ...
    async removeRoute (ctx) {
        try {
            ...
            await getConnection().transaction(async transactionalEntityManager => {
                ...
                // 解绑页面与路由关联或删除关联记录
                const pageRouteRow = await getRepository(PageRoute).findOne({ where: { routeId } })
