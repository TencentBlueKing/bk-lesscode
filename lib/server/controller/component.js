...
export const create = async (ctx) => {
    ...
    try {
        ...
        const checkFileds = ['name', 'displayName', 'type', 'dest', 'version', 'categoryId', 'description', 'log', 'belongProjectId', 'compType', 'framework']
...
export const register = async (ctx) => {
    ...
    try {
        ...
        // 如果是allCompsFlag为true， 则拉取所有， 否则只拉取当前页面使用的自定义组件
...
export const scope = async (ctx) => {
    const { scope, compId, projectId, targetProjects, publicType } = ctx.request.body
    try {
        await getConnection().transaction(async transactionalEntityManager => {
