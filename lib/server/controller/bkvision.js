...
import { getAuthQuerySubfix, getAuthPostParams } from '../service/business/v3-service'
...
export default class bkVisionController {
    ...
    async getShareList (
        ...
        // 鉴权信息
        const queryStr = getAuthQuerySubfix(ctx.cookies)
        
        const url = process.env.BK_VISION_API_URL + '/api/v1/share/get_share_list/?' + queryStr + `&app_code=${v3Config.APP_ID}`
        const res = await ctx.http.get(url)
