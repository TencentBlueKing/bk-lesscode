...
export const getUserBaseList = async (ctx) => {
    ...
    const where = ['`user`.deleteFlag  = 0', '`user`.username  != \'' + DEFAULT_PROJECT_CREATOR + '\'']
    const escaped = []
    if (params.time) {
    ...
    if (params.orderBy) {
        ...
        const sortSqls = {
            createTime: `select username, createTime from user WHERE ${where.join(' AND ')} ORDER BY createTime ${order}`,
            projectCount: `
                SELECT
    ...
    try {
        const result = await manager.query(
            ...
            from
                user
            where
                user.username  != '${DEFAULT_PROJECT_CREATOR}'
...
export const getProjectBaseList = async (ctx) => {
    ...
    // 默认去除demo项目
    const where = [`project.deleteFlag = 0`, `project.projectCode != '${OFFCIAL_PROJECT_CODE}'`]
    ...
    if (params.orderBy) {
        ...
        const sortSqls = {
            createTime: `select id, projectCode, projectName, createTime from project WHERE ${where.join(' AND ')} ORDER BY createTime ${order}`,
            pageCount: `
                SELECT
                    project.id,
                    projectCode,
    ...
    try {
        const result = await manager.query(
            ...
            where
                ...
                AND projectCode != '${OFFCIAL_PROJECT_CODE}'
                AND deleteFlag = 0
            GROUP BY time
            ORDER BY time`,
            [new Date(Number(params.time[0])), new Date(Number(params.time[1]))]
    ...
    try {
        const result = await manager.query(
            ...
            where
                ...
                AND project.projectCode != '${OFFCIAL_PROJECT_CODE}'
                AND page.deleteFlag = 0
            `,
            [new Date(Number(params.time[0])), new Date(Number(params.time[1]))]
        )
...
export const getFuncBaseList = async (ctx) => {
    ...
    const where = [
        ...
        `project.projectCode != '${OFFCIAL_PROJECT_CODE}'`
    ]
...
export const getFuncCountByTime = async (ctx) => {
    ...
    try {
        const result = await manager.query(
            `select
                DATE_FORMAT(func.createTime, ${getDateFormatDes(params.timeType)}) as time,
                count(*) as count
            from
                func
