...
const doubleCheck = async (functionData) => {
    ...
    checkList.forEach(({ funcName, id }) => {
        if (!/^[A-Za-z_][A-Za-z]*[A-Za-z_]?$/.test(funcName)) {
            throw new global.BusinessError(global.i18n.t('函数名称由大小写英文字母组成，开头和结尾还可以是下划线。函数名称【{{n}}】不符合规则，请修改后再试', { n: funcName }), 400, 400)
        }
        if (
            Reflect.has(nameMap, funcName)
            && nameMap[funcName] !== id
        ) {
            throw new global.BusinessError(global.i18n.t('已存在函数名称为【{{n}}】的函数，请修改后再试', { n: funcName }), 400, 400)
        }
    })
}
...
export default class FunctionMarketController {
    ...
    @OutputJson()
    @ProjectAuthorization({ getId: ctx => ctx.request.body.functionData.projectId, needAuthActions: [IAM_ACTION.develop_app[0]] })
    ...
    async bulkCreateFunction (
        ...
        marketFuncList.forEach(({ funcName }) => {
            if (nameMap[funcName]) {
                throw new global.BusinessError(global.i18n.t('批量添加的函数中，存在重复的函数名【{{n}}】，请修改后再试', { n: funcName }), 400, 400)
            } else {
