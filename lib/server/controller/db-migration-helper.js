...
const apiArr = [
    ...
    'newTransformDataSourceType',
    'newTransformShowOperationColumn',
...
// eslint-disable-next-line no-unused-vars
async function newTransformDataSourceType () {
    try {
        const pageRepository = getRepository(Page)
        const pageList = await pageRepository.find({
        ...
        return {
            code: 0,
            message: `newTransformDataSourceType success：${(new Date()).toString()}`,
            data: null
        }
    } catch (error) {
        console.dir(error)
...
// eslint-disable-next-line no-unused-vars
async function newTransformShowOperationColumn () {
    try {
        const pageRepository = getRepository(Page)
        const pageList = await pageRepository.find({
            select: ['id', 'updateTime', 'updateUser', 'content']
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            const modifyData = (node) => {
                // update showOperationColumn
                const renderProps = node?.renderProps || {}
                Object
                    .keys(renderProps)
                    .forEach((renderPropKey) => {
                        const renderProp = renderProps[renderPropKey]
                        if (renderProp.valueType === 'table-data-source' && renderProp?.payload?.sourceData && renderProp.payload.sourceData.showOperationColumn === undefined) {
        ...
        return {
            code: 0,
            message: `newTransformShowOperationColumn success：${(new Date()).toString()}`,
            data: null
        }
    } catch (error) {
        console.dir(error)
...
async function insertLayoutTypeInLayoutInst () {
    try {
        ...
        return {
            code: 0,
            message: 'layoutInst更新成功,添加layoutType列'
        }
    } catch (error) {
        return {
            code: -1,
