...
import IamAppPermModel from '../model/entities/iam-app-perm-model'
...
async function initIamAppPermModelData (apiName) {
    const projectRepo = getRepository(Project)
    try {
        await getConnection().transaction(async transactionalEntityManager => {
            const projectList = await projectRepo.find()

            const iamAppPermModelList = projectList.map(project => {
                const { id } = project
                return {
                    projectId: id,
                    ...
                    systemManagers: [project.createUser],
            ...
            await transactionalEntityManager.save(IamAppPermModel, iamAppPermModelList)
        })
        return {
            code: 0,
            message: `${apiName}: Insert success`
