...
import { METHODS_WITHOUT_DATA, API_PARAM_TYPES, getDefaultJsonScheme } from '../../shared/api'
...
async function generateApi () {
    try {
        ...
        const parseData2JsonScheme = (data, name = 'root') => {
            const jsonScheme = getDefaultJsonScheme({
                name
            })
            const type = Object.prototype.toString.apply(data)
            if (API_PARAM_TYPES.OBJECT.TYPE === type) {
                jsonScheme.properties = {}
                Object.keys(data).forEach((key) => {
                    jsonScheme.properties[key] = parseData2JsonScheme(data[key], key)
                })
            }

            if (API_PARAM_TYPES.ARRAY.TYPE === type) {
                jsonScheme.items = parseData2JsonScheme(data[0], 'items')
            }
        ...
        await getConnection().transaction(async transactionalEntityManager => {
            const projectIdCategoryIdMap = {}
            const taskList = []
            // 生成默认分类
            projectList.forEach((project) => {
                const categoryEntity = categoryRepository.insert({
                    name: '默认分类',
                    projectId: project.id,
                    versionId: project.versionId
                })
                projectIdCategoryIdMap[project.id] = categoryEntity.id
                taskList.push(
                    transactionalEntityManager.save(categoryEntity)
                )
            })
            funcList.forEach(func => {
                if (func.funcType === 1) {
                    ...
                    if (func.funcApiData) {
                        try {
                            const apiData = JSON.parse(func.funcApiData)
                            if (METHODS_WITHOUT_DATA.includes(func.funcMethod)) {
                                // 构造 scheme
                                httpScheme.query = []
                                Object.keys(apiData).forEach((key) => {
                                    httpScheme.query.push({
                                        name: key,
                                        required: false,
                                        default: '',
                                        description: '',
                                ...
                                // 构造 scheme
                                httpScheme.body = parseData2JsonScheme(apiData)
                            }
                        } catch (error) {
                            console.log(`${func.id} 存在不符合规范的 funcApiData，未转换为相应的模型，请手动修改 api`)
                        }
                    }
                    const apiEntity = apiRepository.insert({
                        name: func.funcName,
                        code: func.funcCode,
                        method: func.funcMethod,
                        url: func.funcApiUrl,
                        deleteFlag: func.deleteFlag,
                        projectId: func.projectId,
                        versionId: func.versionId,
                        categoryId: projectIdCategoryIdMap[func.projectId],
                        header: {},
                        query: {},
                        body: {},
                        response: {},
                    ...
                    taskList.push(
                        transactionalEntityManager.save(apiEntity)
                    )
                    // 更新 query or body
                    taskList.push(
                        transactionalEntityManager.update(Func, {
                            id: func.id
                        }, {
