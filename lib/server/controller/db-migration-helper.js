        ...
        await getConnection().transaction(async transactionalEntityManager => {
            ...
            const iamAppPermList = projectList.map(project => {
                ...
                return {
                    projectId: id,
                    systemId: systemId,
                    systemName: `蓝鲸可视化开发平台-${projectName}`,
                    systemNameEn: `lesscode-${projectCode}`,
                    systemDesc: `蓝鲸可视化开发平台-${projectName}`,
...
async function updateRemoteOption (type) {
    try {
        ...
        allPageData.forEach(page => {
            ...
            (targetData || []).forEach((grid, index) => {
                const callBack = (component) => {
                /** 对charts中，配置了remoteOptions的数据，同步至options中 */
                    if (component.type === type && component.renderProps.remoteOptions?.payload?.methodCode) {
                        const originValue = JSON.parse(JSON.stringify(component.renderProps.remoteOptions))
                        component.renderProps.options = { ...originValue, ...{ 'buildInVariableType': 'SYSTEM' } }
                    }
                }
                walkGrid(targetData, grid, callBack, callBack, index)
            })
        ...
        return {
            code: 0,
            message: `${type}remoteOptions数据更新成功`
        }
    } catch (error) {
        return {
            code: -1,
