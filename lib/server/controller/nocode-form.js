...
export default class NocodeFormController {
    ...
    async createForm (
        ...
        const { projectId, tableName } = formData
        const hasTable = await LCDataService.findOne(TABLE_FILE_NAME.DATA_TABLE, { projectId, tableName, deleteFlag: 0 })
        if (hasTable) {
            throw new Error(global.i18n.t('已存在表名为{{0}}的数据表', [tableName]))
        }
