    ...
    async getTemplateInfo (
        ...
        try {
            ...
            list = list.map(({ id, layoutId, path, layoutPath, ...others }) => {
                const fullPath = layoutPath?.endsWith('/') ? layoutPath : `${layoutPath}/`
                const routePath = fullPath + path
                return {
                    ...others,
                    routePath: `/preview-mobile/project/${projectId}?pagePath=${routePath}&pageCode=${others?.pageCode}&pageId=${others?.pageId}`
                }
            })
            ctx.send({
                code: 0,
            ...
            throw new global.BusinessError(err.message || err, -1, 500, err.stack)
        }
    }
}
