...
const modifyMessage = (message) => {
    let result = message
    if ([
        /Parameters error \[reason=\"app code cannot be empty\"\]/,
        /Parameters error \[reason=\"参数 app_code 为空，请进行检查\"\]/
    ].some(reg => result.match(reg))) {
        result = 'Parameters error [reason="参数 app_code 为空，请进行检查"]，请在该应用下凭证管理页面绑定蓝鲸应用信息并获取用户认证凭证'
    }
    return result
}
        ...
        try {
            ...
            const options = {
                headers: {
                    ...
                    'X-TIMEZONE-OFFSET': ctx.request.headers['x-timezone-offset']
                }
            }
            if (withToken) {
            ...
            const { data } = await ctx.http[type](...axiosParam)
            if (data?.message) {
                data.message = modifyMessage(data.message)
            }
            ctx.send(data)
        } catch (err) {
            ctx.throwError({
                message: modifyMessage(err.message)
            })
        }
    },
