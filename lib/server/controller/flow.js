...
export default class FlowController {
    ...
    async getFlowTplList (
        ...
        list.forEach(tpl => {
            const relatedFlowManageContainerPages = []
            pages.forEach(page => {
                const components = JSON.parse(page.content || '[]')

                const traverse = (list) => {
                    for (const comp of list) {
                        if (comp.type === CONTAINER_TYPE_MAP.flowManageContainer) {
                            if (CHECK_CONDITIONS[comp.type]?.(comp, tpl.id)) {
                                const { id, pageName: name, pageCode } = page
                                relatedFlowManageContainerPages.push({ id, name, pageCode })
                            }
                        }
            ...
            tpl.flowManagePages = relatedFlowManageContainerPages
        })
        return { list, count }
    }
    ...
    @OutputJson()
    @ProjectAuthorization({})
    @Post('/tpl/:tplId/updateRelatedPages')
    async updateRelatedPages (
        @HeaderParams({ name: 'x-project-id', require: true }) projectId,
        @PathParams({ name: 'tplId', require: true }) tplId,
        @BodyParams({ name: 'added' }) added, // 页面新增加的容器，{ [key: number]: Array<containerConfig> }
        @BodyParams({ name: 'removed' }) removed // 页面删除的容器, { [key: flowManageContainer|dataManageContainer|formContainer]: Array<{ pageId: number; nodeId?: string; }> }
    ) {
    ...
    @OutputJson()
    @ProjectAuthorization({})
    @Post('/tpl/:tplId/updateRelatedPageContent')
