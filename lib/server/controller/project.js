...
import { fetchAllAppHasActionsPerm } from './iam'
...
module.exports = {
    ...
    async queryProject (ctx) {
        ...
        if (filter !== 'official') {
            ...
            const authedAppRet = await fetchAllAppHasActionsPerm(ctx)

            projectList.forEach(item => {
                // item.hasPerm = item.createUser === userInfo.username
                //     || !!(allAppIdList.indexOf(String(item.id)) > -1)
                item.canDevelop = item.createUser === userInfo.username
                    || (authedAppRet['develop_app'].indexOf(String(item.id)) > -1)
                item.canDeploy = item.createUser === userInfo.username
                    || (authedAppRet['deploy_app'].indexOf(String(item.id)) > -1)
                item.canManagePerms = item.createUser === userInfo.username
                    || (authedAppRet['manage_perms_in_app'].indexOf(String(item.id)) > -1)
                item.canManage = item.createUser === userInfo.username
                    || (authedAppRet['manage_app'].indexOf(String(item.id)) > -1)
                item.canCreate = item.createUser === userInfo.username
                    || (authedAppRet['create_app'].indexOf(String(item.id)) > -1)
            })
        }
    ...
    async createDemoProject (data) {
        console.error('createDemoProjectcreateDemoProjectcreateDemoProjectcreateDemoProject')
        ...
        try {
            ...
            console.error('projectIdprojectIdprojectId', projectId)
