...
import { fetchAllAppHasActionPerm } from './iam'
    ...
    async queryProject (ctx) {
        ...
        switch (filter) {
            ...
            default:
                ...
                query.condition = query.condition.join(' AND ')
                projectList = await projectModel.queryAllProjectInPlatform(query)

                // 查询当前用户具有 DEVELOP_APP 操作权限的所有应用列表
                const allAppListRes = await fetchAllAppHasActionPerm(ctx, IAM_ACTION.DEVELOP_APP)
                const allAppIdList = allAppListRes.data.value || []
                console.error('allAppListRes')
                console.error(allAppListRes)
                console.error()
