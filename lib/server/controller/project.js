...
import { fetchAllAppHasActionPerm, check } from './iam'
...
module.exports = {
    ...
    async queryProject (ctx) {
        ...
        if (filter !== 'official') {
            // 查询当前用户具有 DEVELOP_APP 操作权限的所有应用列表
            const allAppListRes = await fetchAllAppHasActionPerm(ctx, IAM_ACTION.develop_app[0])
            const allAppIdList = allAppListRes.data.value || []

            projectList.forEach(item => {
                // 自己创建的项目或者已经申请操作权限的项目具有 DEVELOP_APP 操作权限
    ...
    async verify (ctx) {
        try {
            ...
            const userInfo = ctx.session.userInfo
            const project = await iamModel.queryProjectByCreatorAndProjectId(userInfo.username, id)
            // 是本人创建的项目
