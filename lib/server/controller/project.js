...
import { fetchActionsPerm, checkInServer, createRatingManager } from './iam'
...
module.exports = {
    async createProject (ctx) {
        ...
        try {
            ...
            const iamRes = await createRatingManager(ctx, projectId, projectCode, projectName)
            if (!iamRes.result) {
                res.code = iamRes.code
                res.message = iamRes.message
                res.data = null
            }

            console.error('iamResiamRes', iamRes)
    ...
    async queryProject (ctx) {
        ...
        if (filter !== 'official') {
            ...
            projectList.forEach(item => {
                item.canDevelop = item.createUser === userInfo.username
                    || (authedRet['develop_app'].indexOf(String(item.id)) > -1)
                item.canDeploy = item.createUser === userInfo.username
                    || (authedRet['deploy_app'].indexOf(String(item.id)) > -1)
                item.canManagePerms = item.createUser === userInfo.username
                    || (authedRet['manage_perms_in_app'].indexOf(String(item.id)) > -1)
                item.canManage = item.createUser === userInfo.username
                    || (authedRet['manage_app'].indexOf(String(item.id)) > -1)
            })
        }
    ...
    async projectDetail (ctx) {
        try {
            ...
            detail.canDevelop = detail.createUser === userInfo.username || iamRes[IAM_ACTION.develop_app[0]]
            detail.canDeploy = detail.createUser === userInfo.username || iamRes[IAM_ACTION.deploy_app[0]]
            detail.canManagePerms = detail.createUser === userInfo.username || iamRes[IAM_ACTION.manage_perms_in_app[0]]
            detail.canManage = detail.createUser === userInfo.username || iamRes[IAM_ACTION.manage_app[0]]
