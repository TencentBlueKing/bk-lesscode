...
module.exports = {
    ...
    async importProject (ctx) {
        const { importProjectData, projectName, projectCode, projectDesc, framework } = ctx.request.body
        // 校验projectCode和projectName
        const checkRes = await projectModel.validProject({ projectCode, projectName })
        if (checkRes.status > 0) {
            ctx.throw(checkRes.status, checkRes.errMsg, { code: checkRes.code })
        ...
        try {
            ...
            ctx.throw(global.i18n.t('导入应用失败:') + (e.message || e))
        }
    },
    ...
    async exportProject (ctx) {
        ...
        if (!projectId || isNaN(projectId) || projectId <= 0) {
            ctx.throw(400, global.i18n.t('无效的项目ID'))
        }
        ...
        try {
            ...
            ctx.throw(global.i18n.t('导出应用失败'))
        } finally {
            // 确保清理临时文件
    ...
    async queryProject (ctx) {
        ...
        if (filter === 'archive') {
            query.condition.push('project.archiveFlag = 1')
        } else {
            query.condition.push('project.archiveFlag = 0')
        }
    ...
    async checkname (ctx) {
        const { name, projectCode, isBlurCheck = false } = ctx.request.body
        // isBlurcheck时， data为true， 代表重复
        let res = {
            code: 0,
            message: 'OK',
            data: null
            ...
            if (name) {
                ...
                if (foundNameProject && foundNameProject.id) {
                    if (isBlurCheck) {
                        ...
                        ctx.throw(200, global.i18n.t('应用名称已经存在'), { code: CODE.BIZ.PROJECT_NAME_EXISTED })
                    }
                }
            } else if (projectCode) {
                const founCodeProject = await projectModel.findProjectDetail({ projectCode })
                if (founCodeProject && founCodeProject.id) {
                    if (isBlurCheck) {
                        res = {
                            code: 0,
                            message: 'OK',
                        ...
                        ctx.throw(200, global.i18n.t('应用ID已经存在'), { code: CODE.BIZ.PROJECT_CODE_EXISTED })
                    }
                }
            }
    ...
    async my (ctx) {
        try {
            ...
            const query = {
                condition: [],
                params: {},
                select: ['project.id', 'project.projectCode', 'project.appCode', 'project.createUser', 'project.projectName', 'project.isEnableDataSource', 'project.framework']
            }
    ...
    async createDemoProject (ctx, data) {
        ...
        try {
            ...
            if (global.IAM_ENABLE && ctx.session) {
                ...
                if (!batchAuthRes.result) {
                    const msg = global.i18n.t('demo 项目在权限中心授权失败: {{0}}: {{1}}', [batchAuthRes.code, batchAuthRes.message])
                    logger.info(msg)
                }
                ...
                if (!createRatingManagerRes.result) {
                    const msg = global.i18n.t('demo 项目在权限中心创建分级管理员失败: {{0}}: {{1}}', [createRatingManagerRes.code, createRatingManagerRes.message])
                    logger.info(msg)
                }
            }
            return projectId
