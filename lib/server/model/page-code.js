...
import { getRenderMap } from '../../client/src/element-materials/modifier/component/slots/render-config'
                ...
                if (format !== 'value') {
                    ...
                    if (method.funcName && method.funcCode) {
                        if (method?.funcParams?.length > 0) {
                            propsStr += `:${propName}="(args) => ${method.funcName}(${this.getFuncParamStr(method, params, `属性【${i}】`)}, ...args)"`
                        } else {
        ...
        if (typeof events === 'object' && Object.keys(events).length) {
            for (const key in events) {
                ...
                if (fun.id && (event.enable === undefined || event.enable)) {
                    ...
                    if (fun?.funcParams?.length > 0) curEventStr = `@${key}="(...args) => ${fun.funcName}(${this.getFuncParamStr(fun, params, `事件【${key}】`, false, 'event')}, ...args)" `
    ...
    renderSlot (type, slots, compId) {
        ...
        slotKeys.forEach((key) => {
            ...
            if (Array.isArray(slot)) {
                ...
                const render = getRenderMap(2)[slot.name] || (() => {})
    ...
    getLifeCycle () {
        ...
        lifeCycleKeys.forEach((key) => {
            ...
            if (method.id) {
                ...
                lifeCycleStrObj[key].push(`this.${method.funcName}(${this.getFuncParamStr(method, params, `页面的【${key}】生命周期`, true)})`)
            }
        })
        if (this.remoteDataStr) {
        ...
        if (method.id) {
            if (chartType) {
                this.remoteDataStr += key.endsWith('options')
                    ? `const ${key}Remote = await this.${method.funcName}(${this.getFuncParamStr(method, params, `远程函数【${method.funcName}】`, true)})\nthis.$set(this, '${key}', ${key}Remote)\n`
                    : `const ${key}Remote = await this.${method.funcName}(${this.getFuncParamStr(method, params, `远程函数【${method.funcName}】`, true)})\nObject.assign(this.${key}, ${key}Remote)\n`
            } else {
                this.remoteDataStr += `this.${key} = await this.${method.funcName}(${this.getFuncParamStr(method, params, `远程函数【${method.funcName}】`, true)})\n`
            }
        }
        // 处理chartRemote
        if (method.funcCode) this.addUsedFunc(method.funcCode)
