    ...
    getPropsStr (type, props, compId, dirProps, slots) {
        ...
        for (const i in props) {
            ...
            if (i !== 'slots' && i !== 'class') {
                ...
                if (format !== 'value') {
                    propVar = val
                } else if (buildInVariableType === 'CUSTOM' && payload?.customVariableCode) {
                    // 自定义变量代替默认变量名称
                    propVar = payload.customVariableCode
                    format = 'variable'
                    val = payload.customVariableCode
                }
        ...
        slotKeys.forEach((key) => {
            ...
            if (Array.isArray(slot)) {
                ...
                do {
                    ...
                    let slotVar = compId
                    // 自定义变量代替默认变量名称
                    if (slot.format === 'value' && slot.buildInVariableType === 'CUSTOM' && slot.payload?.customVariableCode) {
                        slot.format = 'variable'
                        slot.val = slot.payload.customVariableCode
                        slotVar = slot.payload.customVariableCode
                    }
  
                    if (slot.format !== 'value') {
                        disPlayVal = this.handleUsedVariable(slot.format, slot.val, compId)
                    } else if (methodData.methodCode) {
                        this.dataTemplate(slotVar, transformToString(disPlayVal))
                        ...
                        if (typeof slot.val === 'object') {
                            this.dataTemplate(slotVar, transformToString(slot.val))
                            disPlayVal = slotVar
                        } else {
                            disPlayVal = slot.val
                            param.type = 'value'
