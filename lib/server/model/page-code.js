...
class PageCode {
    ...
    getComplexLayout (navContent, componentProps) {
        ...
        return `
            <bk-navigation ${componentProps} head-theme-color=${topMenuBackground} navigation-type="top-bottom" :need-menu="${curLeftMenuKey}.length > 0" class="bk-layout-custom-component-wrapper" @toggle="v => toggleActive=v" theme-color="${sideThemeColor}" :class="{ 'white-navigation': ${isWhiteTopTheme} }">
                ...
                <div class="navigation-header" slot="header">
                    <ul class="header-nav">
                        <li v-for="(item) in ${complexMenuKey}" :class="{ 'item-active': item.id === curNav.id, 'theme-item': ${!isDefaultTopTheme} }" :key="item.id" theme="light navigation-message" class="header-nav-item" @click="goToPage(item)">
                            {{item.name}}
                        </li>
                    </ul>
                    <bk-popover class="nav-head-right" theme="light navigation-message" :arrow="false" offset="-10, 0" placement="bottom-start" :tippy-options="{ 'hideOnClick': false, appendTo: 'parent' }">
    ...
    generateCode (v, inFreeLayout = false, inSwiper = false) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (item.type === 'render-grid') {
                ...
                code += `
                    ${itemClass ? `\n<div${alignStr}class="bk-layout-row-${this.uniqueKey} ${item.componentId}" ${vueDirective} ${propDirective}>` : `<div class="bk-layout-row-${this.uniqueKey}" ${vueDirective} ${propDirective}>`}
                        ${item.renderSlots && item.renderSlots.default && item.renderSlots.default.map(col => {
                            ...
                            return `<div class="bk-layout-col-${this.uniqueKey} ${col.componentId}"${colAlignStr}>
    ...
    getPropsStr (type, props, compId, dirProps, slots) {
        ...
        if (props.data?.valueType === 'table-data-source') {
            propsStr += `tableName="${props.data?.payload?.sourceData?.tableName}" `
        }
        ...
        for (const i in props) {
            ...
            if (i !== 'slots' && i !== 'class') {
                ...
                if (format !== 'value') {
                    ...
                    this.handleUsedVariable('variable', val, compId)
                }
                            ...
                            .reduce((acc, cur) => {
                                ...
                                if (cur === 'count') {
                                    ...
                                    if (item.buildInVariableType === 'CUSTOM') {
                                        this.handleUsedVariable('variable', item.customVariableCode, compId)
                                    ...
                                    this.handleUsedVariable(item.format, item.code, compId)
            ...
            if (Array.isArray(slot)) {
                ...
                do {
                    ...
                    if (slot.format === 'value' && slot.buildInVariableType === 'CUSTOM' && customVariableCode) {
                        ...
                        this.handleUsedVariable('variable', slot.val, compId)
                    }
    ...
    dataSourceTemplate (key, sourceData) {
        const hashId = uuid(4)
        this.remoteDataStr += `const ${key}Source${hashId} = await this.$http.get('/data-source/user/projectId/${this.projectId}/tableName/${sourceData.tableName}')\nthis.${key} = ${key}Source${hashId}.data.list || []\n`
    }
