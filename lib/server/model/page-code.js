    ...
    generataNocodePage () {
        ...
        switch (this.nocodeType) {
            case 'FORM':
                ...
                propsStr += ` :fields="field${key}" table-name="${tableName || ''}" :form-id="${formId || 0}"`
                break
            case 'FLOW':
                this.dataTemplate(`field${key}`, JSON.stringify(fields))
                propsStr += ` :fields="field${key}" table-name="${tableName || ''}" :form-id="${formId || 0}" :service-id="${serviceId || 0}" :version-id="${versionId || 0}"`
                break
            case 'FORM_MANAGE':
            case 'FLOW_MANAGE':
