...
class PageCode {
    ...
    generateComponment (item, vueDirective, propDirective, inFreeLayout = false) {
        ...
        if (item.name && item.name.startsWith('chart-')) {
            ...
            const itemProps = this.getItemProps(item.componentType, item.renderProps, item.componentId, item.renderDirectives, item.renderSlots)
            ...
            if (i !== 'slots' && i !== 'class') {
                ...
                if (format !== 'value') {
                            ...
                            .reduce((acc, cur) => {
                                ...
                                if (cur === 'count') {
                                    ...
                                    if (item.buildInVariableType === 'CUSTOM') {
                                        this.handleUsedVariable('variable', item.customVariableCode, `组件【${compId}】的【${i}】属性`)
                                    ...
                                    this.handleUsedVariable(item.format, item.code, `组件【${compId}】的【${i}】属性`)
                    ...
                    if (method.funcName && method.funcCode) {
                        if (params?.length > 0) {
                            propsStr += `:${propName}="(args) => ${method.funcName}(${this.getFuncParamStr(params, `属性【${i}】`)}, ...args)"`
                        } else {
    ...
    getItemStyles (compId, renderStyles, props, cssNamePrefix = '', componentType) {
        // 分离class和其它style属性
        ...
        if (typeof events === 'object' && Object.keys(events).length) {
            for (const key in events) {
                ...
                if (fun.id && (event.enable === undefined || event.enable)) {
                    ...
                    if (params.length > 0) curEventStr = `@${key}="${fun.funcName}(${this.getFuncParamStr(params, `事件【${key}】`)}, ...arguments)" `
        ...
        switch (valType) {
            ...
            case 'variable':
                ...
                if (!variable && !(errMessagePerfix.indexOf('v-model') > 0 && val.indexOf('.') > 0)) {
                    this.codeErrMessage = `${errMessagePerfix}使用了不存在的变量【${val}】，请修改后重试`
                }
                if (variable) {
    ...
    getDirectives (renderDirectives, renderProps, componentId) {
        ...
        exisDirectives.forEach((directive) => {
            ...
            const disPlayVal = this.handleUsedVariable(format, val, `组件【${componentId}】的【${type}】指令`, dataSourceId, dataSourceType)
        ...
        slotKeys.forEach((key) => {
            ...
            if (Array.isArray(slot)) {
                ...
                do {
                    ...
                    const param = { val: disPlayVal, type: 'variable', payload: slot.payload, valueType: slot.valueType }
                    ...
                    if (slot.format === 'value' && slot.buildInVariableType === 'CUSTOM' && customVariableCode) {
                        ...
                        this.handleUsedVariable('variable', slot.val, `组件【${compId}】的【${key}】属性`)
                    }
  
                    if (slot.format !== 'value') {
                        disPlayVal = this.handleUsedVariable(slot.format, slot.val, `组件【${compId}】的【${key}】属性`)
                    } else if (slot.valueType === 'remote' && methodData.methodCode) {
    ...
    getFuncParamStr (params, errMessagePerfix, withThis) {
        return params
            ?.reduce((acc, cur) => {
                if (cur.format === 'value') {
    ...
    getLifeCycle () {
        ...
        lifeCycleKeys.forEach((key) => {
            ...
            if (method.id) {
                ...
                lifeCycleStrObj[key].push(`this.${method.funcName}(${this.getFuncParamStr(params, `页面的【${key}】生命周期`, true)})`)
            }
        })
        if (this.remoteDataStr) {
    ...
    getImportContent () {
        ...
        if (this.isUseSwiper) {
            importStr += `
                /**
                 * 请先安装 Swiper 相关依赖: npm install swiper@5.2.0 vue-awesome-swiper@4.1.1
                 */
            `
            importStr += `
    ...
    remoteMethodsTemplate (key, payload, chartType = false) {
        ...
        if (method.id) {
            if (chartType) {
                this.remoteDataStr += `const ${key}Remote = await this.${method.funcName}(${this.getFuncParamStr(params, `远程函数【${method.funcName}】`, true)})\nObject.assign(this.${key}, ${key}Remote)\n`
            } else {
                this.remoteDataStr += `this.${key} = await this.${method.funcName}(${this.getFuncParamStr(params, `远程函数【${method.funcName}】`, true)})\n`
            }
        }
        // 处理chartRemote
        if (method.funcCode) this.addUsedFunc(method.funcCode)
    ...
    generataNocodePage () {
        ...
        const { config = {}, fields = [], formIds = {}, serviceId = 0, tableName = '', formId = 0, versionId = 0, content = '' } = this.nocodePayload
        ...
        switch (this.nocodeType) {
            case 'MARKDOWN':
                // this.dataTemplate(`markdown${key}`, `'${content}'`)
                propsStr = `value="${content}" :editable="false" :subfield="false" :toolbars-flag="false" default-open="preview" style="height: 100%;width: 100%;z-index: 5"`
                break
            case 'FORM':
            ...
            case 'FLOW':
                this.dataTemplate(`field${key}`, JSON.stringify(fields))
                propsStr += ` view-type="${this.pageType}" :fields="field${key}" table-name="${tableName || ''}" :form-id="${formId || 0}" :service-id="${serviceId || 0}" :version-id="${versionId || 0}"`
                break
            case 'FORM_MANAGE':
            case 'FLOW_MANAGE':
