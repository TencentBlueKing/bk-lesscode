...
class PageCode {
    ...
    getItemStyles (compId, renderStyles, props, cssNamePrefix = '', componentType) {
        ...
        if (hasStyle) {
            ...
            for (const i in styles) {
                ...
                if (i === 'top' || i === 'left') {
                    tmpStr += `${i}: 0px;\n`
                } else if (i === 'height' && componentType === 'h5-page') {
                    tmpStr += `${i}: 100vh;\n`
                } else {
    ...
    handleUsedVariable (valType, val, errMessagePerfix) {
        let disPlayVal = val
        switch (valType) {
            case 'value':
    ...
    getDirectives (renderDirectives, renderProps, componentId) {
        ...
        exisDirectives.forEach((directive) => {
            ...
            const disPlayVal = this.handleUsedVariable(format, val, `组件【${componentId}】的【${type}】指令`)
    ...
    generataNocodePage () {
        ...
        const { config = {}, fields = [], formIds = {}, serviceId = 0, tableName = '', formId = 0, versionId = 0 } = this.nocodePayload
