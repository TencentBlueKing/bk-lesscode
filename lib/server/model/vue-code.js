...
const { ansiparse, uuid } = require('../util')
...
module.exports = {
    async formatCode (code) {
        try {
            const uid = uuid()
            const tmpFilePath = path.join(__dirname, uid + '_' + ESLINT_TMP_FILE_PATH)
            ...
            await createFile(tmpFilePath)
            const report = cli.executeOnText(formatCode, tmpFilePath)
            CLIEngine.outputFixes(report)

            const ret = await read(tmpFilePath)
