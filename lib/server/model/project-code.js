...
import { uuid, unique, sm4Decrypt } from '../util'
...
const projectCode = {
    ...
    async previewCode (projectId, versionId, platform = 'PC', pageId = '') {
        ...
        // 兼容数据库中没有设置框架的情况
        const framework = project.framework || 'vue2'
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const [
                    ...
                    LCDataService.findOne(TABLE_FILE_NAME.PROJECT_BKFLOW_SPACE, { projectId })
                ])
                ...
                for (const layout of layoutIns) {
                    ...
                    // 生成父路由view文件
                    const navMethodStrList = await this.writeViewCode({
                        currentFilePath,
                        pathName,
                        targetData: [],
                        funcGroups,
                    ...
                    // 父路由用到的函数写到mixins
                    usedMethodList = [...usedMethodList, ...navMethodStrList]
                    // layout.path需要去掉开头的/
                    routerStr += `{
                ...
                // 生成移动端页面路由信息到工程
                const mobileRouteList = pageRouteList.filter(item => item.pageType === 'MOBILE').map(({ id, layoutId, path, layoutPath, ...others }) => ({
                    ...others,
                    routePath: layoutPath + path
                }))
                if (mobileRouteList.length) {
                    const pagePath = path.join(targetPath, 'lib/shared/page-route/info.json')
                    await fse.ensureFile(pagePath)
                    fs.writeFileSync(pagePath, JSON.stringify(mobileRouteList), 'utf8')
                }
                ...
                // 生成 flowTplListMap 文件
                const flowTplListMapPath = path.join(targetPath, 'lib/shared/flow-tpl/index.js')
                await this.writeFlowTplListMap(flowTplListMapPath, flowTplList)
                ...
                if (hasMobilePage) {
                    if (framework === 'vue3') {
                        ...
                        mainFileContent = mainFileContent.replace(/\$\{importLuckyCanvas\}/, 'import bkLuckyCanvas from \'@/components/bk-lucky-canvas/index.vue\'')
                        ...
                        mainFileContent = mainFileContent.replace(/\$\{useLuckyCanvas\}/, 'global.mainComponent.component(\'bk-lucky-canvas\', bkLuckyCanvas)')
                        ...
                        mainFileContent = mainFileContent.replace(/\$\{importLuckyCanvas\}/, 'import bkLuckyCanvas from \'@/components/bk-lucky-canvas/index.vue\'')
                        mainFileContent = mainFileContent.replace(/\$\{remJs\}/, remJS)
                        mainFileContent = mainFileContent.replace(/\$\{useLuckyCanvas\}/, 'Vue.component(\'bk-lucky-canvas\', bkLuckyCanvas)')
                        await this.writePackageJSON(
                            path.join(targetPath, 'package.json'),
                            [
                    ...
                    mainFileContent = mainFileContent.replace(/\$\{importLuckyCanvas\}/, '')
                    mainFileContent = mainFileContent.replace(/\$\{useLuckyCanvas\}/, '')
                ...
                // 有数据表的也要生成网关，因为有可能需要直接被bkvision作为数据源引用
                const isGenerateApigw = flowList.length > 0 || flowTplList.length > 0 || hasBkvision || dataTables.length
                await this.generateDataSource(dataTables, dataTableModifyRecords, targetPath, isGenerateApigw, STATIC_URL, thirdPartDBs)
                ...
                await this.generateFileByReplace(
                    path.join(targetPath, 'lib/server/util/flow/index.js'),
                    path.join(targetPath, 'lib/server/util/flow/index.js'),
                    (content) => {
                        const user = RequestContext.getCurrentUser()
                        return content.replace(/\$\{apiName\}/, `lesscode-${projectId}${projectInfo.projectCode}`.slice(0, 30)).replace(/\$\{user\}/, user.username)
                ...
                await this.generateFileByReplace(
                    path.join(targetPath, 'lib/server/controller/flow.js'),
                    path.join(targetPath, 'lib/server/controller/flow.js'),
                    (content) => {
                        return content.replace(/\$\{spaceId\}/, spaceData.bkFlowSpaceId || 0)
                    }
                )
                await this.generateFileByReplace(
                ...
                await this.generateFileByReplace(
                    ...
                    path.join(targetPath, 'lib/shared/data-source/constant.js'),
                    (content) => {
                        return content.replace(/\$\{tableList\}/, JSON.stringify(dataTables.filter(dataTable => dataTable.thirdPartDBId <= 0)))
                    }
                )
    ...
    async generateDataSource (dataTables = [], dataTableModifyRecords = [], targetPath = '', isGenerateApigw = false, staticUrl, thirdPartDBs) {
        const mysqlDataTables = dataTables.filter(dataTable => dataTable.thirdPartDBId <= 0)
        const thirdPartDBDataTables = dataTables.filter(dataTable => dataTable.thirdPartDBId > 0)
        const hasDataTable = mysqlDataTables.length > 0
        ...
        await this.generateFileByReplace(appPath, appPath, (content) => {
            ...
            const startStr = hasDataTable ? 'createConnection(dataBaseConf).then((connection) => {\r\n    startServer()\r\n    return Promise.all([connection.manager.query(\'SET GLOBAL time_zone = \\\'+00:00\\\';\'), connection.manager.query(\'SET time_zone = \\\'+00:00\\\';\')])\r\n}).catch((err) => logger.error(err.message || err))\r\n' : 'startServer()'
            return content
        ...
        if (hasDataTable) {
            ...
            for (const dataTable of mysqlDataTables) {
        ...
        const mysqlDataTableModifyRecords = dataTableModifyRecords.filter(dataTableModifyRecord => {
            const dataTable = dataTables.find(dataTable => +dataTable.id === +dataTableModifyRecord.tableId)
            return dataTable?.thirdPartDBId <= 0
        })
        if (mysqlDataTableModifyRecords.length) {
            // generate migrations
            ...
            for (const record of mysqlDataTableModifyRecords) {
        ...
        if (thirdPartDBs.length) {
            const thirdPartDBPath = path.join(targetPath, 'lib/server/conf/third-part-db.js')
            await this.generateFileByReplace(thirdPartDBPath, thirdPartDBPath, (content) => {
                const dbINfo = thirdPartDBs.reduce((acc, cur) => {
                    const tableList = thirdPartDBDataTables.filter(thirdPartDBDataTable => +thirdPartDBDataTable.thirdPartDBId === +cur.id)
                    acc[cur.dbName] = {
                        host: cur.host,
                        port: cur.port,
                        dbName: cur.dbName,
                        username: sm4Decrypt(cur.username),
                        password: sm4Decrypt(cur.password),
                        tableList
                    }
                    return acc
                }, {})
                return content
                    .replace(/\$\{thirdPartDB\}/, JSON.stringify(dbINfo))
            })
        } else {
            const thirdPartDBPath = path.join(targetPath, 'lib/server/conf/third-part-db.js')
            await this.generateFileByReplace(thirdPartDBPath, thirdPartDBPath, (content) => {
                return content.replace(/\$\{thirdPartDB\}/, '\'\'')
            })
        }
    },
    ...
    /**
     * Write a list of items to a js file as a map.
     * @param {string} mapPath - The file path of the js file.
     * @param {Array} list - The list of items.
     * @param {string} mapName - The variable name of the map.
     * @param {function} formatItem - A function that formats each item in the list to a string.
     * @returns {Promise<void>}
     */
    async writeMapToFile (mapPath, list = [], mapName, formatItem) {
        return new Promise(async (resolve, reject) => {
            let mapStr = `export const ${mapName} = {\n`
            const arr = list.map(formatItem)
            mapStr += arr.join(',\n')
            mapStr += '}'
            try {
                const [message, fileStr] = await VueCodeModel.formatJsByEslint(mapStr)
                if (message) throw new Error(message)
                await fse.ensureFile(mapPath)
                fs.writeFileSync(mapPath, fileStr, 'utf8')
                resolve()
            } catch (err) {
                reject(err)
            }
    ...
    /**
     * Write flowTplList to a js file as a map.
     * @param {string} flowTplListMapPath - The file path of the js file.
     * @param {Array<{id: string, nodes: Array<string>, name: string}>} flowTplList - The flowTplList.
     * @returns {Promise<void>}
     */
    async writeFlowTplListMap (flowTplListMapPath, flowTplList = []) {
        const formatNodeItem = ({ id, nodes, edges, notifyConfig, name }) => {
            return `'${id}': {
                'nodes': ${nodes},
                'edges': ${edges},
        ...
        return this.writeMapToFile(flowTplListMapPath, flowTplList, 'flowTplListMap', formatNodeItem)
    },
    
    /**
     * Write formList to a js file as a map.
     * @param {string} formMapPath - The file path of the js file.
     * @param {Array<{id: string, content: string, tableName: string}>} formList - The form list.
     * @returns {Promise<void>}
     */
    async writeFormMap (formMapPath, formList = []) {
        const formatFormItem = ({ id, content, tableName }) => {
            return `'${id}': {
                'tableName': '${tableName}',
                'content': ${content}
            }`
        }
        return this.writeMapToFile(formMapPath, formList, 'formMap', formatFormItem)
    },
    ...
    writePackageJSON (packageJsonFilePath, deps, projectId = 0) {
        return new Promise(async (resolve, reject) => {
            const { list } = await LCDataService.get({
                tableFileName: TABLE_FILE_NAME.API_DOMAINS
            })
            const whiteList = list.map(item => item.domain)
            fse.ensureFile(packageJsonFilePath).then(async () => {
                ...
                if (projectId > 0) {
                    ...
                    Object.assign(ret.betterScripts.dev.env, {
                        ...
                        BK_API_DOMAIN_WHITE_LIST: whiteList || []
                    })
