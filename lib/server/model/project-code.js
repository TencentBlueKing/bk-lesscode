...
const projectCode = {
    ...
    async generateCode (projectId, versionId, pathName) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // 生成 nodeListMap 文件
                const nodeListMapPath = path.join(targetPath, 'lib/shared/node-list/index.js')
                await this.writeNodeListMap(nodeListMapPath, nodeList?.list || [])
    ...
    /**
     * Write nodeList to a js file as a map.
     * @param {string} nodeListMapPath - The file path of the js file.
     * @param {Array<{id: string, nodes: Array<string>, name: string}>} nodeList - The nodeList.
     * @returns {Promise<void>}
     */
    async writeNodeListMap (nodeListMapPath, nodeList = []) {
        const formatNodeItem = ({ id, nodes, name }) => {
            return `'${id}': {
                'nodes': ${nodes},
                'name': '${name}'
            }`
        }
        return this.writeMapToFile(nodeListMapPath, nodeList, 'nodeListMap', formatNodeItem)
    },
