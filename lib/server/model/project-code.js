...
const projectCode = {
            ...
            try {
                ...
                await this.generateFileByReplace(
                    ...
                    (content) => {
                        ...
                        return content.replace(/\$\{apiName\}/, `bk-lesscode-${projectInfo.projectCode}-${projectId}`).replace(/\$\{user\}/, user.username)
                    }
                )
                resolve('success')
            } catch (err) {
    ...
    async generateDataSource (dataTables = [], dataTableModifyRecords = [], targetPath, flowList) {
        ...
        await this.generateFileByReplace(appPath, appPath, (content) => {
            ...
            return content
                ...
                .replace(
                    /\$\{gwImport\}/,
                    flowList.length ? 'import { grantApiGWPermissionForItsm } from \'../server/form\'' : ''
                )
                .replace(
    ...
    writeViewCode ({ currentFilePath, pathName, pageCode, targetData, funcGroups, lifeCycle, projectId, pageId, layoutContent, isGenerateNav, layoutType, variableData, styleSetting, apiList, nocodeType = '', nocodePayload }) {
        return new Promise(async (resolve, reject) => {
    ...
    getNocodePayload (route = {}) {
        const nocodePayload = {}
        if (route.nocodeType) {
            if (['FORM', 'FLOW'].includes(route.nocodeType)) {
                Object.assign(nocodePayload, { formId: route.formId, tableName: route.tableName, fields: JSON.parse(route.formContent || '[]'), serviceId: route.flowServiceId })
                ...
                Object.assign(nocodePayload, { tableName: route.tableName, config: JSON.parse(route.content || '{}'), formIds: JSON.parse(route.flowFormIds || '{}'), serviceId: route.flowServiceId })
            }
        }
