...
const flowConf = require('../conf/flow')
// npm.js配置文件不存在时赋值空对象
let npmConf
try {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const [
                    ...
                    { list: thirdPartDBs = [] },
                    { list: flowTplList = [] }
                    // spaceData = {} @todo 待bkflow支持传非当前应用app_code创建空间 20241023
                ] = await Promise.all([
                    routeModel.findProjectRoute(projectId, versionId),
                    routeModel.queryProjectPageRoute(projectId, versionId),
                    getAllGroupAndFunction(projectId, versionId),
                    ...
                    // LCDataService.findOne(TABLE_FILE_NAME.PROJECT_BKFLOW_SPACE, { projectId }) // @todo 待bkflow支持传非当前应用app_code创建空间 20241023
                ])
                ...
                const isGenerateApigw = flowList.length > 0 || flowTplList.length > 0 || hasBkvision
                ...
                await this.generateFileByReplace(
                    ...
                    (content) => {
                        const user = RequestContext.getCurrentUser()
                        return content.replace(/\$\{apiName\}/, `lesscode-${projectId}${projectInfo.projectCode}`.slice(0, 30)).replace(/\$\{user\}/, user.username).replace(/\$\{stageName\}/, httpConf.stageName)
                    }
                )
                await this.generateFileByReplace(
                    ...
                    path.join(targetPath, 'lib/server/controller/flow.js'),
                    (content) => {
                        return content.replace(/\$\{bk_app_code\}/, flowConf.APP_CODE).replace(/\$\{bk_app_secret\}/, flowConf.APP_SECRET).replace(/\$\{spaceId\}/, 141) // @todo 待bkflow支持传非当前应用app_code创建空间 20241023
                    }
                )
                await this.generateFileByReplace(
