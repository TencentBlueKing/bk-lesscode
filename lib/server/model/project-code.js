    ...
    getNocodePayload (route = {}) {
        ...
        if (route.nocodeType) {
            if (['FORM', 'FLOW'].includes(route.nocodeType)) {
                Object.assign(nocodePayload, { fields: JSON.parse(route.formContent || '[]') })
            } else if (route.nocodeType === 'FORM_MANAGE') {
                Object.assign(nocodePayload, { config: JSON.parse(route.content || '{}'), formIds: route.formId })
            } else if (route.nocodeType === 'FLOW_MANAGE') {
                Object.assign(nocodePayload, { config: JSON.parse(route.content || '{}'), formIds: JSON.parse(route.flowFormIds || '{}'), serviceId: route.flowServiceId })
            }
        }
