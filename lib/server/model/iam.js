...
export default {
    ...
    async queryProjectByCreatorAndProjectId (username, id) {
        const result = await getRepository(Project).createQueryBuilder('project')
            .where('project.deleteFlag != 1 AND project.id = :id', { id })
            .getOne()
    ...
    async queryAllProjectByCreator ({ condition = '', params = {}, select }) {
