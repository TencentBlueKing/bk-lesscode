...
module.exports = {
    ...
    getLatestInfo (projectId, env = ['prod', 'stag'], isOffline = [0, 1]) {
        return getRepository(ReleaseVersion).createQueryBuilder('release_version')
            .where('projectId = :projectId', { projectId })
            .andWhere('deleteFlag = 0')
            .andWhere('env IN (:...env)', { env })
            .andWhere('isOffline IN (:...isOffline)', { isOffline })
            .orderBy('id', 'DESC')
            .limit('1')
            .getMany()
    },

    // 获取最近一次version = project_verrsion的部署记录
    getLatestNewVersionInfo (projectId) {
        return getRepository(ReleaseVersion).createQueryBuilder('release_version')
            .where('projectId = :projectId', { projectId })
            .andWhere('releaseType = "PROJECT_VERSION"')
            .orderBy('id', 'DESC')
            .limit('1')
            .getMany()
    },
    ...
    async createVersionFromHistory (versionData) {
        ...
        const newVersionData = {
            ...versionData,
            releaseMd5: preVersion.releaseMd5,
