...
export default {
            ...
            if (projectData.copyFrom) {
                ...
                const [
                    ...
                    getRepository(ApiCategory)
                        ...
                        .where('apiCategory.projectId = :projectId', { projectId: projectData.copyFrom })
                        .getMany(),
                    getRepository(Form)
                        .createQueryBuilder('form')
                        .where('form.projectId = :projectId', { projectId: projectData.copyFrom })
                        .getMany()
                ])
                ...
                if (projectDataTables.length) {
                    ...
                    // 复制form表，并变更dataSourceId和page表的formId
                ...
                if (apiCategoryList.length) {
                    ...
                    const saveCopyApiList = getRepository(Api).create(copyApiList.map(item => {
                        const { id, createTime, updateTime, ...others } = item
                        others.categoryId = apiCategoryIdList[others.categoryId]
                        others.projectId = projectId
                        return others
                    }))
                    ...
                    const copyFuncApiList = await getRepository(FuncApi)
                        ...
                        .where('funcApi.projectId = :projectId', { projectId: projectData.copyFrom })
                        .getMany()
    ...
    queryAllProjectForPerms () {
        return getRepository(Project)
            .createQueryBuilder('project')
            .select(projectSelectFields)
            .where('project.deleteFlag != 1 AND project.isOffcial != 1')
            .orderBy('project.id', 'DESC')
            .getMany()
    }
}
