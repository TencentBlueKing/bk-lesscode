        ...
        return getConnection().transaction(async transactionalEntityManager => {
            ...
            if (projectData.copyFrom) {
                ...
                if (projectPageCopyValues.length) {
                    // 复制应用时不复制流程相关页面
                    let pageIdList = projectPageCopyValues.map(item => item.pageId)
                    let copyPages = await getRepository(Page)
                        .createQueryBuilder('page')
                        .where('page.id IN (:...ids)', { ids: pageIdList })
                        .getMany()
                    copyPages = copyPages.filter(item => item.nocodeType !== 'FLOW' && item.nocodeType !== 'FLOW_MANAGE')
                    // 并更新pageIdList和projectPageCopyValues
                    pageIdList = copyPages.map(item => item.id)
                    const newProjectPageCopyValues = projectPageCopyValues.filter(item => pageIdList.includes(item.pageId))
                    ...
                    const projectPageValues = getRepository(ProjectPage).create(newProjectPageCopyValues.map((item, index) => {
                        const { id, createTime, updateTime, ...others } = item
                        others.projectId = projectId
            ...
            if (projectId) {
                ...
                if (v3Config.APP_ID && v3Config.APP_SECRET && bkTicket) {
