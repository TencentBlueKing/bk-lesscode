...
export const getImportPageComp = async function () {
    const res = await getRepository(PageComp)
        ...
        // .select('pageComp.compType', 'compType')
        // .select('pageComp.compVersion', 'compVersion')
        .select('pageComp.*')
...
export const getProjectComp = async function (projectId, versionId) {
    ...
    if (res.length) {
        ...
        data = res.reduce((result, item) => {
            ...
            item.version = item.version || item.compVersion
            item.type = item.type || item.compType

            if (typeArr.indexOf(item.type) === -1 && item.version) {
