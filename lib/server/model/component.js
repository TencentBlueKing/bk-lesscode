...
export const all = async function (params = {}, condition = [], conParams = {}, orderKey = '') {
    orderKey = orderKey || 'createTime'
...
export const getDataByCompIds = async function (ids, compTypes = []) {
    ids = ids.filter(item => item > 0)
    let whereCon = ''
    if (ids?.length && compTypes?.length) {
        whereCon = '(comp.id IN (:...ids) OR comp.type IN (:...compTypes))'
    } else if (ids.length && !compTypes?.length) {
        whereCon = 'comp.id IN (:...ids)'
    } else if (!ids.length && compTypes?.length) {
        whereCon = 'comp.type IN (:...compTypes)'
    } else {
        return []
    }
    const res = await getRepository(Comp)
...
export const findById = async function (id) {
    const res = await getRepository(Comp).findOne({ where: { id } })
    return res
}
