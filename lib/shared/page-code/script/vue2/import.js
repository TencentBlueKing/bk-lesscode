...
export default function (code) {
   ...
   `
    ...
    if (code.isUseSwiper) {
        importStr += `
            /**
             *${sharedI18n().t('请先安装 Swiper 相关依赖')}: npm install swiper
             */
        `
        importStr += `
    ...
    if (code.isUseBkCharts) {
        importStr += `
             ...
             */
        `
        importStr += 'import bkCharts from \'@/components/patch/bkCharts\'\n'
    }

    // 使用了echarts
    if (code.isUseEcharts) {
        importStr += `/**
            * ${sharedI18n().t('请先安装 echarts 相关依赖')}: npm install echarts
            * ${sharedI18n().t('更多使用请参考')}：https://github.com/ecomfe/vue-echarts#usage
            */
            import eCharts from '@/components/patch/chart'
        `
    }
    ...
    if (!['preview', 'previewSingle'].includes(code.pageType) && code.usingCustomArr && code.usingCustomArr.length) {
        ...
        for (const i in code.usingCustomArr) {
            importStr += `import ${camelCase(forkUsingCustomArr[i], { transform: camelCaseTransformMerge })} from '${code.npmConf.scopename}/${code.usingCustomArr[i]}'\n`
        }
    }
