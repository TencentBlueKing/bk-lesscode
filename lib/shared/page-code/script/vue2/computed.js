...
export default function (code) {
    ...
    if ((['vueCode', 'projectCode'].includes(code.pageType) && code.hasLayout) || code.projectVariables.length || code.pageComputedVariables.length || code.isUseSwiper || code.isGenerateNav) {
        let computedCon = ''
        if (['vueCode', 'projectCode'].includes(code.pageType) && code.hasLayout && code.platform === 'PC') {
            computedCon += '...mapGetters([\'user\']),\n'
            computedCon += getLangComputedStr()
        }
        if (['preview'].includes(code.pageType) && code.isGenerateNav && code.platform === 'PC') {
