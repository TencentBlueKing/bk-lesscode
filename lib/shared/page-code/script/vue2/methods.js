...
export default function (code) {
    ...
    if (code.hasLayout || code.remoteDataStr || (code.usingFuncCodes.length && code.pageType !== 'projectCode') || code.eventActionList?.length > 0 || code.flowActionList?.length > 0) {
        let methodsCon = ''
        ...
        if (code.hasLayout && code.platform !== 'MOBILE') {
            ...
            // 切换语种
            methodsCon += getLangMethodStr(code.pageType)
        }
        ...
        // 操作函数事件
        if (code.flowActionList?.length > 0) {
            code.flowActionList.forEach((action) => {
                methodsCon += `
                    async ${action.name} () {
                        try {
                            ${action.code}
...
// 切换语种
function getLangMethodStr (pageType) {
    let changelgStr = 'changeLang'
    if (pageType === 'preview') {
        changelgStr = 'this.$changeLang'
    }
