 ...
 * @returns { String }
 */
export default function generateChart (code, item, { styles, vueDirective, propDirective, itemProps, css, alignStr = '' }) {
    ...
    let echartsConfig = `${itemProps}`
    // 兼容以前旧数据
    if (item.type === 'chart') {
        echartsConfig = `:options="${item.componentId}"`
    }
    let componentCode = ''
    if (css) {
        componentCode = `
            <div${alignStr}style="${css}" ${vueDirective}>
                <div style="${widthStr}${heightStr}${displayStr}">
                    <chart component-id="${item.componentId}" ${echartsConfig} ${propDirective} autoresize></chart>
                </div>
            </div>`
    } else {
        componentCode = `
...
export function generateEChartData (code, item) {
    ...
    item.type === 'chart' && code.dataTemplate(item.componentId, JSON.stringify(item.renderProps.options?.code))
