...
import { handleRenderStyles, getAlignStr, getFreeLayoutItemStyle, getInsertCommonStyle } from '../../../common/utils'
...
import generateTree from './special-type/bk-tree'
...
export default function generateComponment (code, item, vueDirective, propDirective, inFreeLayout = false) {
    ...
    getInsertCommonStyle(code, alignStr)
    if (inFreeLayout) {
    ...
    if (item.name && (item.name.startsWith('chart-') || item.name.startsWith('echarts-'))) {
        componentCode = generateChart(code, item, { styles, vueDirective, propDirective, itemProps, css, alignStr, itemEvents })
    ...
    } else if (item.type === 'bk-tree' && code.framework === 'vue3') {
        componentCode = generateTree(code, item, { vueDirective, propDirective, itemProps, itemClass, itemEvents, css, alignStr })
    } else if (item.name === 'bk-flow') {
        componentCode = generateFlow(code, item, { css, alignStr })
    } else if (selfClosingTags.includes(item.type)) {
        componentCode = generateSelfClosingTag(code, item, { vueDirective, propDirective, itemProps, itemClass, css, alignStr, itemEvents })
    } else {
