...
export default function generateComponment (code, item, vueDirective, propDirective, inFreeLayout = false) {
    ...
    if (item.name && item.name.startsWith('chart-')) {
        componentCode = generateChart(code, item, { styles, vueDirective, propDirective, css, alignStr, itemEvents })
    } else if (item.type === 'widget-md-editor') {
        componentCode = generateMd(code, item, { itemClass })
    } else if (item.type === 'widget-form') {
        componentCode = generateWidgetForm(code, item, { vueDirective, propDirective, itemProps, itemClass, css, alignStr, itemEvents })
        ...
        // 通用组件处理
            ...
            if (item.type === 'bk-tree' && code.framework === 'vue3') {
                componentCode = generateTree(code, item, { vueDirective, propDirective, itemProps, itemClass, itemEvents, slotStr, css, alignStr })
            } else {
