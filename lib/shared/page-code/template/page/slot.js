...
import { getRenderMap, vue2RenderList, vue3RenderList } from '../../../../client/src/element-materials/modifier/component/slots/render-config'
...
export default function renderSlot (code, type, compId, slots) {
    ...
    slotKeys.forEach((key) => {
        ...
        if (Array.isArray(slot)) {
            ...
            const framework = code.framework === 'vue3' ? 'vue3' : 'vue2'
            let render = () => {}
            // 校验对应的方法存在再赋值
            if (vue2RenderList.includes(slot.name) || vue3RenderList.includes(slot.name)) {
                render = getRenderMap(framework)[slot.name] || (() => {})
            }
            ...
            do {
                ...
                if (slot.format === 'value' && slot.buildInVariableType === 'CUSTOM' && customVariableCode) {
                    ...
                    code.handleUsedVariable('variable', slot.val, `${sharedI18n().t('组件')}【${compId}】【${key}】${sharedI18n().t('属性')}`)
                }

                if (slot.format !== 'value') {
                    disPlayVal = code.handleUsedVariable(slot.format, slot.val, `${sharedI18n().t('组件')}【${compId}】【${key}】${sharedI18n().t('属性')}`)
