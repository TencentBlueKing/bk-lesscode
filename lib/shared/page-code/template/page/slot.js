...
export default function renderSlot (code, type, compId, slots) {
    ...
    slotKeys.forEach((key) => {
        ...
        if (Array.isArray(slot)) {
            ...
            const render = getRenderMap(framework)[slot.name] || (() => {})
            ...
            do {
                ...
                if (slot.format === 'value' && slot.buildInVariableType === 'CUSTOM' && customVariableCode) {
                    ...
                    code.handleUsedVariable('variable', slot.val, `${sharedI18n().t('组件')}【${compId}】${sharedI18n().t('的')}【${key}】${sharedI18n().t('属性')}`)
                }

                if (slot.format !== 'value') {
                    disPlayVal = code.handleUsedVariable(slot.format, slot.val, `${sharedI18n().t('组件')}【${compId}】${sharedI18n().t('的')}【${key}】${sharedI18n().t('属性')}`)
