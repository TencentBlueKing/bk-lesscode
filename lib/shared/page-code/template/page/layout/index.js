import { getAlignStr, getInsertCommonStyle } from '../../../common/utils'
...
export default function generateLayout (code, items = [], inFreeLayout = false) {
    ...
    for (let i = 0; i < len; i++) {
            ...
            templateCode += `
                <div${alignStr}class="bk-layout-row-${code.uniqueKey} ${item.componentId}" ${vueDirective} ${propDirective}>
                    ${item.renderSlots && item.renderSlots.default && item.renderSlots.default.map(col => {
                        getItemStyles(code, col.componentId, col.renderStyles, col.renderProps, { cssNamePrefix: `.bk-layout-col-${code.uniqueKey}` })
                        const colAlignStr = getAlignStr(col.renderAlign || {}, inFreeLayout)
                        getInsertCommonStyle(code, colAlignStr)
                        const {
        ...
        // 自由布局
            ...
            getItemStyles(code, item.componentId, item.renderStyles, item.renderProps, { componentType: item.type, cssNamePrefix: `.bk-free-layout-${code.uniqueKey}` })
            templateCode += `
