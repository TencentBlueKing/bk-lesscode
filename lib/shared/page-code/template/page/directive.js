...
export default function (code, renderDirectives, componentId) {
    ...
    exisDirectives.forEach((directive) => {
        const { type, modifiers = [], prop = '', format, code: val, dataSourceType, thirdPartDBName } = directive
        ...
        const disPlayVal = code.handleUsedVariable(format, val, `${sharedI18n().t('组件')}【${componentId}【${type}】${sharedI18n().t('指令')}`, dataSourceId, dataSourceType, thirdPartDBName)
        ...
        switch (type) {
            ...
            case 'v-for':
                ...
                break
            case 'v-model':
                const directiveProp = code.framework === 'vue3' ? `${type}:${prop}="${disPlayVal}"` : `${type}="${disPlayVal}"`
                propDirectives.push(directiveProp)
                break
            case 'v-show':
            case 'v-html':
