...
import generateInternationMenu from '../page/layout/i18-layout'
/**
 * @desc pc端顶部导航布局
        ...
        return `
            <bk-navigation ${componentProps} navigation-type="top-bottom" :need-menu="false" class="bk-layout-custom-component-wrapper">
                <template #side-header>
                    <span class="title-icon" @click="toRouterName('${layoutContent.logoLink}')">
                        <img src="${layoutContent.logo}" style="width: 28px; height: 28px;">
                    </span>
                    <span @click="toRouterName('${layoutContent.logoLink}')" class="title-desc">${layoutContent.siteName}</span>
                </template>
                <template v-slot:header>
                    <div class="navigation-header">
                        ...
                        <div class="header-right-area">
                            ...
                            ${generateInternationMenu(code, isWhiteTheme, '#content')}
    ...
    return `
        <bk-navigation head-theme-color=${topMenuBackground} ${componentProps} navigation-type="top-bottom" :need-menu="false" class="bk-layout-custom-component-wrapper" :class="{ 'white-navigation': ${isWhiteTheme} }">
            <template slot="side-header">
                ...
                <span class="title-desc" @click="toRouterName('${layoutContent.logoLink}')" :class="{ 'theme-style': ${!isDefaultTheme} }">${layoutContent.siteName}</span>
            </template>
            <div class="navigation-header" slot="header">
                ...
                <div class="header-right-area">
                    ...
                    ${generateInternationMenu(code, isWhiteTheme, 'slot="content"')}
        ...
        return `
            <bk-navigation ${componentProps} navigation-type="left-right" need-menu class="bk-layout-custom-component-wrapper" @toggle="v => toggleActive=v" :class="{ 'white-theme': ${isWhiteTheme}, 'other-theme': ${isOtherTheme} }">
                <template #side-header>
                    ...
                    <span @click="toRouterName('${layoutContent.logoLink}')" class="title-desc" :class="{ 'white-theme-title': ${isWhiteTheme} }">${layoutContent.siteName}</span>
                </template>
                <template #header>
                    <div class="navigation-header">
    ...
    return `
        <bk-navigation ${componentProps} theme-color="${sideMenuBackground}" navigation-type="left-right" need-menu class="bk-layout-custom-component-wrapper" @toggle="v => toggleActive=v" :class="{ 'white-theme': ${isWhiteTheme}, 'other-theme': ${isOtherTheme} }">
            <template slot="side-header">
                ...
                <span class="title-desc" @click="toRouterName('${layoutContent.logoLink}')" :class="{ 'white-theme-title': ${isWhiteTheme} }">${layoutContent.siteName}</span>
            </template>
            <div class="navigation-header" slot="header">
                <div class="header-title">
                    {{curNav.name}}
...
export function getComplexLayout (code, pageContent, componentProps) {
    ...
    const { layoutContent } = code
    const sideCustomConList = layoutContent.topMenuList.filter(item => item?.sideCustomCon?.renderSlots?.default?.length)
    const sideCustomConCode = sideCustomConList.map(menu => {
        if (menu.sideCustomCon?.type === 'render-block') {
            const conArr = []
            conArr.push(menu.sideCustomCon)
            return `<div v-if="curNav.id === '${menu.id}'">
                ${generateLayout(code, conArr)}
            </div>`
        } else {
            return ''
        }
    })
    const sideCustomConCodeStr = sideCustomConCode.join('')
    const topMenuBackground = layoutContent.themeConfig?.topMenuBackground || '#182132'
        ...
        return `
            <bk-navigation ${componentProps} navigation-type="top-bottom" :need-menu="${curLeftMenuKey}.length > 0" class="bk-layout-custom-component-wrapper" @toggle="v => toggleActive=v">
                <template #side-header>
                    ...
                    <span  @click="toRouterName('${layoutContent.logoLink}')" class="title-desc" :class="{ 'theme-style': ${!isDefaultTopTheme} }">${layoutContent.siteName}</span>
                </template>
                <template #header>
                    <div class="navigation-header">
                        <ul class="header-nav">
                        ...
                        <div class="header-right-area">
                            ...
                            ${generateInternationMenu(code, isWhiteTopTheme, '#content')}
    ...
    return `
        <bk-navigation ${componentProps} head-theme-color=${topMenuBackground} navigation-type="top-bottom" :need-menu="${curLeftMenuKey}.length > 0" class="bk-layout-custom-component-wrapper" @toggle="v => toggleActive=v" theme-color="${sideThemeColor}" :class="{ 'white-navigation': ${isWhiteTopTheme} }">
            <template slot="side-header">
                <span @click="toRouterName('${layoutContent.logoLink}')" class="title-icon">
                    <img src="${layoutContent.logo}" style="width: 28px; height: 28px;">
                </span>
                <span  @click="toRouterName('${layoutContent.logoLink}')" class="title-desc" :class="{ 'theme-style': ${!isDefaultTopTheme} }">${layoutContent.siteName}</span>
            </template>
            <div class="navigation-header" slot="header">
                ...
                <div class="header-right-area">
                    ...
                    ${generateInternationMenu(code, isWhiteTopTheme, 'slot="content"')}
            ...
            <template slot="menu">
                ${sideCustomConCodeStr}
                <bk-navigation-menu :default-active="curNav.id" :toggle-active="toggleActive" ${themeColorProps} :class="{ 'white-theme-menu': ${isWhiteTopTheme}}">
                    <bk-navigation-menu-item
                        @click="goToPage(child)"
                        :key="child.id"
