...
import { unitFilter, removeIllegalWords, sandboxRun } from '../../util'
...
export function getValueType (val) {
    ...
    try {
        type = sandboxRun(`typeof ${val}`)
    } catch (error) {
        console.log(error, 'run type error')
    }
    return type
}
...
/**
 * @desc 判断是否插入公共样式
 * @param { CodeModel } code 对齐相关的属性配置
 * @param { string } alignStr 公共样式前缀
 * @returns
 */
export function getInsertCommonStyle (code, alignStr) {
    const val = !code.isStaticCommonStyle && alignStr?.trim() !== '' ? true : code.isStaticCommonStyle
    code.setProperty('isStaticCommonStyle', val)
}
