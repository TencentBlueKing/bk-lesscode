...
export function remoteMethodsTemplate (code, key, payload, chartType = false) {
    ...
    if (method.id) {
        if (chartType) {
            code.remoteDataStr += key.endsWith('options')
            ...
            code.remoteDataStr += `this.${key} = await this.${method.funcName}(${getFuncParamStr(code, method, params, `远程函数【${method.funcName}】`, true)})\n`
        }
    }
    // 处理chartRemote
...
export function dataSourceTemplate (code, key, sourceData) {
    const hashId = uuid(4)
    code.remoteDataStr += `const ${key}Source${hashId} = await this.$http.get('/data-source/user/tableName/${sourceData.tableName}?bkDataSourceType=${sourceData.dataSourceType}')\nthis.${key} = ${key}Source${hashId}.data.list || []\n`
}
