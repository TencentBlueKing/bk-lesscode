...
import { FORBIDDEN_KEYS } from 'shared/constant'
...
export function dataSourceTemplate (code, key, sourceData) {
    const hashId = uuid(4)
    let newValue = code.remoteDataStr
    newValue += `const ${key}Source${hashId} = await this.$http.get('/data-source/user/tableName/${sourceData.tableName}${sourceData.thirdPartDBName ? `/${sourceData.thirdPartDBName}` : ''}?bkDataSourceType=${sourceData.dataSourceType}')\nthis.${key} = ${key}Source${hashId}.data.list || []\n`
    setProperty(code, 'remoteDataStr', newValue)
}
...
export function addFlowActionTemplate (pageCode, eventHandlerName, config) {
    if (config.actionName === 'createAndExecuteTask') {
        addUsedFunc(pageCode, eventHandlerName)
        pageCode.flowActionList.push({
            name: eventHandlerName,
            code: `
                const res = await this.$store.dispatch('createAndExecuteTask', ${config.id})
                if (res.code === 0) {
                    this.$bkMessage({
                        theme: 'success',
                        message: '流程【' + res.data.tplName + '】执行任务创建成功'
                    })
                } else {
                    this.$bkMessage({
                        theme: 'error',
...
export function getFuncParamStr (code, method, params, errMessagePerfix, withThis, defaultFormat = 'value') {
    ...
    return displayParams
        ?.reduce((acc, cur, index) => {
            // 兼容老数据中没有format、则视为format=value
            if (cur.format === 'value' || !cur.format) {
                acc.push(code.handleUsedVariable('value', cur.value, errMessagePerfix) || '\'\'')
 ...
 */
export function handleUsedVariable (code, valType, val, errMessagePerfix, dataSourceId, dataSourceType, thirdPartDBName) {
    let disPlayVal = val
    switch (valType) {
        case 'value':
        ...
        case 'dataSource':
            ...
            dataSourceTemplate(code, dataSourceId, { tableName: val, dataSourceType, thirdPartDBName })
            break
    }
    return disPlayVal
}
