...
export const parseQueryScheme2QueryString = (querySchemes) => {
...
// 编辑态 scheme 转换为值
export const parseScheme2Value = (scheme) => {
    let val = scheme.value
    if (API_PARAM_TYPES.OBJECT.VAL === scheme.type) {
        val = {}
        scheme.children.forEach((item) => {
            if (item.name) {
                val[item.name] = parseScheme2Value(item)
            }
        })
    }
    if (API_PARAM_TYPES.ARRAY.VAL === scheme.type) {
        ...
        scheme.children.forEach((item) => {
            if (item.name) {
                val.push(parseScheme2Value(item))
            }
        })
    }
    return val
