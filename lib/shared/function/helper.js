...
export const replaceFuncKeyword = (funcBody = '', callBack) => {
    ...
    return funcBody.replace(/lesscode((\[\'\$\{prop:([\S]+)\}\'\])|(\[\'\$\{func:([\S]+)\}\'\]))/g, (all, first, second, variableCode, funcStr, funcCode, index) => {
    ...
    return replaceFuncKeyword(funcBody, (all, first, second, variableCode, funcStr, funcCode) => {
        ...
        if (variableCode) {
            ...
            if (curVar) {
                ...
                throw new Error(`函数【${funcName}】里引用的变量【${variableCode}】不存在或非应用级变量，请检查`)
            }
        }
    })
}
...
const recordVariable = (variableCode, funcName) => {
    ...
    variableCodes.forEach((code) => {
        ...
        if (curVar) {
            ...
            throw new Error(`函数【${funcName}】里引用的变量【${code}】不存在或非应用级变量，请检查`)
        }
    })
}
