...
const getMethodStr = (returnMethod) => {
    ...
    if (returnMethod.funcType === 1) {
        ...
        returnMethod.funcStr = `const ${returnMethod.funcName} = (${funcParams}) => { ${validateString} return this.$store.dispatch('getApiData', ${data}).then((${remoteParams}) => { ${returnMethod.funcBody} }) };`
    } else {
