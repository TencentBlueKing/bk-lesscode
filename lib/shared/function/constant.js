...
export const FUNCTION_TIPS = () => ({
    [FUNCTION_TYPE.EMPTY]: '/**\r\n'
        ...
        + '* 6. ' + sharedI18n().t('调用其他远程函数示例如下：lesscode[\'${func:getApiData}\']().then((res) => do(res))\r\n')
        + '* 7. ' + sharedI18n().t('函数用于组件属性时，函数体代码示例如下：\r\n')
        + '* return Promise.all([\r\n'
    ...
    [FUNCTION_TYPE.REMOTE]: '/**\r\n'
        ...
        + '* 2. ' + sharedI18n().t('远程函数是异步函数，函数返回 promise。可以使用 then 回调在远程函数异步执行完执行下一个函数\r\n')
        + '* 3. ' + sharedI18n().t('调用其他远程函数示例如下：lesscode[\'${func:getApiData}\']().then((res) => do(res))\r\n')
        + '* 4. ' + sharedI18n().t('函数用于页面组件属性时：函数必须有返回值，该返回值将会赋值给组件属性\r\n')
        + '* 5. ' + sharedI18n().t('函数用于组件或页面事件时：函数将在事件触发时发起 Ajax 请求，并执行该回调函数\r\n')
        + '* 6. ' + sharedI18n().t('可以使用 “lesscode.变量标识” 唤起自动补全功能，必须通过编辑器自动补全功能选择对应变量，来获取或者修改变量值\r\n')
        + '* 7. ' + sharedI18n().t('可以使用 “lesscode.函数名称” 唤起自动补全功能，必须通过编辑器自动补全功能选择对应函数，来调用应用中的其它函数\r\n')
        + '* 8. ' + sharedI18n().t('例如 Api 返回数据使用参数 res 接收，则代码示例如下：return res.data\r\n')
        + '*/\r\n'
})
