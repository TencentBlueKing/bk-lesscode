...
export const transApiToWebhook = (api, projectId) => {
    ...
    config.headers = [{ checked: true, key: 'x-project-id', value: projectId, select: true }]
    const queryData = parseQuerySchemaToWebhookVal(query)
    const bodyData = parseBodySchemaToWebhookVal(body)
    const data = METHODS_WITHOUT_DATA.includes(method) ? queryData : bodyData
    config.body.content = JSON.stringify({ url, method, data, username: '{{creatorUsername}}' })
    return config
}
