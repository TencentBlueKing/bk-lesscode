...
const isValueInOption = (fields) => {
    // 无值的时候 默认为true（必填在提交的时候校验）
    return fields.value ? true : fields.choice.some(option => option.key === fields.value)
}
/**
 * nocode表单校验
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (hasValidateRulesType.includes(type) && regex !== 'EMPTY') {
            ...
            if (!timeValidateKeys.includes(regex)) {
                console.log('isPassValidate', isPassValidate(regex, value))
                console.log('!   isPassValidate', !isPassValidate(regex, value))
        ...
        } else if (dataSourceField.includes(fields[i].type)) { // value是否是在 choice 中的校验
            if (isValueInOption(fields[i])) {
                validateDataMsg = {
                    result: false,
                    errorMsg: ` 字段【${fields[i].name}】的值不在选项中。`
                }
                break
            }
        }
