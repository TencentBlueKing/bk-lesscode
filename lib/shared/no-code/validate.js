...
// 规则key - name映射
const ruleNameMap = () => ({
    'NUM': sharedI18n().t('数字0-9'),
    'NUMWITHOUTZERO': sharedI18n().t('非零正整数'),
    'FLOAT': sharedI18n().t('浮点数'),
    'NON_NEGATIVE': sharedI18n().t('非负数（0，正数，正浮点数）'),
    'NON_POSITIVE': sharedI18n().t('非正数（0，负数，负浮点数）'),
    'GTE_ZERO': sharedI18n().t('大于零的数（包括正数和正浮点数）'),
    'LTE_ZERO': sharedI18n().t('小于零的数（包括负数和负浮点数）'),
    'LOWER_EN': sharedI18n().t('仅小写字母'),
    'UPPER_EN': sharedI18n().t('仅大写字母'),
    'EN': sharedI18n().t('仅英文字符'),
    'EN_NUM': sharedI18n().t('仅能包含英文字符和数字'),
    'CH': sharedI18n().t('仅中文字符'),
    'EN_CH': sharedI18n().t('仅能包含中英文字符'),
    'EN_CH_NUM': sharedI18n().t('仅能包含中英文，数字，下划线'),
    'START_EN': sharedI18n().t('包含中英文，数字，以英文字符开头'),
    'EMAIL': sharedI18n().t('邮箱'),
    'PHONE_NUM': sharedI18n().t('内地手机号码'),
    'ID_CARD': sharedI18n().t('身份证'),
    'IP': sharedI18n().t('IP地址'),
    'QQ': sharedI18n().t('腾讯QQ号'),
    'AFTER_DATE': sharedI18n().t('系统日期之后'),
    'BEFORE_DATE': sharedI18n().t('系统日期之前'),
    'ONLY_NOW_DATE': sharedI18n().t('系统日期'),
    'AFTER_TIME': sharedI18n().t('系统时间之后'),
    'BEFORE_TIME': sharedI18n().t('系统时间之前')
})
// 数据源添加value是否是在 choice 中的校验
export const dataSourceField = ['SELECT', 'INPUTSELECT', 'MULTISELECT', 'CHECKBOX', 'RADIO']
// 规则key-正则映射
const ruleRegexMap = {
    'NUM': /^[0-9]$/g,
...
const returnValidataMsg = (regexKey) => {
    return {
        result: false,
        errorMsg: sharedI18n().t('校验规则') + ruleNameMap[regexKey] + sharedI18n().t(',校验失败')
    }
}
 ...
 */
export const validateData = (fields, data) => {
    let validateDataMsg = { result: true, errorMsg: '' }
    const len = fields.length
    for (let i = 0; i < len; i++) {
        const field = fields[i]
        const val = data.find(item => item.key === field.key)?.value || ''
        const { type, regex } = field
