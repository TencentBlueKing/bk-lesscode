...
export const generateSqlByCondition = (condition = {}, tableList) => {
    ...
    // 构造 sql
    let sql = generateTableSql(condition.table, fieldIdNameMap, hasMultTable)
        + generateWhereSql(condition.where, fieldIdNameMap, hasMultTable)
        + generateGroupBySql(condition.groupBy, fieldIdNameMap, hasMultTable)
        + generateOrderBySql(condition.orderBy, fieldIdNameMap, hasMultTable)
...
const getConnectionSql = (item, index, key, fieldIdNameMap, hasMultTable) => {
    let sql = `${getTableFieldSql(item.tableName, item.fieldId, fieldIdNameMap, hasMultTable)} ${getConnectionExpression(item)}`
    if (index > 0) {
        sql = `  ${item.type} ${sql}`
    } else {
        sql = `${key} ${sql}`
...
const generateWhereSql = (where, fieldIdNameMap, hasMultTable) => {
    if (!where || where.length <= 0) return ''

    let sql = ''
    where.forEach((whereItem, index) => {
        if (!isEmpty(whereItem.tableName) && !isEmpty(whereItem.fieldId)) {
            sql += `\r\n${getConnectionSql(whereItem, index, 'WHERE', fieldIdNameMap, hasMultTable)}`
        }
    })
    return sql
}

const generateGroupBySql = (groupBy, fieldIdNameMap, hasMultTable) => {
    if (!groupBy) return ''
        ...
        groupBy
            ...
            ?.forEach((item, index) => {
                if (!isEmpty(item.tableName) && !isEmpty(item.fieldId)) {
                    sql += `\r\n  ${getConnectionSql(item, index, 'HAVING', fieldIdNameMap, hasMultTable)}`
                }
            })
    }
    return sql
