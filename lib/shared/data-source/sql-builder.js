 ...
 * @returns params
 */
export const getSqlParam = (condition = {}) => {
    const params = {}
    // where
    condition?.where?.forEach((item) => {
        if (item.param) {
            params[item.param] = `\$\{${item.param}\}`
        }
    })
    // having
    condition?.groupBy?.having?.forEach((item) => {
        if (item.param) {
            params[item.param] = `\$\{${item.param}\}`
        }
    })
    // indexParam
    if (condition?.limit?.indexParam) {
        params[condition.limit.indexParam] = `\$\{${condition.limit.indexParam}\}`
    }
    // lengthParam
    if (condition?.limit?.lengthParam) {
        params[condition.limit.lengthParam] = `\$\{${condition.limit.lengthParam}\}`
    }
    return params
}
