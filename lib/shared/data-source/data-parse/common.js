...
function diffColumns (originColumns, finalColumns) {
    ...
    for (let index = 0; index < finalColumns.length; index++) {
        ...
        // 如果不一样，表示该行有修改或者是新增行
        if (ORM_KEYS.some(key => !['index', 'unique'].includes(key) && finalColumn[key] !== originColumn[key])) {
 ...
 * @param {*} finalColumns 修改后的字段
 */
function diffUnique (originColumns = [], finalColumns) {
    const getDifference = (a1, a2) => {
        return a1.filter((a) => (a.unique && !a2.some(b => (b.unique && a.name === b.name))))
    }
    const dropIndex = getDifference(originColumns, finalColumns).map(data => ({ type: UNIQUE_MODIFY_TYPE.DROP, data }))
    const addIndex = getDifference(finalColumns, originColumns).map(data => ({ type: UNIQUE_MODIFY_TYPE.ADD, data }))
    return [
        ...dropIndex,
        ...addIndex
    ]
...
export const diff = (originDatas, finalDatas) => {
    ...
    for (let index = 0; index < finalDatas.length; index++) {
        ...
        const uniqueData = diffUnique(originData.columns, finalData.columns)
        if (originData.tableName) {
            ...
            if (modifyColumns.length || indexData.length || uniqueData.length) {
                const data = {
                    ...finalData,
                    columns: modifyColumns
                }
                result.push({ type: TABLE_MODIFY_TYPE.MODIFY, data, index: indexData, unique: uniqueData })
            }
        } else {
            result.push({ type: TABLE_MODIFY_TYPE.CREATE, data: finalData, index: indexData, unique: uniqueData })
        }
    }
    // 删除的表
...
export const diffDatas = (originTableDatas, finalTableDatas) => {
    ...
    for (let index = 0, l = finalTableDatas.length; index < l; index++) {
        ...
        finalDatas.forEach((finalData) => {
            ...
            } else if (finalData._dataImportOperationType === DATA_IMPORT_OPERATION_TYPE().DE_DUPLICATION_INSERT.ID) {
                diffResult.push({ type: DATA_MODIFY_TYPE.DE_DUPLICATION_INSERT, data: finalData, tableName: finalTableData.tableName })
            } else if (finalData._dataImportOperationType === DATA_IMPORT_OPERATION_TYPE().UPDATE_INSERT.ID) {
                diffResult.push({ type: DATA_MODIFY_TYPE.UPDATE_INSERT, data: finalData, tableName: finalTableData.tableName })
            } else {
