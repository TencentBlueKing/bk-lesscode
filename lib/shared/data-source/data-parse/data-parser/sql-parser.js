        ...
        insertDefinitions.forEach((insertDefinition) => {
            ...
            insertDefinition.values.forEach(value => {
                ...
                if (value.value.length !== currentColumns.length) {
                    throw new Error(sharedI18n().t('字段值和字段个数不匹配：') + sqlParser.sqlify(insertDefinition))
                }
...
function transformJson2Sql (originDatas, finalDatas) {
    ...
    diffResult.forEach(({ type, data, tableName }) => {
        ...
        const updateInsertKeys = tableKeys.reduce((acc, cur) => {
        ...
        switch (type) {
            case DATA_MODIFY_TYPE.INSERT:
                ...
                break
            case DATA_MODIFY_TYPE.UPDATE_INSERT:
                sqlArr.push(`INSERT INTO \`${tableName}\`(\`${tableKeys.join('\`,\`')}\`) VALUES(${tableValues.join(',')}) ON DUPLICATE KEY UPDATE ${updateInsertKeys.join(',')};`)
                break
            case DATA_MODIFY_TYPE.DE_DUPLICATION_INSERT:
                sqlArr.push(`INSERT IGNORE INTO \`${tableName}\`(\`${tableKeys.join('\`,\`')}\`) VALUES(${tableValues.join(',')});`)
                break
