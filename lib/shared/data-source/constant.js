...
export const DATA_IMPORT_OPERATION_TYPE = {
    ALL_INSERT: {
        ID: 'ALL_NEW',
        NAME: '新增导入',
        TIPS: '1. 所有导入的数据以新增的方式插入数据库<br />2. 新增数据的唯一性约束字段（包括 id）不能和已有数据重复，否则新增失败'
    },
    UPDATE_INSERT: {
        ID: 'UPDATE_NEW',
        NAME: '更新导入',
        TIPS: '1. 导入的数据中，如果某条数据已经在数据库中存在则更新这条数据。如果不存在则新增这条数据<br />2. 通过数据中存在的唯一性约束字段（包括 id）判断是否在数据库中存在该条数据'
    },
    DE_DUPLICATION_INSERT: {
        ID: 'DE_DUPLICATION_NEW',
        NAME: '去重导入',
        TIPS: '1. 导入的数据中，如果某条数据已经在数据库中存在则忽略这条数据。如果不存在则新增这条数据<br />2. 通过数据中存在的唯一性约束字段（包括 id）判断是否在数据库中存在该条数据'
    }
}
