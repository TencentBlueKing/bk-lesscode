...
export const handleImportData = (files, fileType, columns) => {
    ...
    if (fileType === 'sql') {
        const dataSqlParser = new DataSqlParser(files, columns)
    ...
    return [
        {
            ...
            query: [
                getDefaultApiEditScheme({
                    ...
                    description: sharedI18n().t('查询表某个具体数据')
                })
            ],
            ...
            summary: sharedI18n().t('根据 id 查询某条数据详情')
        },
        {
            id: uuid(),
            name: getName('get'),
            ...
            summary: sharedI18n().t('分页获取') + tableName + sharedI18n().t('表的数据,返回该页数据和数据总数目')
        },
        {
            id: uuid(),
            name: getName('add'),
        ...
        },
        {
            ...
            query: [
                getDefaultApiEditScheme({
                    ...
                    type: API_PARAM_TYPES.NUMBER.VAL,
                    required: true,
                    description: sharedI18n().t('删除数据的 id 字段')
                })
            ],
            response: parseValue2Scheme({
                code: 0,
