 ...
 */
export const getDataSourceApiList = (tableName, columns, thirdPartDBName) => {
    ...
    const getName = (perfix, suffix = '') => {
        ...
        return perfix + upperCaseTableName.replace(/[\-_](.)/g, x => (x.slice(1)).toUpperCase()) + suffix
    }
    const apiUrlSuffix = thirdPartDBName ? `/${thirdPartDBName}` : ''
    return [
        {
            id: uuid(),
            name: getName('query'),
            url: `/api/data-source/user/tableName/${tableName}/detail${apiUrlSuffix}`,
        ...
        },
        {
            ...
            url: `/api/data-source/user/tableName/${tableName}/update/condition${apiUrlSuffix}`,
...
export const findTable = (tableName, dataSourceType, tableList, bkBaseBizList) => {
    let table
    if (['preview', 'third-part'].includes(dataSourceType)) {
        // preview 列表找到当前表
...
export const checkTable = (tables) => {
    tables.forEach(({ tableName, columns }) => {
        ...
        // 字段信息检查
        const checkKeys = ORM_KEYS.filter(key => !['enum', 'columnId'].includes(key))
