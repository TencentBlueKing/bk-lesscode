                    ...
                    tips: (dir) => {
                        return dir.code
                            ? window.i18n.t('渲染多个组件时，如果需要基于 v-for 指令的值给每个组件单独赋值。可以使用如下规则给该组件或子组件的指令或属性的表达式赋值（当前组件的 v-if 和 v-show 指令除外）：')
                                + window.i18n.t('<br> 1. 当输入的值为数组、数字或者对象时，请使用【{0}】为表达式赋值', [this.id])
                                + window.i18n.t('<br> 2. 当输入的值为数组且数组子项为对象时，请使用【{0}Item.对象key】为表达式赋值', [this.id])
                                + window.i18n.t('<br> 3. 当输入的值为数据表时，请使用【{0}Item.表字段名】为表达式赋值', [this.id])
