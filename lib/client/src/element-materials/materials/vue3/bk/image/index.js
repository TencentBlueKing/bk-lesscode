...
export default {
    ...
    group: '基础',
    order: 1,
    document: 'https://magicbox.bk.tencent.com/magicbox/3.0/image',

    events: [
        {
            name: 'loaded',
            tips: '图片加载成功触发，事件回调参数 (event: Event)'
        },
        {
            name: 'error',
            tips: '图片加载失败触发，事件回调参数 (event: Event)'
        },
        {
            name: 'close',
            tips: '关闭时触发'
        ...
        {
            name: 'change',
            tips: '图片地址改变时触发，事件回调参数 (event: Event)'
        }
    ],
    ...
    props: {
        ...
        fallback: {
            type: 'src',
            val: '',
            tips: '加载失败容错地址,会被 Slots error 覆盖'
        },
        placeholder: {
            type: 'string',
            val: '',
