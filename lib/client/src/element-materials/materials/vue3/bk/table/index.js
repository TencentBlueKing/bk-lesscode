...
export default {
    ...
    props: {
        ...
        'pagination': {
            type: ['boolean', 'object'],
            val: false,
            tips: '分页配置, 默认值为false，不启用分页; 设置为 true，启用分页功能，默认值参考分页组件 Pagination'
        },
        paginationHeihgt: {
            type: 'number',
            val: 42,
        ...
        border: {
            ...
            val: 'row',
            tips: '表格边框显示设置，可以是一个组合; 生效规则: 除非单独设置 none,否则会追加每个设置'
        },
        'outer-border': {
            type: 'boolean',
            val: false,
        ...
        isSelectedFn: {
            type: 'function',
            val: '',
            tips: '提供自定义判定当前行是否选中, 如果设置了此属性，其他判定均不生效, ({ row, cell, data }) => bool'
        },
        rowKey: {
        ...
        asyncData: {
            type: 'boolean',
            val: false,
            tips: '为避免不必要的数据修改导致的不可控组件更新,默认组件不会对传入组件的data进行任何修改,设置此属性为true则会对源数据进行同步（如：启用selection，勾选时想要自动同步到源数据）, 目前只会对指定了selectionKey的情况下才会对指定的字段数据进行更新，同时需要指定 rowKey，保证匹配到的row是正确的目标对象'
        },
        rowHover: {
            type: 'string',
        ...
        defaultSort: {
            type: 'object',
            val: {},
            tips: '如果只指定了 prop, 没有指定 order, 则默认顺序是 asc, 配置格式：{ column: order }'
        },
        isRowSelectEnable: {
