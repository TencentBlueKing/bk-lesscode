...
import {
    ...
    findRelatedVariableFromRenderEvent
...
import { getNavCustomCon, getSideNavCustomConsMap } from './static/nav-custom-con'
...
export default class Node {
    ...
    get parentNode () {
        const root = getRoot()
        let parentNode = findParent(root, this.componentId)
        // 如果内容区域找不到，再查找导航区域
        if (!parentNode) {
            // 顶部导航拖拽区域
            const nav = getNavCustomCon()
            parentNode = findParent(nav, this.componentId)
            if (parentNode) return parentNode
            // 复合导航的侧边拖拽区域
            const sideCustomCons = Object.values(getSideNavCustomConsMap())
            sideCustomCons.forEach(sideCon => {
                parentNode = findParent(sideCon, this.componentId)
                if (parentNode) return parentNode
            })
        }
        return parentNode
    ...
    get variable () {
        ...
        const eventRelatedVariableMap = findRelatedVariableFromRenderEvent(this.renderEvents)
        return Object.seal(Object.assign({}, propRelatedVariableMap, directiveRelatedVariableMap, slotRelatedVariableMap, eventRelatedVariableMap))
    }
    /**
     * @desc 关联函数
     * @returns { Object }
     ...
     */
     @readonly
     @notify
    setRenderPerms (perms = []) {
        setRenderPerms(this, perms)
        return this
    }
}
