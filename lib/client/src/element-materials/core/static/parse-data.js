...
export const traverse = (parentNode, childDataList, slot) => {
    ...
    childDataList.forEach(childData => {
        handleModelVarAndFunc(childData)
...
const handleModelVarAndFunc = (modelData) => {
    const updateStore = (data, storeGetter, storeAction, keyField) => {
        data.forEach(async (item) => {
            const existItem = store.getters[storeGetter].find((storeItem) => storeItem[keyField] === item[keyField])
            if (!existItem) {
                const params = {
                    ...item,
                    ...(storeAction === 'variable/addVariable' ? { pageCode: '', effectiveRange: 0 } : {})
                }
                await store.dispatch(storeAction, params)
            }
        })
    }
    if (modelData?.vars?.length) {
        updateStore(modelData.vars, 'variable/variableList', 'variable/addVariable', 'variableCode')
    }
    if (modelData?.functions?.length) {
        updateStore(modelData.functions, 'functions/functionList', 'functions/createFunction', 'funcCode')
        // 刷新函数列表
        store.dispatch('functions/getFunctionList', { projectId: parseInt(router.currentRoute.params.projectId),
