...
export default function (type, name, customFramework) {
     ...
     */
    const materialFramework = customFramework || framework
    const elementMaterials = materialFramework === 'vue2' ? vue2ElementMaterials : vue3ElementMaterials
    // 同一个type名称，对应不同组件，需要根据name进一步区分
    const sameTypeArr = ['bk-charts', 'chart', 'echarts', 'bk-input', 'bk-radio-group', 'widget-van-picker']

    if (sameTypeArr.indexOf(type) !== -1 && name) {
        // 根据名称中是否包含'van'来确定库
        const library = name.includes('van') ? elementMaterials.vant : elementMaterials.bk
        const material = library.find(item => item.name === name)
    ...
    const material = materialMap[materialFramework][type]
    if (!material) {
        return null
    }
    return _.cloneDeep(material)
