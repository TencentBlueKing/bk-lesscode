...
export default {
    ...
    actions: {
        ...
        getFlowData (state, params) {
            return http.get(`${prefix}/detail`, params).then(response => {
                return response.data
            })
        },
        // 获取流程详情数据，由itsm返回流程的具体配置
        getFlowConfig (state, id) {
            // return http.get(`${proxyPrefix}/service/projects/10/`, params).then(response => response.data)
            return http.get(`${prefix}/service/projects/${id}`).then(response => response.data)
        },
        // 获取流程图节点
        getFlowNodes (state, params) {
            return http.get(`${prefix}/workflow/states`, { params }).then(response => response.data)
        },
        // 获取流程图连线
        getFlowLines (state, params) {
            return http.get(`${prefix}/workflow/transitions`, { params }).then(response => response.data)
        },
        getAllUser () {
