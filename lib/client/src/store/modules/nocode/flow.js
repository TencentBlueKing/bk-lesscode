...
export default {
    ...
    state: {
        flowConfig: {},
        flowNodes: [],
        delCreateTicketPageId: null // 被删除的提单页面
    },
    mutations: {
        ...
        // 记录被删除的提单页面id
        setDeletedPageId (state, id) {
            state.delCreateTicketPageId = id
        },
    ...
    actions: {
        ...
        // 部署流程
        deployFlow ({ state }, id) {
            return http.post(`${prefix}/serviceDeploy`, { id }).then(response => response.data)
        },
        // 归档/恢复流程
        ...
        debugFlowApi ({ state }, data) {
            return http.post(`${prefix}/postman/run_api/`, data).then(response => response.data)
        }
    }
}
