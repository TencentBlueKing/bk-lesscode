...
export default {
    ...
    mutations: {
        ...
        setDataProcessConfig (state, payload) {
            const { data, projectId, projectCode } = payload
            ...
            state.nodeData.extras.webhook_info = {
                ...
                success_exp: '',
                settings: { timeout: 10 },
                url: `${BK_API_URL_TMPL.replace('{api_name}', `bk-lesscode-${projectCode}-${projectId}`)}/\{{env}}/dataManage`
            }
            state.isNodeDataChanged = true
        },
        // 设置api节点配置
        setApiNodeConfig (state, data) {
            const apiInfo = cloneDeep(data)
            state.nodeData.extras.api_info = apiInfo
            state.nodeData.extras.webhook_info = transApiToWebhook(apiInfo)
