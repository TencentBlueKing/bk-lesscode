...
export default {
    ...
    mutations: {
        ...
        resetFieldList (state) {
            state.fieldsList = []
        },
        setTableFields (state, tableFieldsConfig) {
            state.tableFieldsConfig = tableFieldsConfig || {}
        },
        resetTableFields (state) {
            state.tableFieldsConfig = {}
        }
    },
    getters: {
        fieldsList: state => state.fieldsList,
        tableFieldsConfig: state => state.tableFieldsConfig
    },
    actions: {
        // 获取字段的校验方式
        ...
        // 获取第三方系统列表
        getRemoteSystem (_, params) {
            return http.get('/api-manage/api', { params }).then(response => response.data)
        },
        // 获取特定第三方系统api列表
