...
export default {
    ...
    actions: {
        ...
        getSaasApiDoc ({ rootGetters }, params) {
            return http.get(`/projects/${rootGetters['projectId']}/saas-module-story/getBuilderApiDoc?schemaUrl=${params.schemaUrl}`).then(response => {
                const data = response.data
                return data
            })
        },
        updateSchemaApiList ({ rootGetters, commit }, params) {
            return http.get(`/projects/${rootGetters['projectId']}/saas-module-story/getBuilderApiDoc?schemaUrl=${params.schemaUrl}&type=API_DETAIL`).then(response => {
                const data = response.data
