...
export default {
    ...
    actions: {
        ...
        findOne (_, params) {
            return http.get(`${perfix}/getTableDetail`, { params }).then(response => {
                const data = response.data || []
                return data
            })
        },
        getTableDatas (_, { tableName, thirdPartDBName, ...params }) {
            return http.get(`${perfix}/user/tableName/${tableName}${thirdPartDBName ? `/${thirdPartDBName}` : ''}`, { params }).then(response => {
                const data = response.data || []
                return data
            })
        },
        ...
        queryBySql (_, params) {
            return http.post(`${perfix}/user/queryBySql/time${params.thirdPartDBName ? `/${params.thirdPartDBName}` : ''}`, params).then(response => {
                const data = response.data || []
                return data
            })
        },
