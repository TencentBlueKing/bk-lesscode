...
export default {
    ...
    actions: {
        ...
        // 获取包含流程工作台容器的页面
        getRelatedWokbenchPages () {
            return http.get(`${prefix}/queryRelatedWorkbenchPages`).then(response => response.data)
        },
        // 创建流程
        ...
        updateRelatedPages ({ state }, { tplId, params }) {
            return http.post(`${prefix}/updateRelatedPages`, params).then(response => response.data)
        },
        // 更新流程关联页面内容
        updateRelatedPageContent ({ state }, { params }) {
            return http.post(`${prefix}/tpl/related/updateRelatedPageContent`, params).then(response => response.data)
        },
        // 部署流程
