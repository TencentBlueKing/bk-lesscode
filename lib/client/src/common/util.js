...
export const execCopy = (value, message = window.i18n.t('复制成功')) => {
...
/**
 * 自动根据平台单位转换，在设计稿中样式采用px，将默认单位转为px单位或rpx单位，如没有设定过样式则按照业务默认的设计稿单位设值
 * @param {*} node 节点
 * @param {*} styleName 样式名称
 * @param {*} pxNumber 数字单位 px
 * @param {*} isGet 是否是只读取: true读取样式值，false设置样式值
 * @returns
 */
export function autoStyle (node, styleName, pxNumber, isGet = false) {
    ...
    // 检查这个节点是否已经有了某个属性(styleName的属性值)的渲染样式
    ...
    // 如果当前单位为rpx
        ...
        // 由于画布渲染问题，暂时无法支持百分比的拖拽，因此暂时使用px拖拽
        return isGet ? pxNumber + 'px' : node.setStyle(styleName, pxNumber + 'px')

        /** 百分比拖拽逻辑 */
        // const parentNodeWidth = node.$elm.parentElement?.clientWidth
        // const perctenge = parseInt(((pxNumber / parentNodeWidth) * 100))

        // return isGet ? pxNumber + 'px' : node.setStyle(styleName, perctenge + '%')
    }
    // 如果当前单位既不是px也不是rpx，则抛出异常
    throw Error(window.i18n.t('传入错误参数，无法进行单位自动转换'))
}

export function setCompProp (node, propName, propValue) {
    node.setProp(propName, LC.utils.genPropFormatValue({
        format: 'value',
        code: propValue,
...
/**
 * 重写table的表头结构, 表头溢出省略加上title属性
 */
export function renderHeaderAddTitle (h, data) {
    return h(
        'span',
        {
            attrs: {
