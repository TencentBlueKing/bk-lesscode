<template>
    ...
    >
        <section class="mb10">
            <bk-button @click="importFunction" class="mr10" theme="primary" v-bk-tooltips="{ content: '只可导入JSON文件格式，且文件内容需要是JSON格式的数组' }">{{ $t('导入') }}</bk-button>
            <slot></slot>
        </section>
            ...
            importFunction () {
                uploadFile().then((fileList) => {
                    try {
                        ...
                        this.$bkMessage({ theme: 'error', message: this.$t('文件内容需要是JSON格式的数组') })
                    }
                })
            },
