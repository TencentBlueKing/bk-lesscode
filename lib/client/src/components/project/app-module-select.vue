    ...
    <section>
        <div v-if="!showEdit" :class="$style['show-setting']">
            ...
            <span v-else>
                ...
                <span v-else @click="toBindApp" style="cursor: pointer;color: #3a84ff">，去绑定</span>
            </span>
        </div>
        <div v-else :class="$style['edit-setting']">
            <bk-select :class="$style['saas-option']" v-model="tmpAppCode" placeholder="请选择应用" :clearable="false" searchable :loading="isLoadingAppList" @toggle="toggleApplicationList" @selected="changeAppCode">
                ...
                <div slot="extension" :class="$style['selector-create-item']">
                    <a :href="`${createLinkUrl}/app/create`" target="_blank"><i class="bk-drag-icon bk-drag-add-line" />去创建应用</a>
                </div>
            </bk-select>
            <bk-select :class="$style['saas-option']" v-model="tmpModuleCode" placeholder="请选择模块" :clearable="false" searchable :loading="isLoadingModuleList" @toggle="toggleModuleList">
        ...
        computed: {
            ...
            bindInfoTips () {
                return {
                    placement: 'top',
                    width: '284px',
                    content: `必须绑定“源码管理”方式为“蓝鲸可视化开发平台提供源码包”的蓝鲸应用模块，<a target="_blank" href="${this.createLinkUrl}" style="cursor: pointer;color: #3a84ff">跳转创建</a>`
                }
            }
        },
