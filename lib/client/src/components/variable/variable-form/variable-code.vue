...
<script>
    ...
    export default {
        ...
        data () {
            return {
                tips: '/**\r\n'
                    + window.i18n.t('* 1. 计算变量适用于复杂数据计算\r\n')
                    + window.i18n.t('* 2. 计算变量内部可以执行函数和变量，最后需将执行结果返回\r\n')
                    + window.i18n.t('* 3. 如果内部使用的变量发生变化，计算变量会实时重新计算\r\n')
                    + window.i18n.t('* 4. 注意：计算变量必须有返回值，不能给计算变量设置值\r\n')
                    + window.i18n.t('* 5. 示例如下：\r\n')
