...
import { autoStyle, setCompProp } from '@/common/util.js'
...
export default function () {
    ...
    const calcPosition = (event) => {
        ...
        if (isWidthResizeable) {
            ...
            setCompSize(proxy.activeComponentData, 'width', newWidth)
            // 指定了width，right不生效
        ...
        if (isHeightResizeable) {
            ...
            setCompSize(proxy.activeComponentData, 'height', newHeight)
            // 指定了height, bottom不生效
            if (componentDataParentNode.value.type === 'free-layout') {
    ...
    const setCompSize = (componentData, key, val) => {
        if (componentData?.type === 'chart' || componentData?.type === 'bk-charts') {
            if (key === 'width' && val) {
                val = val + 'px'
            }
            setCompProp(componentData, key, val)
        } else {
            autoStyle(componentData, key, val)
        }
    }
