...
export default function () {
    ...
    const calcPosition = (event) => {
        ...
        if (isMarginTopResizeable) {
            ...
            proxy.activeComponentData.setStyle('marginTop', `${newMarginTop}px`)
            state.tipTopStyles = {
        ...
        if (isMarginLeftResizeable) {
            ...
            proxy.activeComponentData.setStyle('marginLeft', `${newMarginLeft}px`)
            state.tipLeftStyles = {
    ...
    const handleMarginTop = (event) => {
        ...
        moveStartMarginTop = parseInt(proxy.activeComponentData.style['margin-top'] || 0, 10)
        if (!dragLine) {
    ...
    const handleMarginLeft = (event) => {
        ...
        document.body.style.userSelect = 'none'
        moveStartMarginLeft = moveStartMarginTop = parseInt(proxy.activeComponentData.style['margin-left'] || 0, 10)
