...
import BkLuckyCanvas from './widget/bk-lucky-canvas'
...
export default {
    ...
    render (render) {
        ...
        // 交互式组件需要处理隐藏显示逻辑 (deprecated， 使用 staticValue 方案实现在画布内的组件隐藏显示)
        // if (this.componentData.isInteractiveComponent) {
        //     props.value = this.componentData.interactiveShow
        //     props['is-show'] = this.componentData.interactiveShow
        //     events.input = value => {
        ...
        if (['widget-bk-table', 'widget-el-table'].includes(this.componentData.type)) {
            ...
            if (this.componentData.renderProps.data.valueType === 'table-data-source') {
                ...
                props.thirdPartDBName = this.componentData.renderProps.data?.payload?.sourceData?.thirdPartDBName
            }
            // 处理分页逻辑
        ...
        const { renderProps: comRenderProps, renderDirectives: comRenderDirectives, type: comType } = this.componentData
