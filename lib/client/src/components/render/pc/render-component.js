...
export default {
    ...
    render (h, context) {
        ...
        // table 组件逻辑
        if (['widget-bk-table', 'widget-el-table'].includes(componentData.type)) {
            // 处理操作逻辑
            if (componentData.renderProps.data.valueType === 'table-data-source') {
                props.tableName = componentData.renderProps.data?.payload?.sourceData?.tableName
                props.bkDataSourceType = componentData.renderProps.data?.payload?.sourceData?.dataSourceType
                props.showOperationColumn = componentData.renderProps.data?.payload?.sourceData?.showOperationColumn
            }
            // 处理分页逻辑
            if (componentData.renderProps.pagination) {
                const paginationPayload = componentData.renderProps.pagination?.payload || {}
            ...
            props.dataValueType = componentData.renderProps.data.valueType
        }
        ...
        const renderSlotMap = Object.keys(componentData.slot).reduce((result, slotName) => {
            ...
            result[slotName] = () => slotList.map(slot => {
                ...
                return h('render-slot', {
                    props: {
                        slotData: slot,
                        key: componentData.renderSlotKey
                    }
                })
            })
            return result
