...
export default {
    ...
    methods: {
        ...
        async getFields () {
            ...
            this.loading = true
            const res = await this.$http.get('nocode-form/detail', { params: { formId: this.propsData.formId } })
            const list = JSON.parse(res.data.content).filter(field => !['description', 'divider'].includes(field.type))
