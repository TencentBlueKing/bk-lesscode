...
export default {
    ...
    created () {
        const helpMenuList = this.getCurTemplateData().helpMenuList || []
        this.setCurTemplateData({
            ...this.getCurTemplateData(),
            helpMenuList
        })
    ...
    render (render) {
        ...
        return h({
            ...
            props: {
                theme: 'light lesscode-layout-message empty-padding date-empty',
                class: { 'date-empty': this.getCurTemplateData().helpMenuList?.length <= 0 },
                arrow: false,
                offset: '-20,2',
            ...
            slots: {
                default: () => h({
                    component: 'div',
                    class: ['header-help', { 'white-theme-help-icon': this.isWhiteTopTheme }],
                    on: {
                        click: this.handleHelpMenuClick
                    },
                    children: [
                        h({
                            component: 'svg',
                ...
                content: () => {
                    return h({
                        component: 'div',
                        slot: 'content',
                        children: this.getCurTemplateData().helpMenuList?.map(helpMenu => h({
                            component: 'div',
                            class: 'help-link-item',
                            key: helpMenu.name,
                            children: helpMenu.name
                        }))
                    })
                }
            }
