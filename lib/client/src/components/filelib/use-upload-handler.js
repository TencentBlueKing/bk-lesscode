...
export default (props, uploadRef) => {
    ...
    const handleError = (err, rawFile, response) => {
        const file = getFile(rawFile)
        if (!file) return

        file.status = 'fail'
        file.statusText = response?.message
    ...
    const handleStart = (file) => {
        ...
        if (isImage && file.size > (props.maxImageSize * 1000 ** 2)) {
            uploadFile.status = 'fail'
            uploadFile.statusText = window.i18n.t('超过限制大小')
            error = new Error(uploadFile.statusText)
        }

        if (!isImage && file.size > (props.maxFileSize * 1000 ** 2)) {
            uploadFile.status = 'fail'
            uploadFile.statusText = window.i18n.t('超过限制大小')
            error = new Error(uploadFile.statusText)
        }
