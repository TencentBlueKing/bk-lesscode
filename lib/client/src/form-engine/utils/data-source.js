...
// 将数据源配置转换为实际的数据值
export async function transDataSourceValue (dataSource, formValue, instance) {
                ...
                const params = {
                    ...
                    conditions: {
                        ...
                        expressions: conditions.filter(item => item.key).map(item => {
                            ...
                            const conditionVal = type === 'field' ? formValue[value] : value
                            return { key, type, value: conditionVal, condition: logic }
                        })
                    }
                }
