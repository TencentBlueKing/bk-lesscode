...
export default {
        ...
        value: {
            handler (val) {
                if (Array.isArray(val)) {
                    ...
                    if (framework === 'vue2' && typeof val === 'string') {
                        this.localVal = val.length > 0 ? [val] : []
                    } else {
                        this.localVal = val
                    }
                }
    ...
    render (render) {
        ...
        return this.hostLoading ? null : h({
            component: framework === 'vue3' ? 'bk-member-selector' : bkUserSelectorV2,
            props: {
                value: self.localVal,
