...
export default {
    ...
    methods: {
        getFieldsVal () {
            ...
            this.fields.forEach(field => {
                ...
                if (isLesscodeVarStr(field.configure.value)) {
                    ...
                    if (varCode) {
                        const variable = variableList.find(v => v.variableCode === varCode)
                        if (variable) {
                            value[field.configure.key] = variable.defaultValue.all
                        } else {
                            throw new Error(`变量 ${varCode} 未找到`)
                        }
                    }
                } else {
        ...
        handleAdd (e) {
            ...
            const newFieldSetting = { ...defaultSetting, unsaved: true }
            this.list.splice(e.newIndex, 0, newFieldSetting)
            this.updateFields()
            this.handleActiveField(newFieldSetting)
        },
        // 拖动排序
        handleEnd (e) {
        ...
        handleCopyField (field) {
            ...
            if (index > -1) {
                ...
                this.updateFields()
                this.handleActiveField(setting)
            }
        },
    ...
    render (render) {
        ...
        const renderFields = () => h({
            ...
            attrs: {
                'data-empty-content': self.emptyTips || (self.isReuseForm ? self.$t('请选择复用数据表') : self.$t('请拖入表单编辑器控件或配置数据源')),
                'role': 'draggable'
            },
            children: self.list.map(field => h({
                ...
                class: [
                    ...
                    {
                        actived: self.activedId === field.id,
