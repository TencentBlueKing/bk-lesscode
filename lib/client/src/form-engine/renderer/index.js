...
import { getLogicalComparisonResult, isConditionsMet, computeFormulas, getRealFormula, evalFun, computeDateDiff } from '../utils/condition-parser'
...
export default {
    ...
    methods: {
            ...
            this.fields.forEach(field => {
                if ('value' in field.configure) {
                    if (isLesscodeVarStr(field.configure.value)) {
                        ...
                        const varKey = `bk_lesscode_${varCode}`
                        if (varCode && varKey in this.globalVars) {
                            value[field.configure.key] = cloneDeep(this.globalVars[varKey])
                        }
                    } else {
        ...
        getFulfillAssociationRule (rules, fieldType) {
            ...
            rules.forEach(group => {
                ...
                const isfullFill = group.relations.every(relation => {
                    const { field: relFieldKey, logic = '==', type, value } = relation
                    // 如果是评分组件
                    if (fieldType === 'rate') {
                        // 评分组件所绑定的字段默认为变量类型
                        ...
                        if (type === 'CONST') { // 字段值是否等于常量
                            return getLogicalComparisonResult(logic, this.value[relFieldKey], value)
                        }
                        // 字段值是否等于变量
                        return value in this.value && getLogicalComparisonResult(logic, this.value[relFieldKey], this.value[value])
                    }
                })
                if (isfullFill) {
                    fulfillRule = group
        ...
        async handleSubmit () {
            ...
            // 流程管理容器，人工节点处理侧滑面板保存按钮需要自定义调用
            if (typeof this.submitFn === 'function') {
                this.submitFn(this.value)
                return
            }

            try {
                this.pending = true
                const { action, flowTplId, nodeId } = this.dataSource
                if (action === 'executeFlow') {
                    const res = await this.$http.post(`/flow/tpl/${flowTplId}/createAndExecuteTask`)
                    await this.$http.post(`/flow/task/${res.data.id}/manual_node/${nodeId}/submit`, this.value)
                    this.$bkMessage({
                        theme: 'success',
                        message: this.$t('流程提单成功')
                    })
                } else {
                    const formattedVal = []
