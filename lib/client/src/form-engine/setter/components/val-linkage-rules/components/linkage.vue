...
<script>
    ...
    export default {
        ...
        data () {
            return {
                configData: cloneDeep(this.value),
                formList: [],
        ...
        watch: {
            show (val) {
                if (val) {
                    this.configData = cloneDeep(this.value)
                }
            }
        },
        created () {
                    ...
                    return !this.configData.rules.some((group, index) => {
                        const hasRelationFormEmpty = group.relations.some(item => {
                            ...
                            if (msg) {
                                this.$bkMessage({
                                    theme: 'error',
                                    message: `【${window.i18n.t('规则')}${index + 1}】${msg}`
                        ...
                        if (this.checkLoopReference(this.configData.rules, this.field.configure.key)) {
                            this.$bkMessage({
                                theme: 'error',
