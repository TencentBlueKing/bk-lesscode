...
Vue.component('display-name', DisplayName)
...
auth.requestCurrentUser().then(user => {
    ...
    if (!user.isAuthenticated) {
        ...
        // 多租户模式，初始化displayname组件
        if (window.BK_ENV_ENABLE_TENANT === '1') {
            BkUserDisplayName.configure({
                // 必填，租户 ID
                tenantId: user.tenantId || 'default',
                // 必填，网关地址
                apiBaseUrl: `${window.BK_COMPONENT_API_URL}/api/bk-user-web/prod`,
