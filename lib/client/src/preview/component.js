 ...
 */
import Vue from 'vue'
import { registerComponent, install } from 'bk-lesscode-render'
// 注入组件库
...
export const registerComponents = (projectId, versionId) => {
    return new Promise((resolve, reject) => {
        ...
        script.onload = () => {
            window.previewCustomCompontensPlugin.forEach(callback => {
                ...
                registerComponent(config.type, source)
            })
            resolve()
        }
        script.onerror = (err) => {
        ...
        registerComponent('renderColumn', widgetTableColumn)
