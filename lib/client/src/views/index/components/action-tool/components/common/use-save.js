    ...
    const recTree = node => {
        ...
        Object.keys(node.renderProps).forEach(propKey => {
            ...
            if (prop.buildInVariableType === BUILDIN_VARIABLE_TYPE_LIST[1].VAL && (!prop.payload || !prop.payload.customVariableCode)) {
                errorStack.push(`组件【${node.componentId}】的属性【${propKey}】变量类型配置为【${BUILDIN_VARIABLE_TYPE_LIST[1].NAME}】但没有选择变量`)
            }
        })
        ...
        Object.keys(node.renderSlots).forEach(slotKey => {
            ...
            if (slot.buildInVariableType === BUILDIN_VARIABLE_TYPE_LIST[1].VAL && (!slot.payload || !slot.payload.customVariableCode)) {
                errorStack.push(`组件【${node.componentId}】的【${slotMaterial.displayName}】的变量类型配置为【${BUILDIN_VARIABLE_TYPE_LIST[1].NAME}】但没有选择变量`)
            }
        })
