            ...
            if (event.enable) {
                if (event.type === EVENT_TYPE.ACTION) {
                    ...
                    event.actions?.forEach((action) => {
                        if (isEmpty(action.id) || (isEmpty(action.value) && action.type !== EVENT_ACTION_TYPE.METHOD)) {
                            errorStack.push(window.i18n.t('组件【{0}】的【{1}】【事件】的事件行为，没有设置具体的值', [node.componentId, eventKey]))
                        }
                        if (action.type === EVENT_ACTION_TYPE.COMPONENT && action.value.some((item) => (isEmpty(item.key) || isEmpty(item.value)))) {
                            errorStack.push(window.i18n.t('组件【{0}】的【{1}】【事件】的事件行为，没有设置具体的值', [node.componentId, eventKey]))
                        }
                    })
                } else {
