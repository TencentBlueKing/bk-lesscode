...
const checkFormConfig = (nodes) => {
    ...
    nodes.forEach(node => {
        ...
        if (type === 'USE_FORM' && !relatedId) {
            ...
            if (fields.length === 0) {
                fieldsEmpty.push(node.componentId)
            }
        }
    })
    ...
    if (fieldsEmpty.length > 0) {
        configErrors.push(`组件${fieldsEmpty.map(item => '【' + item + '】').join(', ')}的表单数据源为新建表方式，但没有配置表单字段`)
    }
    return configErrors
}

export const autoTableName = (pageCode, nodeId) => {
    return `${pageCode}_${nodeId}`
}
    ...
    const saveFormContainers = () => {
        ...
        const getRequests = (nodes) => {
            return nodes.map(node => {
                ...
                // 去掉表单容器内字段为新增的标识
                const fields = node.renderProps.fields.code.map(item => {
                    const field = { ...item }
                ...
                const params = {
                    ...
                    tableName: tableName || autoTableName(pageDetail.value.pageCode, node.componentId), // pageCode + componentId
        ...
        return Promise.all(getRequests(compsToUpdate)).then(responses => {
            responses.forEach((res, index) => {
                ...
                const fieldsValue = node.renderProps.fields.code.map(item => {
                    const field = { ...item }
                    delete field.unsaved
                    return field
                })
