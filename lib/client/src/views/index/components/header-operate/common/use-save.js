...
import { EVENT_TYPE, EVENT_ACTION_TYPE } from 'shared/function/constant'
import { replaceFuncKeyword } from 'shared/function/helper'

const parseFuncBody = funcBody => {
    const funcBodyContainMethod = {}
    const funcBodyContainVariable = {}
    replaceFuncKeyword(funcBody, (all, first, second, variableCode, funcStr, funcCode) => {
        if (funcCode) {
            funcBodyContainMethod[funcCode] = true
        }
        if (variableCode) {
            funcBodyContainVariable[variableCode] = true
        }
    })

    return {
        funcBodyContainMethod,
        funcBodyContainVariable
    }
}
...
const checkValueConfig = rootNode => {
    ...
    const recTree = node => {
        ...
        Object.keys(node.renderEvents).forEach(eventKey => {
            ...
            if (event.enable) {
                if (event.type === EVENT_TYPE.ACTION) {
                    if (isEmpty(event.actions)) {
                        errorStack.push(window.i18n.t('组件【{0}】的【{1}】【事件】没有设置具体事件行为', [node.componentId, eventKey]))
                    }
                    event.actions?.forEach((action) => {
                        if ((isEmpty(action.id) && action.type !== EVENT_ACTION_TYPE.LINK)
                            || (isEmpty(action.value) && action.type !== EVENT_ACTION_TYPE.METHOD)
                            || (action.type === EVENT_ACTION_TYPE.COMPONENT && action.value.some((item) => (isEmpty(item.key) || isEmpty(item.value))))
                        ) {
                            errorStack.push(window.i18n.t('组件【{0}】的【{1}】【事件】的事件行为，没有设置具体的值', [node.componentId, eventKey]))
                        }
                    })
                } else if (event.type === EVENT_TYPE.FLOW) {
                    if (isEmpty(event.flow)) {
                        errorStack.push(window.i18n.t('组件【{0}】的【{1}】【事件】没有设置具体操作流程配置', [node.componentId, eventKey]))
                    }
                    if (isEmpty(event.flow.id)) {
                        errorStack.push(window.i18n.t('组件【{0}】的【{1}】【事件】操作流程配置，没有选择具体流程', [node.componentId, eventKey]))
                    }
                    if (isEmpty(event.flow.actionName)) {
                        errorStack.push(window.i18n.t('组件【{0}】的【{1}】【事件】操作流程配置，没有选择具体操作行为', [node.componentId, eventKey]))
                    }
                } else {
                    if (isEmpty(event.methodCode)) {
                        errorStack.push(window.i18n.t('组件【{0}】的【{1}】【事件】没有设置具体函数', [node.componentId, eventKey]))
                    }
...
export default () => {
    ...
    const submit = () => {
        ...
        Object.keys(relatedMethodCodeMap).forEach(methodCode => {
            ...
            const { funcBodyContainMethod, funcBodyContainVariable } = parseFuncBody(funcbody)
            Object.assign(funcBodyContainontainMethodMap, funcBodyContainMethod)
            Object.assign(funcBodyContainontainVariableMap, funcBodyContainVariable)
