            ...
            Object
                ...
                .forEach((key) => {
                    ...
                    if (['variable', 'expression'].includes(val.format) && val.code === '') {
                        errorStack.push(`组件【${componentId}】的【pagination】的【${key}】配置为变量或表达式，但没有选择变量或者填写表达式`)
                    }
                    if ([BUILDIN_VARIABLE_TYPE_LIST[1].VAL].includes(val.buildInVariableType) && !val.customVariableCode) {
                        errorStack.push(`组件【${componentId}】的【pagination】的【${key}】的变量类型配置为【${BUILDIN_VARIABLE_TYPE_LIST[1].NAME}】但没有选择变量`)
                    }
                })
        }
    }
    ...
    const checkMethodParams = (params = [], componentId, type, key) => {
        params.forEach((param) => {
            if (['variable', 'expression'].includes(param.format) && isEmpty(param.code)) {
                errorStack.push(`组件【${componentId}】的【${type}】的【${key}】的参数配置为【${formatMap[param.format]}】但没有设置具体值`)
            }
        })
    }
    const recTree = node => {
        ...
        Object.keys(node.renderEvents).forEach(eventKey => {
            ...
            if (event.enable) {
                if (isEmpty(event.methodCode)) {
                    errorStack.push(`组件【${node.componentId}】的【事件】的【${eventKey}】没有设置具体函数`)
                }
                checkMethodParams(event.params, node.componentId, '事件', eventKey)
            }
        })
