...
export default (cmdMessage) => {
    ...
    let templateList
    Promise.all([
        store.dispatch('pageTemplate/list', { type: 'OFFCIAL', framework: 'all' }),
        store.dispatch('pageTemplate/list', { projectId: route.params.projectId, framework: 'all' })
    ]).then(([
        offcialList,
        projectTemplateList
    ]) => {
    ...
    const handleGetTemplates = () => {
        ...
        if (templates?.length > 0) {
            ...
            cmdMessage.value += [
                '',
                '# cmd',
                'There is no template for the current project.'
            ].join('\n')
        }
    }
