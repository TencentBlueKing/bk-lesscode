...
export default (cmdMessage) => {
    ...
    // 工具方法
    const handleGetCustomComponents = () => {
        ...
        window.customCompontensPlugin.forEach(registerCallback => {
            ...
            customComponents.push(config)
        })
        return customComponents
    }
    const isCustomComponent = (type) => {
        const customComponents = handleGetCustomComponents()
        return !!customComponents.some(x => x.type === type)
    }
    ...
    const handleInsert = (type, componentId) => {
        ...
        if (isExist) {
            ...
            node.isCustomComponent = isCustomComponent(type)
    ...
    const handleGetAll = () => {
        ...
        const customComponentTips = customComponents.length
            ? [
                'You can use these custom component type:',
                ...customComponents.map(component => `- ${component.type} (${component.displayName})`)
            ]
            : []
        cmdMessage.value += [
            '',
