        ...
        >
            <section v-bkloading="{ isLoading }">
                <div style="margin-bottom: 16px;font-size:14px;">
                    {{ $t('即将导出应用') }}用<span style="font-weight: bold;">【{{projectName}}】</span>{{ $t('，您将得到应用JSON文件，您可以使用该JSON文件通过“应用开发”-》“新建”-》“导入应用”来创建新应用。') }}
                </div>
                <section class="intro-container">
                    <p style="font-weight: bold;">{{ $t('注意：') }}</p>
                    <p>{{ $t('1. 导出的应用中不包含流程及其相关页面内容') }}</p>
                    <p>{{ $t('2. 如果应用内使用了“文件管理”里的文件内容，请另行对应用的文件进行导出并上传到新建的应用内，并替换文件使用链接') }}</p>
                    <p>{{ $t('3. 如果应用内使用了应用本身的自定义组件，请另行在新应用内上传使用自定义组件并保证组件ID维持不变') }}</p>
                    <p>{{ $t('4. 如果应用内使用了其它应用公开自定义组件，请确保该环境的自定义组件在新应用也能使用或自行替换') }}</p>
