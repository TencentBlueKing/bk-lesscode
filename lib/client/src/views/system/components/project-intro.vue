        ...
        data () {
            return {
                steps: [
                    ...
                    {
                        title: '本地数据库配置',
                        desc: '如果有用到数据源、表单页面等相关涉及到数据库的功能，请先在lib/server/conf/db-base.js配置本地开发环境数据库，未用到则跳过此步骤'
                    },
                    {
                        title: '登录',
                        desc: '如需本地运行， 请先到lib/server/conf/token文件中，填写dev配置中的appCode跟secret，整个框架自带登录实现，在刚打开时，如果没有登录会直接跳到登录页，如果打开后，登录过期（接口返回401状态）会弹出登录窗口'
                    },
                    {
                        title: '启动服务',
                        desc: 'npm run dev'
