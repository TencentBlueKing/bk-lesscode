<template>
    <ul class="lesscode-deploy-timeline" :class="extCls" :style="{ width: `${width}px` }">
        <li
            class="lesscode-timeline-dot"
            v-for="(item, index) in list"
            :key="index"
            :class="['lesscode-timeline-dot', makeClass(item, index)]"
            @click="toggle(item)"
            @mouseenter="handleMouseenter(item)"
            @mouseleave="handleMouseleave(item)">
            <template v-if="isNeedIconcool(item)">
                <round-loading ext-cls="lesscode-deploy-timeline-loading" v-if="item.loading" />
                <div class="lesscode-timeline-icon" v-else>
                    <i :class="['bk-drag-icon', `bk-drag-${iconEnum[item.name]}`]" :style="{ color: computedIconColor(item) }"></i>
                </div>
            </template>
            <template v-else>
                <round-loading ext-cls="lesscode-deploy-timeline-loading" v-if="item.status === 'pending'" />
                <div class="lesscode-timeline-node-icon" :class="item.status" v-else></div>
            </template>

            <div class="lesscode-timeline-section">
                <div
                    :class="['lesscode-timeline-title', { 'is-weight': !!item.stage }, { 'is-default': item.status === 'default' && !item.stage }]"
                    v-if="item.tag !== ''">
                ...
                <div class="lesscode-timeline-content" v-if="item.content">
                    {{ item.content }}
                </div>
            </div>
        </li>
...
<script>
    import roundLoading from './round-loading/index.vue'
    ...
    export default {
        ...
        data () {
            return {
                ...
                iconEnum: { build: 'flow', preparation: 'time-2', release: 'yuanma' },
                curHoverItem: {},
                curSelectedItem: {}
            }
        },
        ...
        methods: {
            ...
            makeClass (templateData, index) {
                const classPrefix = 'lesscode-timeline-item-'
                const classNames = []
...
<style lang="postcss" scoped>
    ...
    .lesscode-deploy-timeline {
        ...
        .lesscode-timeline-item-default {
            &::after {
                border-color: #dcdee5;
            }
        }
