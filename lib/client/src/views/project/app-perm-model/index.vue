        ...
        <section class="app-perm-model-head">
            <bk-alert type="info" closable style="flex: 1">
                <template slot="title">
                    ...
                    <div class="tips">
                        <p>{{ $t('注意：') }}</p>
                        <p>{{ $t('1. 应用权限模型在应用预览环境、预发布环境不生效') }}</p>
                        <p>{{ $t('2. 应用需部署至生产环境后，最新权限模型方可在生产环境生效') }}</p>
                        <p>{{ $t('3. 应用未部署至生产环境时，系统管理员默认为本应用创建者，并不可修改；应用部署至生产环境之后，系统管理员可在权限中心设置') }}</p>
                        <p>{{ $t('4. 系统管理员可以设置该应用的系统管理员，设置入口：权限中心 -> 权限管理 -> 系统管理员') }}</p>
                        <p>{{ $t('5. 系统管理员也可以设置该应用的权限审批流程，设置入口：权限中心 -> 权限管理 -> 审批流程') }}</p>
                        <p>{{ $t('6. 权限中心自定义权限默认审批流程为：【审批节点：申请人上级审批确认 -> 系统管理员审批】，系统管理员可根据自己的需求修改审批流程') }}</p>
                    </div>
                </template>
            </bk-alert>
        </section>
