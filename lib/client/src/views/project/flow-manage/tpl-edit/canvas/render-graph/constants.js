...
export const GET_INITIALIZE_FLOW_TPL_STRUCTURE = () => {
    ...
    const nodes = initialNodes.map(item => {
        return GET_NODE_DEFAULT_CONFIG(item.type, { axis: item.axis, isAutoAdd: true })
    })
    const edges = []
    ...
    switch (type) {
        ...
        case 'DataProcessing':
            return {
                ...
                sourceManualNode: '', // 人工节点id，标识当前节点为添加人工节点时自动添加的
                ...commonConfig,
                ...customConfig
            }
        case 'API':
...
export const MANUAL_NODE_BINDING_TYPES = [
    { id: 'NEW_FORM', name: window.i18n.t('新建空白表单') },
    { id: 'CITE_FORM', name: window.i18n.t('引用已有表单') },
    { id: 'USE_FORM', name: window.i18n.t('复用已有表单') }
]
